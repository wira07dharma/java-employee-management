...
//            System.out.println("PstPerkiraan.getAccountIdByNo() sql : " + sql);
    ...
    public static Vector getListAccount(Vector vectGroup, Vector vectNumber, Vector vectName, int start, int recordToGet, String sortBy, long department) {
        ...
        try {
            String sql = "SELECT DISTINCT " + fieldNames[FLD_IDPERKIRAAN]
                    ...
                    + ", " + fieldNames[FLD_TANDA_DEBET_KREDIT]                    
                    + ", " + fieldNames[FLD_WEIGHT]     
                    + ", " + fieldNames[FLD_GENERAL_ACCOUNT_LINK]     
                    + ", " + fieldNames[FLD_ACCOUNT_GROUP]     
                    + ", " + fieldNames[FLD_COMPANY_ID]     
                    + ", " + fieldNames[FLD_ARAP_ACCOUNT]     
                    + ", " + fieldNames[FLD_EXPENSE_TYPE]   
                    + ", " + fieldNames[FLD_EXPENSE_FIXED_VARIABLE]   
                    + " FROM " + TBL_PERKIRAAN;
            ...
            if (vectName != null && vectName.size() > 0) {
                for (int a = 0; a < vectName.size(); a++) {
                    if (vectName.get(a).toString() != null && vectName.get(a).toString().length() > 0) {
                        if (a == 0) {
                            strName = ((fieldNames[FLD_NAMA] + " " + sLike + "'%" + vectName.get(a) + "%'") + " OR "
                                    + (fieldNames[FLD_ACCOUNT_NAME_ENGLISH] + " " + sLike + "'%" + vectName.get(a) + "%'"));
                        } else {
                            strName = strName + " OR " + fieldNames[FLD_NAMA] + " " + sLike + "'%" + vectName.get(a) + "%'  " + " OR "
                                    + (fieldNames[FLD_ACCOUNT_NAME_ENGLISH] + " " + sLike + "'%" + vectName.get(a) + "%'");
                        }
                        strName = "(" + strName + ")";
                    }
    ...
    /**
     * @author rusdianta This method used to get account based on parent-child
     * association algorithm is using "stack" as temporary storage to find
     * association and sorting the result automatically
     * @param group, this param used when the value of "parentOid" is 0, group
     * point to the type of account that want to get
     * @param parentOid, searching process will begin with this value, if the
     * value = 0, searching will begin from the top parent of all account if
     * group = 0 and parentOid = 0, all account will be listed
     * @return Vector of Perkiraan that have been sorting based on account group
     * and account number
     */
    public static Vector getAllAccount(int group, long parentOid) {
        ...
        try {
            ...
            // check for the account with wrong setting :  parent_id=0 with level not 1, ;parent_id = coa ID it self, and fix it
            Vector vCoA = list(0,10000, fieldNames[FLD_ACCOUNT_GROUP]+"="+ group + " AND " + fieldNames[FLD_ID_PARENT]+"="+ fieldNames[FLD_IDPERKIRAAN] +
                    " OR ( " + fieldNames[FLD_ID_PARENT]+"=0 AND "+ fieldNames[FLD_LEVEL] + "<>1 )", fieldNames[FLD_NOPERKIRAAN] );
            if(vCoA!=null && vCoA.size()>0){
                for(int i=0; i<vCoA.size();i++){
                    Perkiraan account = (Perkiraan) vCoA.get(i);
                    // check of parent id problem 
                        if(account.getLevel()<=1){
                          account.setIdParent(0);
                          account.setLevel(1);
                        }else{ // check for accounts can be as parent of this account
                           Vector vCoATemp = list(0,1, fieldNames[FLD_ACCOUNT_GROUP]+"="+ account.getAccountGroup() + " AND " 
                                   + fieldNames[FLD_LEVEL]+" > 1 AND " + fieldNames[FLD_LEVEL]+" < "+ account.getLevel() 
                                   , fieldNames[FLD_LEVEL] + " DESC " );
                            if(vCoATemp!=null && vCoATemp.size()>0){
                                Perkiraan coaParentTemp = (Perkiraan) vCoATemp.get(i);
                                account.setIdParent(coaParentTemp.getOID());                              
                            }else{
                                // no acount upper the current account found set to level 1.
                                account.setIdParent(0);
                                account.setLevel(1);  
                            }
                        }
    ...
    public static Vector getAccountByGroup(int iAccGroup) {
        return getAccountByGroup(iAccGroup, ACC_POSTED);
    }
     ...
     *
     * @param iAccGroup
     * @param postedIdx : PstPerkiraan.ACC_NOTPOSTED , PstPerkiraan.ACC_POSTED
     * or -1= Header & Postable
     * @return
     */
    public static Vector getAccountByGroup(int iAccGroup, int postedIdx) {
        DBResultSet dbrs = null;
        Vector vResult = new Vector(1, 1);
        try {
            String sql = "SELECT * FROM " + TBL_PERKIRAAN
                    + " WHERE " + fieldNames[PstPerkiraan.FLD_ACCOUNT_GROUP] + " = " + iAccGroup
                    + ((postedIdx == ACC_POSTED || postedIdx == ACC_NOTPOSTED) ? (" AND " + fieldNames[PstPerkiraan.FLD_POSTABLE] + " = " + postedIdx) : "") + " ORDER BY " + fieldNames[FLD_NOPERKIRAAN];
    ...
    public static Hashtable getListCoAByCode() {
        Hashtable hListCoA = new Hashtable();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_PERKIRAAN;
            ...
            while (rs.next()) {
                Perkiraan objCoA = new Perkiraan();
                resultToObject(rs, objCoA);
                hListCoA.put(objCoA.getNoPerkiraan(), objCoA);
            }
            rs.close();
            return hListCoA;
    ...
    public static Hashtable getListCoAByOid() {
        Hashtable hListCoA = new Hashtable();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_PERKIRAAN;
            ...
            while (rs.next()) {
                Perkiraan objCoA = new Perkiraan();
                resultToObject(rs, objCoA);
                hListCoA.put("" + objCoA.getOID(), objCoA);
            }
            rs.close();
            return hListCoA;
    ...
    public static Hashtable getListCoAByCodeHeaderOnly() {
        Hashtable hListCoA = new Hashtable();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_PERKIRAAN + "WHERE " + fieldNames[FLD_POSTABLE] + "=0";
    ...
    public static Hashtable<String, Perkiraan> getListCoAByOidHeaderOnly() {
        Hashtable hListCoA = new Hashtable();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_PERKIRAAN + " WHERE " + fieldNames[FLD_POSTABLE] + "=0 ORDER BY " + fieldNames[FLD_NOPERKIRAAN];
