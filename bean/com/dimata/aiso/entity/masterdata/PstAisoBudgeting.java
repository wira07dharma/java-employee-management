...
public class PstAisoBudgeting extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language{
    ...
    public long deleteExc(long loid) throws DBException {
     ...
     public static int deleteBy(long periodeId, long perkiraanId)
            throws DBException {
        ...
        try {
            ...
            return statement.executeUpdate("DELETE FROM "+TBL_AISO_BUDGETING + " WHERE "+fieldNames[FLD_PERIODE_ID]+"="+
                    periodeId +" AND "+fieldNames[FLD_ID_PERKIRAAN]+"="+perkiraanId);
        } catch (SQLException sqlexception) {
            sqlexception.printStackTrace(System.err);
            throw new DBException(new PstAisoBudgeting(0),sqlexception);
        } finally {
                              ...
                              String strOrder)
    {
      ...
      public static Hashtable listInHashtable(long periodId){
         return listInHashtable(0,  0, fieldNames[FLD_PERIODE_ID]+"="+periodId,"");
    }
    
     public static Hashtable listInHashtable(int iStart,  int iRecToGet,
                              String strWhere,
                              String strOrder)
    {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                objBudgeting.setPeriodeId(rs.getLong(PstAisoBudgeting.fieldNames[PstAisoBudgeting.FLD_PERIODE_ID]));
                objBudgeting.setBudget(rs.getDouble(PstAisoBudgeting.fieldNames[PstAisoBudgeting.FLD_BUDGET]));                
                lists.put(""+objBudgeting.getIdPerkiraan(), objBudgeting);                
            }
