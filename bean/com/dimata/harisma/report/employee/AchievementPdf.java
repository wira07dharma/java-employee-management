...
public class AchievementPdf  extends HttpServlet
{
    ...
    throws ServletException, java.io.IOException {
        ...
        //String pathImage = "http://localhost:8080/harisma/images/achieveheader.jpg";

        String approot = FRMQueryString.requestString(request, "approot");
        System.out.println("approot : "+approot);
        String pathImage = approot+ "/images/achieveheader.jpg";
        ...
        Vector listAchievement = new Vector();
        HttpSession sess = request.getSession(true);
        try{
            listAchievement = (Vector)sess.getValue("ACHIEVE");
        }catch(Exception e){
            System.out.println("Exc : "+e.toString());
            listAchievement = new Vector();
        }

        Vector header = new Vector(1,1);
        Vector vctContent = new Vector(1,1);

        if((listAchievement != null) && (listAchievement.size()>0))
        {

              header = (Vector) listAchievement.get(0);
              document.add(getHeader(header, pathImage));

              vctContent = (Vector) listAchievement.get(1);
    ...
    private static Table getHeader(Vector vct, String pathImage) throws BadElementException, DocumentException {
        ...
        if(vct!=null && vct.size()>0){
            ...
            hcell = new Cell(new Chunk("SPECIAL ACHIEVEMENT", fontTitle));
	        hcell.setBorderColor(new Color(255,255,255));
	        hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
	        hcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
	        hcell.setColspan(6);
     ...
     private static Table getListHeader() throws BadElementException, DocumentException {
        ...
        hcell = new Cell(new Chunk("Type of Award", fontListHeader));
        hcell.setBackgroundColor(bgColor);
        hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
        hcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
        list.addCell(hcell);

        hcell = new Cell(new Chunk("Presented by", fontListHeader));
        hcell.setBackgroundColor(bgColor);
        hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
        hcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
        list.addCell(hcell);
     ...
     private static Table getContent(Vector vct, Document document, PdfWriter writer) throws BadElementException, DocumentException {
        ...
        if(vct!=null && vct.size()>0){
            for(int i=0; i<vct.size(); i++){
                ...
                SpecialAchievement sp = (SpecialAchievement)vct.get(i);
		        ...
		        hcell = new Cell(new Chunk(sp.getTypeOfAward(), fontLsContent));
		        hcell.setHorizontalAlignment(Element.ALIGN_LEFT);
		        hcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
		        list.addCell(hcell);
		
		        hcell = new Cell(new Chunk(sp.getPresentedBy(), fontLsContent));
		        hcell.setHorizontalAlignment(Element.ALIGN_LEFT);
		        hcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
		        list.addCell(hcell);
		
		        hcell = new Cell(new Chunk(Formater.formatDate(sp.getDate(), "dd MMMM yyyy"), fontLsContent));
		        hcell.setHorizontalAlignment(Element.ALIGN_CENTER);
		        hcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
		        list.addCell(hcell);
