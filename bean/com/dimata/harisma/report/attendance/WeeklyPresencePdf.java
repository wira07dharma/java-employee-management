/*
 * WeeklyNightShiftPdf.java
 *
 * Created on June 3, 2004, 6:04 PM
 */
 ...
 * @author  gedhy
 */
public class WeeklyPresencePdf extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            try{
	        vctWeeklyPresence = (Vector)sessEmpPresence.getValue("PRESENCE_RPT_WEEKLY");
            }catch(Exception e){
         ...
         //   Vector listPresenceData = new Vector(1,1);
            if(vctWeeklyPresence != null && vctWeeklyPresence.size()>0){
                try{
                        if(vctWeeklyPresence.get(0)!=null)
	            	presence = (Date) vctWeeklyPresence.get(0);
                        if(vctWeeklyPresence.get(1)!=null && oidDepartment!=0){
	                oidDepartment = Long.parseLong((String)vctWeeklyPresence.get(1));
	                department = (Department)PstDepartment.fetchExc(oidDepartment);
                        }
                        if(vctWeeklyPresence.get(6) !=null){
                        oidSection = Long.parseLong((String)vctWeeklyPresence.get(6));
                        if(oidSection !=0){
                            section = (com.dimata.harisma.entity.masterdata.Section)PstSection.fetchExc(oidSection);
                        }
                      }
                        if(vctWeeklyPresence.get(2)!=null){
	                listPresence = (Vector)vctWeeklyPresence.get(2);}
                        if(vctWeeklyPresence.get(3)!=null){
                        strWeek = String.valueOf(vctWeeklyPresence.get(3));}
                        if(vctWeeklyPresence.get(4)!=null){
                        startDate = (Date)vctWeeklyPresence.get(4);}
                        if(vctWeeklyPresence.get(5)!=null ){
                        endDate = (Date)vctWeeklyPresence.get(5);}
                        /*  if(vctWeeklyPresence.get(6)!=null ){
                       empNum = String.valueOf(vctWeeklyPresence.get(6));}
                          if(vctWeeklyPresence.get(7)!=null ){
                        fullName = String.valueOf(vctWeeklyPresence.get(7));}
                            if(vctWeeklyPresence.get(8)!=null ){
                        oidSection = Long.parseLong((String)vctWeeklyPresence.get(8));*/
                    // }
                          
                              
                        //listPresenceData = PstDepartment.listStruktur(empNum,fullName);
                }
                catch(Exception e)
                {
            ...
            String strReportTitle = //"ATTENDANCE RECORD : " + (Formater.formatDate(presence,"dd MMMM yyyy")).toUpperCase() +
                                    "WEEKLY PRESENCE REPORT : " + strWeek.toUpperCase() + " OF " + (Formater.formatDate(presence,"MMMM yyyy").toUpperCase()) +
            ...
            if(listPresence!=null && listPresence.size()>0)
            {
                ...
                {          
                    Vector itemPresence = (Vector)listPresence.get(i);
                    
                    // no, payroll and name
                    cellEmpData = new Cell(new Chunk(String.valueOf(itemPresence.get(0)),fontContent));
                    ...
                    for(int isch=3; isch<itemPresence.size()-1; isch++)
                    {
    ...
    {        
        ...
        {                        
            ...
            for(int itDetail=intStartDate; itDetail<=intEndDate; itDetail++)
            {
                ...
                datatable.setBackgroundColor(summaryColor);                
                datatable.setDefaultHorizontalAlignment(Cell.ALIGN_CENTER);
                datatable.addCell(new Phrase("Sym", font1));
                
                datatable.setDefaultColspan(1);
                datatable.setDefaultRowspan(1);
                datatable.setBackgroundColor(summaryColor);                
                datatable.setDefaultHorizontalAlignment(Cell.ALIGN_CENTER);
                datatable.addCell(new Phrase("Dur", font1));
            }
