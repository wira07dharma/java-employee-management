 ...
 * @author Dimata 007
 */
public class CustomRptXLSSummary extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //set file name
        response.setHeader("Content-Disposition", "attachment; filename=" + customRptMain.getRptMainTitle() + "_summary.xls");
        ...
        for (int ja = 0; ja < jArraySubTotal.length(); ja++) {
            ...
            cell = row.createCell((short) cols);
            cell.setCellValue(jo.getString("SUBTOTAL_TITLE"));
            cell.setCellStyle(style3);
            cols++;
        }
            ...
            for (int ja = 0; ja < jArraySubTotal.length(); ja++) {
                ...
                if (jo.get("SUBTOTAL_SHOW").equals(true) && i > 0) {
                    ...
                    for (int ja2 = 0; ja2 < jArraySubTotal.length(); ja2++) {
                        ...
                        cell = row.createCell((short) coloum);
                        cell.setCellValue(jo2.getString("SUBTOTAL_DATA"));
                        cell.setCellStyle(style2);
                        coloum++;
                    }
                    
                    cell = row.createCell((short) coloum);
                    cell.setCellValue(jo.getInt("SUBTOTAL_COUNT"));
                    cell.setCellStyle(style2);
                    coloum++;
        ...
        int lastCol = jArraySubTotal.length();
        ...
        cell = row.createCell((short) coloum);
        cell.setCellValue(grandTotalEmployee);
        cell.setCellStyle(style4);
        sheet.autoSizeColumn(coloum);
        coloum++;
