...
public class PstPeriode extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc {
    ...
    private static Vector<Periode> journalAblePeriode= null; // list periode yang bisa di journal, artinya tanggal start <= tanggal sekarang
    
    public static final String[] periodStatus = {
        "Open",
        "Closed",
        "Prepared Open"
    };
    ...
    /**
     * @return the jurnalAblePeriode
     */
    public static Vector<Periode> getJurnalAblePeriode() {
        if(journalAblePeriode==null){            
            reloadJournalAblePeriode();
        }        
        return journalAblePeriode;
    }
    
    public static void reloadJournalAblePeriode(){
        journalAblePeriode = list(0,120, fieldNames[FLD_TGLAWAL] + " <= \""+ Formater.formatDate(new Date(), "yyyy-MM-dd hh:mm:ss")+"\"", fieldNames[FLD_TGLAWAL] + " ASC "  );               
    }
    ...
    public static Periode getFirstPeriod(){
        ...
        String orderBy = PstPeriode.fieldNames[PstPeriode.FLD_TGLAWAL] + " ASC ";
        Vector lists = PstPeriode.list(0,1,"",orderBy);
    ...
    /* This method used to get the current period */
    public static Periode getActivePeriod() {
        DBResultSet dbrs = null;
        Periode per = null;
        try {
            String sql = "SELECT * FROM " + PstPeriode.TBL_PERIODE
            + " WHERE " + PstPeriode.fieldNames[PstPeriode.FLD_POSTED]
            + " = " + PstPeriode.PERIOD_OPEN  + " ORDER BY " + PstPeriode.fieldNames[PstPeriode.FLD_TGLAKHIR]
                + " ASC  LIMIT 0,1";
            //System.out.println("SQL getCurrPeriod ::::::::::::::::: "+sql);
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();            
     ...
     * this method used to get periode interval (in month)
     */
    public static int getPeriodIntervalYTD(long periodeId) {
        int result = 1;
        String whereClause = PstPeriode.fieldNames[PstPeriode.FLD_IDPERIODE] + " = " + periodeId;
        Vector vectPeriode = PstPeriode.list(0, 0, whereClause, "");
        if (vectPeriode != null && vectPeriode.size() > 0) {
    ...
    public static long getPeriodOidBefore(java.util.Date actualDate) {
        ...
        try {
            ...
            + "\" ORDER BY " + PstPeriode.fieldNames[PstPeriode.FLD_TGLAKHIR]
            + " DESC";
            int limitStart=0,recordToGet=1;
            switch (DBHandler.DBSVR_TYPE) {
                case DBHandler.DBSVR_MYSQL :
                    if(limitStart == 0 && recordToGet == 0)
                        sql = sql + "";
    ...
    public static long getPeriodOidAfter(java.util.Date actualDate) {
        long lPeriodOid = 0;
        DBResultSet dbrs = null;
        try {
            java.sql.Date sqlDate = new java.sql.Date(actualDate.getYear(), actualDate.getMonth(), actualDate.getDay());
    ...
    public static Periode getPeriodAfter(java.util.Date actualDate) {
    ...
    public static long listIdOfPeriode(java.util.Date lastDate) {
      ...
      public static Vector<Integer> listYearOfPeriode(java.util.Date theDate) {
        DBResultSet dbrs = null;
        Vector<Integer> results = new Vector();
        String strDate = (theDate!=null ? "\"" + Formater.formatDate(theDate, "yyyy") + "\"" : "");
        try{
            String sql = "SELECT DISTINCT( YEAR( " + PstPeriode.fieldNames[PstPeriode.FLD_TGLAWAL] +
            " )) AS YEAR_PRD FROM " + PstPeriode.TBL_PERIODE +
            (theDate!=null ? (" WHERE YEAR(" + PstPeriode.fieldNames[PstPeriode.FLD_TGLAWAL] +") =" + strDate):"") +
            " ORDER BY " + PstPeriode.fieldNames[PstPeriode.FLD_TGLAWAL] +
            " DESC";
            ...
            while(rs.next()) {
                results.add(rs.getInt("YEAR_PRD"));
            }
        } catch(Exception e) {
            System.out.println("Err list period : "+e.toString());
        } finally {
    ...
    /** this method used to get periodId just before current period */
    public static Periode getPeriodIdJustAfter(long currPeriodId){
        Periode periode=null;
        if(currPeriodId!=0){
            try{
                Periode currPer = fetchExc(currPeriodId);
                if(currPer==null || currPer.getOID()==0){
                    return periode;
                }
            //Vector vectLastPeriod = SessPeriode.getLastPeriodId();
            String orderBy = PstPeriode.fieldNames[PstPeriode.FLD_TGLAWAL] + " ASC ";
            String where = fieldNames[FLD_TGLAWAL] + " > '"  + Formater.formatDate(currPer.getTglAkhir(),"yyyy-MM-dd") +"' ";
            Vector vectLastPeriod = PstPeriode.list(0,1,where,orderBy);
            //System.out.println("vectLastPeriod size : "+vectLastPeriod.size());
            if(vectLastPeriod!=null && vectLastPeriod.size()>0){                
                     return (Periode)vectLastPeriod.get(0);                                    
            }
          } catch(Exception exc){              
          }
        }
    ...
    /** this method used to get periodId just before current period */
    public static Periode getPeriodJustBefore(long currPeriodId){
        Periode periode=null;
        if(currPeriodId!=0){
            try{
                Periode currPer = fetchExc(currPeriodId);
            ...
            //Vector vectLastPeriod = SessPeriode.getLastPeriodId();
            String orderBy = PstPeriode.fieldNames[PstPeriode.FLD_TGLAWAL] + " DESC ";
            String where = fieldNames[FLD_TGLAKHIR] + " < '"  + Formater.formatDate(currPer.getTglAwal(),"yyyy-MM-dd") +"' ";
            Vector vectLastPeriod = PstPeriode.list(0,1,where,orderBy);
            //System.out.println("vectLastPeriod size : "+vectLastPeriod.size());
            if(vectLastPeriod!=null && vectLastPeriod.size()>0){                
                     return (Periode)vectLastPeriod.get(0);                                    
     ...
     * this method used to get periodId for one year along specify by startDate and dueDate
     */
    public static Vector<Periode> listPerBtwStartDueDate(java.util.Date startDate,
    java.util.Date dueDate){
        Vector result = new Vector(1,1);
        ...
        try{
            ...
            " AND " + strDueDate + " ORDER BY "+ fieldNames[FLD_TGLAWAL];
    ...
    public static Periode getFirstPeriodOnYearObj(java.util.Date selectedDate){
        DBResultSet dbrs = null;
        Periode periode = null;
        String strDate = "\"" + Formater.formatDate(selectedDate,"yyyy-MM-dd") + "\"";
        try{
            ...
            " WHERE YEAR(" + PstPeriode.fieldNames[PstPeriode.FLD_TGLAWAL] + ") " +
            " = YEAR(" + strDate + ")" +
            " ORDER BY " + PstPeriode.fieldNames[PstPeriode.FLD_TGLAWAL] + " ASC LIMIT 0,1";
            //System.out.println("---> SessPeriode.getFirstPeriodOnYear() sql : " + sql);
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while(rs.next()){
