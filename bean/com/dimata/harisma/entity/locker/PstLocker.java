...
public class PstLocker extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language { 
	...
	public static final  String TBL_HR_LOCKER = "hr_locker";//"HR_LOCKER";
    ...
    public static long deleteByEmployee(long oidEmployee) {
        long lockerId = 0;
        
        try {            
            Employee emp = PstEmployee.fetchExc(oidEmployee);
            lockerId = emp.getLockerId();
        }
        catch(Exception e) {
        ...
        try{
			String sql = " DELETE FROM "+PstLocker.TBL_HR_LOCKER+                        
                         " WHERE "+PstLocker.fieldNames[PstLocker.FLD_LOCKER_ID]+
                         " = "+lockerId;
    ...
    public static boolean isLockerNumberUsed(long locationId,String lockerNum, long lockerOid){
        boolean isEnable = false;
        String whereClause = "";
        whereClause += fieldNames[FLD_LOCATION_ID]
                +" = "+locationId
                +" AND "+fieldNames[FLD_LOCKER_NUMBER]
                +" = '"+lockerNum+"'"
                +" AND "+fieldNames[FLD_LOCKER_ID]
                +" != "+lockerOid;
        Vector vList = new Vector(1,1);
        vList = list(0, 0, whereClause, null);
        if(vList.size()>0){
            return true;
        }
        return isEnable;
