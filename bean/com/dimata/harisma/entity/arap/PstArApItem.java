...
public class PstArApItem extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc {
    ...
    public static final String TBL_ARAP_ITEM = "hr_arap_item";
    ...
    public static ArApItem fetchExc(long Oid) throws DBException {
        try {
            ...
            PstArApItem pstArAp = new PstArApItem(Oid);
            aktiva.setOID(Oid);

            aktiva.setArApMainId(pstArAp.getlong(FLD_ARAP_MAIN_ID));
            aktiva.setAngsuran(pstArAp.getdouble(FLD_ANGSURAN));
            aktiva.setDueDate(pstArAp.getDate(FLD_DUE_DATE));
            aktiva.setDescription(pstArAp.getString(FLD_DESCRIPTION));
            aktiva.setArApItemStatus(pstArAp.getInt(FLD_ARAP_ITEM_STATUS));
            aktiva.setLeftToPay(pstArAp.getdouble(FLD_LEFT_TO_PAY));
            aktiva.setCurrencyId(pstArAp.getlong(FLD_CURRENCY_ID));
            aktiva.setRate(pstArAp.getdouble(FLD_RATE));
            aktiva.setSellingAktivaId(pstArAp.getlong(FLD_SELLING_AKTIVA_ID));
            aktiva.setReceiveAktivaId(pstArAp.getlong(FLD_RECEIVE_AKTIVA_ID));
    ...
    public static long insertExc(ArApItem aktiva) throws DBException {
        try {
            PstArApItem pstArAp = new PstArApItem(0);

            pstArAp.setLong(FLD_ARAP_MAIN_ID, aktiva.getArApMainId());
            pstArAp.setDouble(FLD_ANGSURAN, aktiva.getAngsuran());
            pstArAp.setDate(FLD_DUE_DATE, aktiva.getDueDate());
            pstArAp.setString(FLD_DESCRIPTION, aktiva.getDescription());
            pstArAp.setInt(FLD_ARAP_ITEM_STATUS, aktiva.getArApItemStatus());
            pstArAp.setDouble(FLD_LEFT_TO_PAY, aktiva.getLeftToPay());
            pstArAp.setLong(FLD_CURRENCY_ID, aktiva.getCurrencyId());
            pstArAp.setDouble(FLD_RATE, aktiva.getRate());
            pstArAp.setLong(FLD_SELLING_AKTIVA_ID, aktiva.getSellingAktivaId());
            pstArAp.setLong(FLD_RECEIVE_AKTIVA_ID, aktiva.getReceiveAktivaId());
            
            pstArAp.insert();
            aktiva.setOID(pstArAp.getlong(FLD_ANGSURAN));
        } catch (DBException dbe) {
            throw dbe;
        } catch (Exception e) {
            throw new DBException(new PstArApItem(0), DBException.UNKNOWN);
    ...
    public static long updateExc(ArApItem aktiva, Connection objConnection) throws DBException {
        try {
            ...
            if (aktiva != null && aktiva.getOID() != 0) {
                PstArApItem pstArAp = new PstArApItem(aktiva.getOID());
                pstArAp.setLong(FLD_ARAP_MAIN_ID, aktiva.getArApMainId());
                pstArAp.setDouble(FLD_ANGSURAN, aktiva.getAngsuran());
                pstArAp.setDate(FLD_DUE_DATE, aktiva.getDueDate());
                pstArAp.setString(FLD_DESCRIPTION, aktiva.getDescription());
		...
		if(objConnection == null){
		    ...
		    pstArAp.updateTran(objConnection);
		}
    ...
    public static Vector getTotalAngsuranWithEmp(long empId, Date startPeriod, Date endPeriod, long dedId) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        
        try {
            String sql = "SELECT * FROM hr_arap_item hai "+
                    "INNER JOIN hr_arap_main ham "+
                    "ON ham.arap_main_id = hai.arap_main_id "+
                    "WHERE (hai.due_date BETWEEN '"+startPeriod+"' AND '"+endPeriod+"')"+
                    " AND hai.arap_item_status = 0"+
                    " AND ham.employee_id = '"+empId+"'"+
                    " AND ham.component_deduction_id = '"+dedId+"'";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
    ...
    public static void deleteByMainId(long oid) {
        String whereClause = PstArApItem.fieldNames[PstArApItem.FLD_ARAP_MAIN_ID] + "=" + oid + " AND  ANGSURAN = LEFT_TO_PAY";
        try {
            String sql = " DELETE FROM " + TBL_ARAP_ITEM;
            if (whereClause != null && whereClause.length() > 0)
                sql = sql + " WHERE " + whereClause;
            DBHandler.delete(sql);
        } catch (Exception e) {
            System.out.println(e);
        } finally {            
        }
