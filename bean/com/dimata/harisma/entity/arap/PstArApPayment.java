...
public class PstArApPayment extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc {
    ...
    public static final String TBL_ARAP_PAYMENT = "hr_arap_payment";
    ...
    public static ArApPayment fetchExc(long Oid) throws DBException {
        try {
            ...
            arap.setEmployeeId(pstArApPayment.getlong(FLD_EMPLOYEE_ID));
            arap.setPaymentType(pstArApPayment.getInt(FLD_PAYMENT_TYPE));
    ...
    public static long insertExc(ArApPayment arap) throws DBException {
        try {
            ...
            pstArApPayment.setLong(FLD_EMPLOYEE_ID, arap.getEmployeeId());
            pstArApPayment.setInt(FLD_PAYMENT_TYPE, arap.getPaymentType());
    ...
    public static long updateExc(ArApPayment arap, Connection objConnection) throws DBException {
        try {
            if (arap != null && arap.getOID() != 0) {
                ...
                pstArApPayment.setInt(FLD_ARAP_TYPE, arap.getArApType());
		pstArApPayment.setLong(FLD_EMPLOYEE_ID, arap.getEmployeeId());
		pstArApPayment.setLong(FLD_PAYMENT_TYPE, arap.getPaymentType());
    ...
    private static void resultToObject(ResultSet rs, ArApPayment arap) {
        try {
            ...
            arap.setEmployeeId(rs.getLong(PstArApPayment.fieldNames[PstArApPayment.FLD_EMPLOYEE_ID]));
            arap.setPaymentType(rs.getInt(PstArApPayment.fieldNames[PstArApPayment.FLD_PAYMENT_TYPE]));
    ...
    public static void deleteByMainId(long oid) {
        String whereClause = PstArApPayment.fieldNames[PstArApPayment.FLD_ARAP_MAIN_ID] + "=" + oid;
        try {
            String sql = " DELETE FROM " + TBL_ARAP_PAYMENT;
            if (whereClause != null && whereClause.length() > 0)
                sql = sql + " WHERE " + whereClause;
