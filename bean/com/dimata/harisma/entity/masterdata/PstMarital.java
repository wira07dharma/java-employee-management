...
public class PstMarital extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language { 
	...
	public static final  String TBL_HR_MARITAL = "hr_marital";//"HR_MARITAL";
        ...
        public static final int FLD_MARITAL_STATUS_TAX = 4;
	...
	public static Marital fetchExc(long oid) throws DBException{ 
		try{ 
			...
			marital.setNumOfChildren(pstMarital.getInt(FLD_NUM_OF_CHILDREN));
                        marital.setMaritalStatusTax(pstMarital.getInt(FLD_MARITAL_STATUS_TAX));
	...
	public static long insertExc(Marital marital) throws DBException{ 
		try{ 
			...
			pstMarital.setInt(FLD_NUM_OF_CHILDREN, marital.getNumOfChildren());
                        pstMarital.setInt(FLD_MARITAL_STATUS_TAX, marital.getMaritalStatusTax());
	...
	public static Vector<Marital>  list(int limitStart,int recordToGet, String whereClause, String order){
        ...
        public static Hashtable<String,Marital> listMap(int limitStart,int recordToGet, String whereClause, String order){
		Hashtable<String,Marital> lists = new Hashtable(); 
		DBResultSet dbrs = null;
		try {
			String sql = "SELECT * FROM " + TBL_HR_MARITAL; 
			if(whereClause != null && whereClause.length() > 0)
			...
			while(rs.next()) {
				Marital marital = new Marital();
				resultToObject(rs, marital);
				lists.put(""+marital.getOID(), marital);
			}
			rs.close();
			return lists;
	...
	private static void resultToObject(ResultSet rs, Marital marital){
		try{
			...
			marital.setNumOfChildren(rs.getInt(PstMarital.fieldNames[PstMarital.FLD_NUM_OF_CHILDREN]));
                        marital.setMaritalStatusTax(rs.getInt(PstMarital.fieldNames[PstMarital.FLD_MARITAL_STATUS_TAX]));
