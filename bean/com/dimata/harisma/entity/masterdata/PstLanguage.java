...
public class PstLanguage extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language { 
	...
	public static final  String TBL_HR_LANGUAGE = "hr_language";//"HR_LANGUAGE";
	...
	public static boolean checkOID(long languageId){
        ...
        public static String getLangName(long oid){
            String name = "";
            DBResultSet dbrs = null;
            try{
                String sql = "SELECT LANGUAGE FROM " + TBL_HR_LANGUAGE + " WHERE LANGUAGE_ID=" + oid;

                dbrs = DBHandler.execQueryResult(sql);
                ResultSet rs = dbrs.getResultSet();

                while(rs.next()){
                    name = rs.getString(PstLanguage.fieldNames[PstLanguage.FLD_LANGUAGE]);
                }
