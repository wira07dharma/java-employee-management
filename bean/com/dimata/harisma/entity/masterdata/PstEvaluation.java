...
public class PstEvaluation extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language { 
	...
	public static final  String TBL_HR_EVALUATION = "hr_evaluation";//"HR_EVALUATION";
	...
	public static final  int FLD_DESRIPTION = 3;
        public static final  int FLD_MAX_PERCENTAGE=4;
        public static final  int FLD_EVAL_TYPE =5;
        public static final  int FLD_MAX_POINT =6;
	...
	public static Evaluation fetchExc(long oid) throws DBException{ 
		try{ 
			...
			evaluation.setMaxPercentage(pstEvaluation.getdouble(FLD_MAX_PERCENTAGE));
                        evaluation.setEvalType(pstEvaluation.getLong(FLD_EVAL_TYPE));
                        evaluation.setMaxPoint(pstEvaluation.getInt(FLD_MAX_POINT));
			return evaluation; 
	...
	public static long insertExc(Evaluation evaluation) throws DBException{ 
		try{ 
			...
			pstEvaluation.setDouble(FLD_MAX_PERCENTAGE, evaluation.getMaxPercentage());
                        pstEvaluation.setLong(FLD_EVAL_TYPE, evaluation.getEvalType());
                        pstEvaluation.setInt(FLD_MAX_POINT, evaluation.getMaxPoint());
	...
	public static Vector list(int limitStart,int recordToGet, String whereClause, String order){
        ...
        public static Evaluation getByMaxPoint(float point, long typeID){
		DBResultSet dbrs = null;
		try {
			String sql = "SELECT * FROM " + TBL_HR_EVALUATION 
			 + " WHERE " + fieldNames[FLD_MAX_POINT] +">="+ point + " AND " + fieldNames[FLD_EVAL_TYPE] + "=" + typeID
			 + " ORDER BY " + fieldNames[FLD_MAX_POINT] +" ASC "
			 + " LIMIT 0,1";
			dbrs = DBHandler.execQueryResult(sql);
			ResultSet rs = dbrs.getResultSet();
                        Evaluation evaluation = new Evaluation();
        ...
        public static Evaluation getByMaxPercentage(float point, long typeID){
		DBResultSet dbrs = null;
		try {
			String sql = "SELECT * FROM " + TBL_HR_EVALUATION 
			 + " WHERE " + fieldNames[FLD_MAX_PERCENTAGE] +">="+ point + " AND " + fieldNames[FLD_EVAL_TYPE] + "=" + typeID
			 + " ORDER BY " + fieldNames[FLD_MAX_PERCENTAGE] +" ASC "
			 + " LIMIT 0,1";
			dbrs = DBHandler.execQueryResult(sql);
			ResultSet rs = dbrs.getResultSet();
                        Evaluation evaluation = new Evaluation();
	...
	private static void resultToObject(ResultSet rs, Evaluation evaluation){
		try{
			...
			evaluation.setMaxPercentage(rs.getDouble(PstEvaluation.fieldNames[PstEvaluation.FLD_MAX_PERCENTAGE]));
                        evaluation.setEvalType(rs.getLong(PstEvaluation.fieldNames[PstEvaluation.FLD_EVAL_TYPE]));
                        evaluation.setMaxPoint(rs.getInt(PstEvaluation.fieldNames[FLD_MAX_POINT]));
		}catch(Exception e){ }
	}
