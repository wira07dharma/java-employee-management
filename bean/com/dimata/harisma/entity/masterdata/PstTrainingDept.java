...
/* package java */ 
import com.dimata.harisma.entity.employee.PstTrainingActivityPlan;
import java.io.*
;
import java.sql.*
;import java.util.*
...
public class PstTrainingDept extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language { 
	...
	public static final  String TBL_HR_TRAINING_DEPT = "hr_training_dept";//"HR_TRAINING_DEPT";
    ...
    public static Vector getTrainingByDepartment(long depOID){
        ...
        try{
            String sql = "SELECT DISTINCT HT.* FROM "+PstTrainingDept.TBL_HR_TRAINING_DEPT+" TD "+
                        ...
                        " ORDER BY HT."+PstTraining.fieldNames[PstTraining.FLD_NAME] ;
    ...
    public static Vector getTrainingByDept(long depOID){
        DBResultSet dbrs = null;
        Vector result = new Vector(1,1);
        Vector resultTraining = new Vector(1,1);
        String variable="";
        try{
            ...
            String ssql="";
            if(depOID==0){
                ssql= " SELECT TX." + PstTrainingDept.fieldNames[PstTrainingDept.FLD_TRAINING_ID] +
                         " FROM " + PstTrainingDept.TBL_HR_TRAINING_DEPT + " TX " +
                         " GROUP BY TX." + PstTrainingDept.fieldNames[PstTrainingDept.FLD_TRAINING_ID] +
                         " HAVING COUNT(TX." + PstTrainingDept.fieldNames[PstTrainingDept.FLD_DEPARTMENT_ID] + ") > 1" ;
                dbrs=DBHandler.execQueryResult(ssql);
                ResultSet rs = dbrs.getResultSet();
                while(rs.next()){
                  
                   long oidTrainingDept=rs.getLong(PstTrainingDept.fieldNames[PstTrainingDept.FLD_TRAINING_ID]);
                   resultTraining.add(oidTrainingDept);
            ...
            String sql = "SELECT DISTINCT HT.* FROM "+PstTrainingDept.TBL_HR_TRAINING_DEPT+" TD "+
                        ...
                        " = TD."+PstTrainingDept.fieldNames[PstTrainingDept.FLD_TRAINING_ID] ;
                    if(depOID==0){
                        sql=sql+" AND TD." + PstTrainingActivityPlan.fieldNames[PstTrainingActivityPlan.FLD_TRAINING_ID]  +
                        " IN (" ;
                        if(resultTraining!=null && resultTraining.size()>0){
                               for(int x=0;x<resultTraining.size();x++){
                    int hasil = x-resultTraining.size();
                  long oidTraining=(Long)resultTraining.get(x);
                ...
                sql=sql + variable+")";
                        ...
                        sql += " WHERE TD."+PstTrainingDept.fieldNames[PstTrainingDept.FLD_DEPARTMENT_ID]+" = "+depOID +
                                    " AND TD." + PstTrainingActivityPlan.fieldNames[PstTrainingActivityPlan.FLD_TRAINING_ID] + 
                                    " NOT IN ("  +
                                        " SELECT TX." + PstTrainingDept.fieldNames[PstTrainingDept.FLD_TRAINING_ID] +
                                        " FROM " + PstTrainingDept.TBL_HR_TRAINING_DEPT + " TX " +
                                        " GROUP BY TX." + PstTrainingDept.fieldNames[PstTrainingDept.FLD_TRAINING_ID] +
                                        " HAVING COUNT(TX." + PstTrainingDept.fieldNames[PstTrainingDept.FLD_DEPARTMENT_ID] + ") > 1" +                                                    
                                    ")";
                        ...
                        /*if(depOID > 0) {
                             ...
                             sql += " AND TD." + PstTrainingActivityPlan.fieldNames[PstTrainingActivityPlan.FLD_TRAINING_ID] + 
                                    " IN ("  +
                                        " SELECT TX." + PstTrainingDept.fieldNames[PstTrainingDept.FLD_TRAINING_ID] +
                                        " FROM " + PstTrainingDept.TBL_HR_TRAINING_DEPT + " TX " +
                                        " GROUP BY TX." + PstTrainingDept.fieldNames[PstTrainingDept.FLD_TRAINING_ID] +
                 ...
                 sql += " ORDER BY HT."+PstTraining.fieldNames[PstTraining.FLD_NAME] ;
    ...
    public static Vector getDepartmentByTraining(long trainOID){
        DBResultSet dbrs = null;
        Vector result = new Vector(1,1);
        
        try{
            String sql = " SELECT TD."+PstTrainingDept.fieldNames[PstTrainingDept.FLD_DEPARTMENT_ID]+
                         " FROM "+PstTrainingDept.TBL_HR_TRAINING_DEPT+" TD "+
                         " INNER JOIN "+PstTrainingActivityPlan.TBL_HR_TRAINING_ACTIVITY_PLAN+" TP " +
                         " ON TD."+PstTrainingDept.fieldNames[PstTrainingDept.FLD_TRAINING_ID]+
                         " = TP."+PstTrainingActivityPlan.fieldNames[PstTrainingActivityPlan.FLD_TRAINING_ID]+
                         " WHERE TD."+PstTrainingDept.fieldNames[PstTrainingDept.FLD_TRAINING_ID]+" = "+trainOID +
                         " GROUP BY TD."+PstTrainingDept.fieldNames[PstTrainingDept.FLD_DEPARTMENT_ID];
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
    ...
    public static Vector getTrainingByDepartment(long depOID, long type){
        DBResultSet dbrs = null;
        Vector result = new Vector(1,1);
        ...
        try{       
            String sql = " SELECT DISTINCT HT.* FROM "+ PstTraining.TBL_HR_TRAINING +" HT "+
                         " INNER JOIN "+ PstTrainingDept.TBL_HR_TRAINING_DEPT + " TD " +
                         " ON HT." + PstTraining.fieldNames[PstTraining.FLD_TRAINING_ID]+
                         " = TD." + PstTrainingDept.fieldNames[PstTrainingDept.FLD_TRAINING_ID] ;

                         if(depOID != DEPT_ALL) {
                            if(type != TYPE_ALL) {
                                sql += " WHERE TD."+PstTrainingDept.fieldNames[PstTrainingDept.FLD_DEPARTMENT_ID]+" = "+depOID;
                                sql += " AND HT."+PstTraining.fieldNames[PstTraining.FLD_TYPE] + "=" + type; 
                            }
                            else {
                                sql += " WHERE TD."+PstTrainingDept.fieldNames[PstTrainingDept.FLD_DEPARTMENT_ID]+" = "+depOID;
                            }
                            ...
                            if(type != TYPE_ALL) {
                                sql += " WHERE HT."+PstTraining.fieldNames[PstTraining.FLD_TYPE] + "=" + type; 
                            }
                            else {
                                ;
                            }
                         ...
                         sql = sql + " ORDER BY HT."+PstTraining.fieldNames[PstTraining.FLD_NAME] ;
