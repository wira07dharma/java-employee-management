...
public class PstNegara extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
	...
	public static Vector list(int limitStart,int recordToGet, String whereClause, String order){
        ...
        public static Hashtable<String, Negara> listMap(int limitStart,int recordToGet, String whereClause, String order){
		Hashtable<String, Negara>  lists = new Hashtable();
		DBResultSet dbrs = null;
		try {
			String sql = "SELECT * FROM " + TBL_BKD_NEGARA;
			if(whereClause != null && whereClause.length() > 0)
			...
			while(rs.next()) {
				Negara negara = new Negara();
				resultToObject(rs, negara);
				lists.put(""+negara.getOID(), negara);
			}
			rs.close();
			return lists;
