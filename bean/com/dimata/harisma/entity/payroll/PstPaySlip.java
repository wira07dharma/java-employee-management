...
public class PstPaySlip extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final String TBL_PAY_SLIP = "pay_slip";//"PAY_SLIP";
    ...
    public static String NPWP_DESCRIPTION = "total NPWP";
     //by priska 2016-05-10
    public static String DANA_PENDIDIKAN = "DANA_PENDIDIKAN";
    public static String DANA_PENDIDIKAN_DESCRIPTION = "DANA_PENDIDIKAN";
    
     //by priska 2016-05-10
    public static String RESIGNED_REASON = "RESIGNED_REASON";
    public static String RESIGNED_REASON_DESCRIPTION = "RESIGNED_REASON DESCRIPTION";
            //by priska 2015-04-18
    ...
    public static long insertExcWithDetail(PaySlip paySlip) throws DBException {
        ...
        try {
            ...
            if (oid != 0) {
                ...
                boolean isResigned = PstEmployee.checkResignedOnPeriod(paySlip.getEmployeeId(), paySlip.getPeriodId());
                if (isResigned){
                    PstPaySlip.deleteExc(oid);
                } else {
                    paySlip.setOID(oid);
                    PstPaySlip.updateExc(paySlip);
                }
    ...
    public static double getValuemapping(Date fromdate, Date todate, long employeeId, SalaryLevelDetail salaryComp) {
        ...
        try {
            String sql = " SELECT * FROM " + PstValue_Mapping.TBL_VALUE_MAPPING + " WHERE "
                    ...
                    + " >= \"" + Formater.formatDate(fromdate, "yyyy-MM-dd  00:00:00") + "\") OR (END_DATE = \"0000-00-00  00:00:00\")  OR ( END_DATE IS NULL ) )"
					+ " ORDER BY START_DATE";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            ...
            while (rs.next()) {
               // Employee employee = PstEmployee.fetchExc(employeeId);
                ...
                long VmTaxMaritalId     = rs.getLong(PstValue_Mapping.fieldNames[PstValue_Mapping.FLD_TAX_MARITAL_ID]);
//                int losFromInDay   = rs.getInt(PstValue_Mapping.fieldNames[PstValue_Mapping.FLD_LOS_FROM_IN_DAY]);
//                int losFromInMonth = rs.getInt(PstValue_Mapping.fieldNames[PstValue_Mapping.FLD_LOS_FROM_IN_MONTH]);
//                int losFromInYear  = rs.getInt(PstValue_Mapping.fieldNames[PstValue_Mapping.FLD_LOS_FROM_IN_YEAR]);
//                int losToInDay     = rs.getInt(PstValue_Mapping.fieldNames[PstValue_Mapping.FLD_LOS_TO_IN_DAY]);
//                int losToInMonth   = rs.getInt(PstValue_Mapping.fieldNames[PstValue_Mapping.FLD_LOS_TO_IN_MONTH]);
//                int losToInYear    = rs.getInt(PstValue_Mapping.fieldNames[PstValue_Mapping.FLD_LOS_TO_IN_YEAR]);
//                int losCurrentDate = rs.getInt(PstValue_Mapping.fieldNames[PstValue_Mapping.FLD_LOS_CURRENT_DATE]);
//                Date losPerCurrentDate = rs.getDate(PstValue_Mapping.fieldNames[PstValue_Mapping.FLD_LOS_PER_CURRENT_DATE]);
                ...
                if ((VmMaritalId != 0) && (VmMaritalId > 0)) {
                    if (VmMaritalId != employee.getTaxMaritalId()) {
                        nilaitf = false;
                    }
                }
...
//                if ((losPerCurrentDate!= null) || (losCurrentDate != 0) ){
//                    Date fromDate = new Date(employee.getCommencingDate().getYear()+losFromInYear, employee.getCommencingDate().getMonth()+losFromInMonth, employee.getCommencingDate().getDate()+losFromInDay);
//                Date toDate = new Date(employee.getCommencingDate().getYear()+losToInYear, employee.getCommencingDate().getMonth()+losToInMonth, employee.getCommencingDate().getDate()+losToInDay);
//                
...
//                }
                ...
                if ((VmTaxMaritalId != 0) && (VmTaxMaritalId > 0)) {
                    if (VmTaxMaritalId != employee.getTaxMaritalId()) {
                        nilaitf = false;
                    }
                }
                if (nilaitf) {
    ...
    public static String calculatePaySlipComponent(
        ...
        try {
            ...
            System.out.println("Exeception CLIENT_NAME:" + e);
        }
...
//            }
        ...
        if (checkString(formula, GET_AGE) > -1) {
            ...
            //karena bpjs di borobudur dapatnya satu bulan lebih awal dari hari kelahiran 
            if (ClientName.equals("BOROBUDUR")){
                 birthDate.setMonth(birthDate.getMonth()-1);
            }
            long monthDiff = DateCalc.monthDifference(birthDate, newDate);
            double yearDiferent = (monthDiff/12);
            
            
            formSQL = formSQL.replaceAll(GET_AGE, "" + yearDiferent);
...
//
//                }
//                pajak1.setPtkpDiriSendiri(ptkpDiriSendiri);
//                pajak1.setPtkpKawin(ptkpKawin);
//                pajak1.setPtkpKawinAnak1(ptkpAnak1);
//                pajak1.setPtkpKawinAnak2(ptkpAnak2);
//                pajak1.setPtkpKawinAnak3(ptkpAnak3);
//                pajak1.setBiayaJabatanMaksimal(jbtanMax);
//                pajak1.setBiayaJabatanPersen(jabatanPersen);
//                pajak1.setNettoPercenKonsultan(nettoPercenKonsultan);
//                pajak1.setPercen1(percen1);
//                pajak1.setPercen2(percen2);
//                pajak1.setPercen3(percen3);
//                pajak1.setPercen4(percen4);
//                pajak1.setRange1(range1);
//                pajak1.setRange2(range2);
//                pajak1.setRange3(range3);
//
//            } catch (Exception exc) {
//
//            }
            ...
            double total_tax = TaxCalculator.calcSalaryTax(paySlip.getEmployeeId(), payPeriod, prevPeriod, pajak);
                   total_tax = TaxCalculator.calcSalaryTax(paySlip.getEmployeeId(), payPeriod, prevPeriod, pajak);
            formSQL = formSQL.replaceAll(PAY_COMP_SALARY_TAX, "" + total_tax);
            newTotalTax = total_tax;
        }
        
        if (checkString(formula, DANA_PENDIDIKAN) > -1) {
            formSQL = formSQL.replaceAll(DANA_PENDIDIKAN, "" + employee.getDanaPendidikan() + "");
            System.out.print(""+employee.getFullName()+"-"+employee.getDanaPendidikan());
        }

        if (checkString(formula, RESIGNED_REASON) > -1) {
            String resignReasonName = "";
            try{
                resignReasonName = PstResignedReason.GetResignedReasonName(employee.getResignedReasonId());
            }catch (Exception e){}
            formSQL = formSQL.replaceAll(RESIGNED_REASON, "" + resignReasonName + "");
        }
...
//        }
    ...
    public static double getBenefitEmployee(long periodId, long employeeId, int part) {
        ...
        try {
            ...
            while (rs.next()) {
                if (part == 1) {
                    amount = amount + rs.getDouble(PstBenefitPeriodEmp.fieldNames[PstBenefitPeriodEmp.FLD_AMOUNT_PART_1]);
                } else {
                    amount = amount + rs.getDouble(PstBenefitPeriodEmp.fieldNames[PstBenefitPeriodEmp.FLD_AMOUNT_PART_2]);
                }
            }
            rs.close();
            return amount;
    ...
    public static int getJumlahPayslip( long employeeId, String Tahun ) {
        ...
        try {
            ...
            //penambahan dari borobudur jika tidak resign maka 12
            //Gunadi 15-12-21017
            Employee employee = new Employee();
            try{
                employee = PstEmployee.fetchExc(employeeId);
            }catch (Exception e){System.out.println("jika tidak resign maka 12");
            }
     ...
     public static  int deletePayslipwithoutComp(long oidPeriod) {
        try {
            String sql = "DELETE FROM pay_slip WHERE pay_slip_id NOT IN (SELECT pay_slip_id FROM pay_slip_comp) AND period_id = "+oidPeriod+";";
            int status = DBHandler.execUpdate(sql);
            return 0;
        } catch (Exception exc) {
     ...
     public static int deleteResignedEmpPayslip(long oidPeriod) {
         try {
            String sql = "DELETE FROM pay_slip WHERE pay_slip_id IN ("
                       + "SELECT ps.PAY_SLIP_ID FROM pay_slip AS ps INNER JOIN hr_employee AS emp ON ps.EMPLOYEE_ID = emp.EMPLOYEE_ID "
                       + "INNER JOIN pay_period AS pp ON ps.PERIOD_ID = pp.PERIOD_ID WHERE emp.RESIGNED_DATE < pp.START_DATE AND pp.PERIOD_ID = "+oidPeriod+");";
            int status = DBHandler.execUpdate(sql);
            return 0;
        } catch (Exception exc) {
            System.out.println("error delete  " + exc.toString());
