 ...
 * @author Ganki Priska
 */
public class AttPepito implements AttTransfer_I, Runnable {
...
//port 3307‏
//user : root‏
//password : dianpramana‏  
//database : mybase_pepito‏

    String dsnName="localhost:3307";/// 205.101.0.214:3306 alamat tmpt mysql
    String user="root";//profinger
    String password="dianpramana";//""
    String dbName="mybase_pepito";//melia
   
    // Data untuk database machine
    String tableName="absensi";   // nama table untuk database mesin
    String machineNumber="trans_mesin"; // field name - nomor mesin
    String employeeID="trans_kode";    // field name - id karyawan di mesin data 
                            //in-out bisa berbeda dengan employee ID di databank harisma                             
    String checkTime ="trans_jam";    // field name - waktu attendance
    String checkDate ="trans_tgl";    // field name - tanggal attendance 
    String checkType ="trans_status";    // field name - jenis IN, OUT, OUT-PERSONAL, IN-PERSONAL, OUT ON DUTY, CALLBACLsesuai 
                             // tergantung dari jenis mesin yang disupport pada saat masuk harisma akan di convert 
    String verifyCode = "trans_flag";  // field name - apakah sukses di verify atau tidak
    ...
    String dataIDFld = "data_id"; // ID dari att_log(mesin absennya)
    ...
    int flagSdhSelesaiCountMencariDataTgl=1;
     ...
     public AttPepito() {
            initClass(100, "AttPepito", user,password, tableName,
            "trans_kode", "trans_mesin", "trans_jam", "trans_status", "trans_flag", "status"); 
