...
/**
 * @Note : Table CHECKINCHECKOUT di tambahakan columns : STATUS dengan tipe :
 * tinyint default=0
 * @author ktanjana
 */
public class AttHitfptaBPD implements AttTransfer_I, Runnable {
    ...
    // database ODBC
    String dsnName = "AttHitfptaBPD";
    String user = "Admin";
    String password = "ithITtECH";
    // Data untuk database machine
    String tableName = "PersonalLog";   // nama table untuk database mesin
    String machineNumber = "TerminalID"; // field name - nomor mesin
    String dataIDFld = "DATA_ID"; // field name - nomor mesin
    String employee_num = "FingerPrintID"; // field di database mesin untuk payroll number
    String employeeID = "FingerPrintID";    // field name - id karyawan di mesin data 
    //in-out bisa berbeda dengan employee ID di databank harisma                             
    String checkTime = "DateTime";    // field name - waktu attendance
    ...
    String verifyCode = "VerifyMode";  // field name - apakah sukses di verify atau tidak
    String status = "STATUS";       // field name - status yang akan di set oleh harisma setelah data diambil : 
    ...
    public AttHitfptaBPD() {
        initClass(10, "AttHitfptaBPD", "Admin", "ithITtECH", "PersonalLog",
                "FingerPrintID", "TerminalID", "DateTime", "", "VerifyMode", "STATUS");
    }
    ...
    public void run() {
        ...
        while (this.running) {
            ...
            try {
                ...
                sql = "select u." + this.employee_num 
                        + ",a." + this.checkTime 
                        //+ ",a." + this.checkType //tidak memakai check type
                        + ",a." + this.machineNumber 
                        + ",a." + this.dataIDFld 
                        + " from " + this.tableName 
                        + " a left join Employee u on a." + this.employeeID
                        + "= u." + this.employeeID + " where "
                        + this.status + "=" + STATUS_NEW + " OR " + this.status + " IS NULL ORDER BY " + this.checkTime;
    ...
    public boolean updateODBC(String pkData, String userid, String checktime, String db_access) {
        ...
        try {
            ...
            String qry = "UPDATE PersonalLog SET STATUS=" + STATUS_PROCES + " WHERE " + this.dataIDFld + "=" + pkData;
    ...
    public Vector<Employee> getEmployees() {
        ...
        try {
            ...
            String sql;
            sql = "select FingerPrintID, FingerPrintID, EmployeeFirstName from Employee";
            stmt = getConn.createStatement();
            rs = stmt.executeQuery(sql);
            long userid = 0;
            String badgenumber = "";
            String name = "";
            while (rs.next()) {
                userid = rs.getLong("FingerPrintID");
                badgenumber = rs.getString("FingerPrintID");
                name = rs.getString("EmployeeFirstName");
