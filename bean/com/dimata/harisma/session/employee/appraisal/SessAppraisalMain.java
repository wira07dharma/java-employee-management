 ...
 * @author artha
 */
public class SessAppraisalMain {
    ...
    public static Vector searchAppraisal(SrcAppraisal srcAppraisal, int start, int recordToGet)
    {
        ...
        try {
            ...
            if(srcAppraisal.getDepartmentId() != 0){
               
                sql = sql + "DEPT."+PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT_ID]+" = "+ srcAppraisal.getDepartmentId()+" AND ";
    ...
    public static Vector searchAppraisal(SrcAppraisal srcAppraisal, int start, int recordToGet,Vector depOIDs)
    {
        DBResultSet dbrs = null;
        Vector result = new Vector(1,1);
        if (srcAppraisal == null)
        ...
        try {
            ...
            if(srcAppraisal.getDepartmentId() != 0){
                ...
                sql = sql + " AND "+ "DEPT."+PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT_ID]+" = "+ srcAppraisal.getDepartmentId()+" AND ";
                   
            }else{          
                       
               if( (depOIDs!=null) && (depOIDs.size()>0)){
                     sql=sql+" AND (";
                     for(int idx=0; idx < depOIDs.size();idx++){
                         sql=sql+"( DEPT."+PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT_ID]+" = "+
                         depOIDs.get(idx)+" ) "+ ( idx < (depOIDs.size()-1) ? (" OR "):" ");
                     }
                     sql=sql+" ) AND ";
               } else {
                 sql=sql+ " AND ";
    ...
    public static int countAppraisal(SrcAppraisal srcAppraisal, Vector depOIDs)
    {
       DBResultSet dbrs = null;
       Vector result = new Vector(1,1);
       if (srcAppraisal == null)
