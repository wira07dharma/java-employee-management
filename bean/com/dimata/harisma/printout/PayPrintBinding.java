...
import static com.dimata.harisma.printout.PayPrintText.getLeftMargin;
import static com.dimata.harisma.printout.PayPrintText.getMaxLeftgSiteLength;
import static com.dimata.harisma.printout.PayPrintText.getPageLength;
import static com.dimata.harisma.printout.PayPrintText.getPageWidth;
import static com.dimata.harisma.printout.PayPrintText.startColSlipValue;
 ...
 * @author Hendra Putu
 */
public class PayPrintBinding extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setHeader("Content-Disposition", "attachment; filename=Pay_Print_Binding.txt");
            ...
            switch (MODEL_PAYSLIP) {
                ...
                case PaySlip.MODEL_UP_DOWN:
                    try {
                        ...
                        int m = enter;
                        obj.setLine(m, 1, "No.", 5);
                        obj.setLine(m, 5, ""+increment, 5);
                        ...
                        int endRow = 0;
                        int rightRow = tempN+2+(listPaySalary != null ? listPaySalary.size() : 0)+ (listPayDeduction != null ? listPayDeduction.size() : 0) - (codeTax_count != null && codeTax_count.length() > 0 ? 2 : 0);
                        if (rightRow > n){
                            endRow = rightRow;
                        } else {
                            endRow = n+3;
                        }
                        obj.setLine(getStartRowSlipComp()+endRow, 1,"----------------------------------------------------------------------------------------------------", 90);
                        if (payslipOT == 0) {
