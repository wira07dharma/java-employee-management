/*
 * CtrlPayAdditional.java
 *
 * Created on November 23, 2007, 2:23 PM
 */
 ...
 * @author  yunny
 */
public class CtrlPayAdditional extends Control implements I_Language{
    ...
    private PstPayAdditional pstPayAdditional;
    private FrmPayAdditional frmPayAdditional;

    int language = LANGUAGE_DEFAULT;
    
      /** Creates a new instance of CtrlPayComponent */
   public CtrlPayAdditional(HttpServletRequest request){
		msgString = "";
		payAdditional = new PayAdditional();
        //locker = new Locker();
		try{
			pstPayAdditional = new PstPayAdditional(0);
            //pstLocker = new PstLocker(0);
		}catch(Exception e){;}

        frmPayAdditional = new FrmPayAdditional(request,payAdditional);
    ...
    private String getSystemMessage(int msgCode){
		switch (msgCode){
			case I_DBExceptionInfo.MULTIPLE_ID :
				this.frmPayAdditional.addError(frmPayAdditional.FRM_FIELD_SUMM_ADD_ID , resultText[language][RSLT_EST_CODE_EXIST] );
    ...
    public int getLanguage(){ return language; }
      ...
      public PayAdditional getPayAdditional() { return payAdditional; } 
      
      public FrmPayAdditional getForm() { return frmPayAdditional; }
      
      public String getMessage(){ return msgString; }
      
      public int getStart() { return start; }
      
      public int action(int cmd , long oidPayAdditional){
		msgString = "";
		...
		switch(cmd){
			...
			case Command.SAVE :
				if(oidPayAdditional != 0){
					try{
						payAdditional = PstPayAdditional.fetchExc(oidPayAdditional);
					}catch(Exception exc){
					}
				}

				frmPayAdditional.requestEntityObject(payAdditional); 

                                //System.out.println("frmEmployee.errorSize() : "+frmEmployee.errorSize());
                                
				if(frmPayAdditional.errorSize()>0) {
				...
				if(payAdditional.getOID()==0){
					try{
						long oid = pstPayAdditional.insertExc(this.payAdditional);
					...
					try {
						long oid = pstPayAdditional.updateExc(this.payAdditional);
                        /**
                        * using in aiso to set employee to be contact
                        PstContactList pstContactList = new PstContactList();
                        if(employee.getIsAssignToAccounting()){
			...
			case Command.DELETE :
                        System.out.println("oidPayComponent "+oidPayAdditional);
				if (oidPayAdditional != 0){
					try{
						long oid = PstPayAdditional.deleteExc(oidPayAdditional);
                                        	if(oid!=0){
    ...
    /** Creates a new instance of CtrlPayAdditional */
    public CtrlPayAdditional() {
    }
