/*
 * CtrlPayEmpLevel.java
 *
 * Created on April 5, 2007, 1:16 PM
 */
 ...
 * @author  yunny
 */
public class CtrlPayEmpLevel extends Control implements I_Language {
	...
	private PstPayEmpLevel pstPayEmpLevel;
	private FrmPayEmpLevel frmPayEmpLevel;
        
        int language = LANGUAGE_DEFAULT;
        
    
      /** Creates a new instance of CtrlPayComponent */
    public CtrlPayEmpLevel(HttpServletRequest request){
		msgString = "";
		payEmpLevel = new PayEmpLevel();
        //locker = new Locker();
		try{
			pstPayEmpLevel = new PstPayEmpLevel(0);
            //pstLocker = new PstLocker(0);
		}catch(Exception e){;}

        frmPayEmpLevel = new FrmPayEmpLevel(request,payEmpLevel);
     ...
     private String getSystemMessage(int msgCode){
		switch (msgCode){
			case I_DBExceptionInfo.MULTIPLE_ID :
				this.frmPayEmpLevel.addError(frmPayEmpLevel.FRM_FIELD_PAY_EMP_LEVEL_ID , resultText[language][RSLT_EST_CODE_EXIST] );
     ...
     public int getLanguage(){ return language; }
      ...
      public PayEmpLevel getPayEmpLevel() { return payEmpLevel; } 
      
      public FrmPayEmpLevel getForm() { return frmPayEmpLevel; }
      
      public String getMessage(){ return msgString; }
      
      public int getStart() { return start; }
      
      public int action(int cmd , long oidPayEmpLevel,Vector listEmp){
		msgString = "";
		...
		switch(cmd){
			...
			case Command.SAVE :
				if(oidPayEmpLevel != 0){
					try{
						payEmpLevel = PstPayEmpLevel.fetchExc(oidPayEmpLevel);
					}catch(Exception exc){
					}
				}

				frmPayEmpLevel.requestEntityObject(payEmpLevel); 

                                //System.out.println("frmEmployee.errorSize() : "+frmEmployee.errorSize());
                                
				if(frmPayEmpLevel.errorSize()>0) {
				...
				if(payEmpLevel.getOID()==0){
					...
					try{
                                            /*System.out.println("masuk ga  "+listEmp.size());
                                           
                                             for(int i=0;i<listEmp.size();i++){
                                                 System.out.println("masuk ga111111111111111111111111  "+listEmp.size());
                                                 PayEmpLevel objEmpl = (PayEmpLevel)listEmp.get(i);
                                                 System.out.println("masuk ga2222222222222222222222222  "+listEmp.size());
                                                 System.out.println("Employee id"+objEmpl.getEmployeeId());
                                                 System.out.println("Level Code"+objEmpl.getLevelCode());
                                                 System.out.println("Start Date"+objEmpl.getStartDate());
                                                 System.out.println("iterasi ke-  "+i);*/
                                                /*pay.setEmployeeId(objSetEmp.getEmployeeId());
                                                pay.setLevelCode(objSetEmp.getLevelCode());
                                                pay.setStartDate(objSetEmp.getStartDate());
                                                pay.set*/
                                             
                                                    long oid = pstPayEmpLevel.insertExc(this.payEmpLevel);
					...
					try {
						long oid = pstPayEmpLevel.updateExc(this.payEmpLevel);
                        /**
                        * using in aiso to set employee to be contact
                        PstContactList pstContactList = new PstContactList();
                        if(employee.getIsAssignToAccounting()){
			...
			case Command.DELETE :
				...
				if (oidPayEmpLevel != 0){
					try{
						long oid = PstPayEmpLevel.deleteExc(oidPayEmpLevel);
