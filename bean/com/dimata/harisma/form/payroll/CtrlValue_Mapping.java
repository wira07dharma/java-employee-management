...
public class CtrlValue_Mapping extends Control implements I_Language {
    ...
    public Value_Mapping getValue_Mapping() {
        return value_Mapping;
    }

    public FrmValue_Mapping getForm() {
        return frmValue_Mapping;
    }
    ...
    public int action(int cmd, long oidValue_Mapping) {
        ...
        switch (cmd) {
            ...
            case Command.SAVE:
                ...
                //pengecekan dengan employee mapping yang sudah ada
                
                Vector list = new Vector();
                    try {
                        if(value_Mapping.getEmployee_id() > 0){
                            String where  = pstValue_Mapping.fieldNames[pstValue_Mapping.FLD_VALUE_MAPPING_ID] + " != " + value_Mapping.getOID() +" AND "+ pstValue_Mapping.fieldNames[pstValue_Mapping.FLD_COMP_CODE] + " = \"" + value_Mapping.getCompCode() + "\" AND "+ pstValue_Mapping.fieldNames[pstValue_Mapping.FLD_EMPLOYEE_ID] + " = " + value_Mapping.getEmployee_id() + " AND ( ( \"" + Formater.formatDate(value_Mapping.getStartdate(), "yyyy-MM-dd  00:00:00") + "\" BETWEEN " + pstValue_Mapping.fieldNames[pstValue_Mapping.FLD_START_DATE] + " AND " + pstValue_Mapping.fieldNames[pstValue_Mapping.FLD_END_DATE] + " ) OR ( \"" + Formater.formatDate(value_Mapping.getEnddate(), "yyyy-MM-dd  00:00:00") + "\" BETWEEN " + pstValue_Mapping.fieldNames[pstValue_Mapping.FLD_START_DATE] + " AND " + pstValue_Mapping.fieldNames[pstValue_Mapping.FLD_END_DATE] + " ) ) ";
                            list = PstValue_Mapping.list(0, 0, where, "");
                        }
                    } catch (Exception exc) {
                    }
