...
public class DSJ_PrintObj implements Serializable {
    ...
    public int addLines(int numLines, String text) {
        if(lines==null)
            lines = new Vector(1,1);
        String str = "";
        //for(int i=0;i<intCpi;i++){
    ...
    /**
     * add lines dengan wrap text dengan max rows 
     * @param index
     * @param col
     * @param text
     * @param maxLen
     * @param maxRows 
     */
    public void setLine(int index, int col, String text, int maxLen, int maxRows) {
        String str="";
        if(text==null){
            str ="";
        }
        if(text.length()>maxLen){
            ...
            int rowPlus=0;
            try{ do{
                str = text.substring(lenSet, (lenSet+maxLen-1) < text.length() ? (lenSet+maxLen-1) :  (text.length()));
                //str = text.substring(lenSet, (lenSet+maxLen-1) < text.length() ? (lenSet+maxLen-1) :  (text.length()-1));
                setLine(index+rowPlus,col,str);
                rowPlus++;
                
                lenSet = lenSet  + str.length();
            }while(rowPlus <  maxRows && lenSet<text.length());
            } catch(Exception exc){
   ...
   // static int[] cpiChar = {80,96,263};
    static int[] cpiChar = {80,150,137};// 96, 163
