...
public class ControlCheckBox {
     ...
     * @return
     */
    public static String draw(String name, String style, String[] selectVal, Vector keys,
        ...
        String s = "";
        String cssLabel = "";
        String htmlSelect = "<div class=\"" + style + "\">";
        //String selVal="";
        Hashtable hashSelectValue = new Hashtable();
        if (selectVal != null && selectVal.length > 0) {
            for (int idx = 0; idx < selectVal.length; idx++) {
                hashSelectValue.put(selectVal[idx], true);
            }
        }
        for (int i = 0; i < keys.size(); i++) {
            ...
            boolean cekSelectValue = hashSelectValue != null && hashSelectValue.size() > 0 && hashSelectValue.get(key) != null ? (Boolean) (hashSelectValue.get(key)) : false;
            s = cekSelectValue ? "checked" : "";
            cssLabel = cekSelectValue ? "class=\"multiselect-on\"" : "";

            htmlSelect = htmlSelect + "<label " + cssLabel + "><input type=\"checkbox\" name=\"" + name + "\"value=\"" + key + "\"" + s + "/>" + val + "</label>";
            //htmlSelect = htmlSelect + "\t<option value=\"" + key + "\"" + s + ">" + val + "</option>\n";
        }
        htmlSelect = htmlSelect + "</div>\n";
        return htmlSelect;
    }

    public static String draw(String name, String style, Vector selectVal, Vector keys, Vector vals, String attTag){
        return draw( name,  style,  selectVal,  keys,  vals,  attTag);
    } 
    public static String draw(String name, String style, Vector selectVal, Vector keys,
        Vector vals, String attTag, int rowbreak) {
        if (name == null) {
            name = new String("cbbox");
        }
        String s = "";
        ...
        if (selectVal != null && selectVal.size() > 0) {
            for (int idx = 0; idx < selectVal.size(); idx++) {
                hashSelectValue.put(""+selectVal.get(idx).toString(), true);
            }
        }
        for (int i = 0; i < keys.size(); i++) {
            String key = "";
            try {
                key = (String) (keys.get(i).toString());
                String val = (String) (vals.get(i).toString());
                boolean cekSelectValue = hashSelectValue != null && hashSelectValue.size() > 0 && hashSelectValue.get(key) != null ? (Boolean) (hashSelectValue.get(key)) : false;
                s = cekSelectValue ? "checked" : "";
                cssLabel = cekSelectValue ? "class=\"multiselect-on\"" : "";

                htmlSelect = htmlSelect + "<label " + cssLabel + "><input type=\"checkbox\" name=\"" + name + "\"value=\"" + key + "\"" + s + "/>" + val + "</label>" +((rowbreak>0 && i>= rowbreak && (i%rowbreak==0))?"<br>":"");
                //htmlSelect = htmlSelect + "\t<option value=\"" + key + "\"" + s + ">" + val + "</option>\n";
            } catch (Exception exc) {
                htmlSelect = htmlSelect + "" + key + "<br>" + exc;
            }
        }
        htmlSelect = htmlSelect + "</div>\n";
        return htmlSelect;
    ...
    public String draw(Vector vctValue, Vector vctCaption, String vctChecked[]) {
        Vector checks = new Vector();
        if (vctChecked != null && vctChecked.length > 0) {
            for (int i = 0; i < vctChecked.length; i++) {
                checks.add(vctChecked[i]);
            }
        }
        return draw(vctValue, vctCaption, checks);
    }
...
//            System.out.println(str);
    ...
    public String draw(String name, Vector vctValue, Vector vctCaption, String vctChecked[]) {
        this.name = name;
        Vector checks = new Vector();
        if (vctChecked != null && vctChecked.length > 0) {
            for (int i = 0; i < vctChecked.length; i++) {
    ...
    /**
     * Keterangan: untuk membuat checkBox
     *
     * @param name
     * @param vctValue
     * @param vctCaption
