<% 
/* 
 * Page Name  		:  employeefam_list.jsp
...
public String drawList(Vector objectClass, int st){
	...
	ctrlist.addHeader("Birthday","7%");
        ...
        ctrlist.addHeader("Spouse","10%");
	ctrlist.addHeader("Covered","10%");
	ctrlist.addHeader("Birthday","7%");
	ctrlist.addHeader("Child 1","10%");
	ctrlist.addHeader("Covered","10%");
	ctrlist.addHeader("Birthday","7%");
	ctrlist.addHeader("Child 2","10%");
	ctrlist.addHeader("Covered","10%");
	ctrlist.addHeader("Birthday","7%");
	ctrlist.addHeader("Child 3","10%");
	ctrlist.addHeader("Covered","10%");
	ctrlist.addHeader("Birthday","7%");
	...
	for (int i = 0; i < objectClass.size(); i++) {
		...
		rowx.add(employee.getBirthDate()==null?"":"<nobr>" + Formater.formatDate(employee.getBirthDate(),"dd MMM yyyy") + "</nobr>");
                
                FamilyMemberList famList = employee.getFamilyMemberList();
                if(famList!=null){
                    if(famList.getSpouse()!=null){
                        rowx.add(""+famList.getSpouse().getFullName());
                        rowx.add(""+(famList.getSpouse().getGuaranteed()? "Yes" :"No"));
                        rowx.add(""+ famList.getSpouse().getBirthDate());                 
                    } else{
                        rowx.add("");
                        rowx.add("");
                        rowx.add("");                                            
                    ...
                    if(famList.getChild(0)!=null){
                        rowx.add(""+famList.getChild(0).getFullName());
                        rowx.add(""+(famList.getChild(0).getGuaranteed()? "Yes" :"No"));
                        rowx.add(""+ famList.getChild(0).getBirthDate());                                         
                    }else{
                        rowx.add("");
                        rowx.add("");
                        rowx.add("");                                            
                    ...
                    if(famList.getChild(1)!=null){
                        rowx.add(""+famList.getChild(1).getFullName());
                        rowx.add(""+(famList.getChild(1).getGuaranteed()? "Yes" :"No"));
                        rowx.add(""+ famList.getChild(1).getBirthDate());                                         
                    }else{
                        rowx.add("");
                        rowx.add("");
                        rowx.add("");                                            
                    ...
                    if(famList.getChild(2)!=null){
                        rowx.add(""+famList.getChild(2).getFullName());
                        rowx.add(""+(famList.getChild(2).getGuaranteed()? "Yes" :"No"));
                        rowx.add(""+ famList.getChild(2).getBirthDate());                                         
                    }else{
                        rowx.add("");
                        rowx.add("");
                        rowx.add("");                                            
...
if(iCommand == Command.SAVE && prevCommand==Command.ADD)
{
	listEmployee = sessEmployee.searchEmployeeAndFamily(new SrcEmployee(), start, recordToGet);
}
else
{   
    try{
	listEmployee = sessEmployee.searchEmployeeAndFamily(srcEmployee, start, recordToGet);
        }catch(Exception ex){
...
<script language="JavaScript">
	...
	function cmdPrintXLS(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.EmployeeFamilyListXLS");
	}

	function cmdAdd(){
		document.frm_employee.command.value="<%=Command.ADD%>";
		document.frm_employee.prev_command.value="<%=Command.ADD%>";
		document.frm_employee.action="../employee/databank/employee_edit.jsp";
	...
	function cmdEdit(oid){
		...
		document.frm_employee.action="../employee/databank/familymember.jsp";
		document.frm_employee.submit();
	}

	function cmdListFirst(){
		document.frm_employee.command.value="<%=Command.FIRST%>";
		document.frm_employee.action="employeefam_list.jsp";
		document.frm_employee.submit();
	}
	...
	function cmdBack(){
		...
		document.frm_employee.action="srcemployeefam.jsp";
		document.frm_employee.submit();
	}
          ...
          <td height="20">
		    <font color="#FF6600" face="Arial"><strong>
			  <!-- #BeginEditable "contenttitle" --> 
                  Employee & Family &gt; Search Result<!-- #EndEditable --> 
            </strong></font>
	      </td>
        </tr>
        <tr> 
