 ...
 */
public Vector drawList(Vector listPresenceData,Vector listPresencePersonalInOut, Date dtStartDate, Date dtEndDate, String weekTitle,Hashtable hashTblSchedule, Vector vOvertimeDetail) 
{
        int intStartDate =  dtStartDate !=null ? dtStartDate.getDate(): 0;
        int intEndDate =  dtEndDate !=null ? dtEndDate.getDate(): 0;
	Vector result = new Vector(1,1);
	if(listPresenceData!=null && listPresenceData.size()>0)
	{
		...
		for(int j=intStartDate; j<=intEndDate; j++){
			...
			ctrlist.addHeader("Sym", "3%", "0", "0");
			ctrlist.addHeader("Dur", "5%", "0", "0");						
		}
		...
		int dataAmount = 0; 			
		int maxPresenceData = listPresenceData.size(); 
                Hashtable breakTimeDuration = PstScheduleSymbol.getBreakTimeDuration(); 
                Presence presenceBreak = new Presence();
                long preBreakOut = 0;
                long preBreakIn = 0;
                 Date tmpDate = new Date(dtStartDate.getTime());
                 int totalPresence = 0;
		for(int i=0; i<maxPresenceData; i++) 
		{
			PresenceWeekly presenceWeekly = (PresenceWeekly)listPresenceData.get(i);
			String empNum = presenceWeekly.getEmpNum();
			String empName = presenceWeekly.getEmpName();
			Vector empSchedules = presenceWeekly.getEmpSchedules();
			Vector empActualIn = presenceWeekly.getEmpIn(); 
			Vector empActualOut = presenceWeekly.getEmpOut();
			Vector empSchedules2 = presenceWeekly.getEmpSchedules2();
			Vector empActualIn2 = presenceWeekly.getEmpIn2();
			Vector empActualOut2 = presenceWeekly.getEmpOut2();
			...
			//{ 
                ...
                int kk=0; 																							
                 Date dtIdx = presenceWeekly.getDayIdx(kk);
                 long empId=presenceWeekly.getEmployeId();																					
                for(int x=dtStartDate.getDate(); x <=dtEndDate.getDate() ;x++)
                {
                    
				int presenceNull = 0; // menghandle apakah presence dalam week terpilih null atau tidak
                                long nightShiftScheduleId = 0; ///menentukan schedule Id dari schedule symbol 1st
                                 long nightShiftScheduleId2 = 0; ///menentukan schedule Id dari schedule symbol 2nd
                                  int totPresence=0;
                                 //long breakDuration1st = 0;
                                  //long breakDuration2nd =0;
                                    long breakDurationPersonal = 0; 
                                   //update by satyrya 2012-10-03
                                      if(empSchedules !=null && empSchedules.size() > 0){
					nightShiftScheduleId = Long.parseLong(String.valueOf(empSchedules.get(kk)));
                                      } 
                                      if(empSchedules2 !=null && empSchedules2.size() > 0){
					   nightShiftScheduleId2 = Long.parseLong(String.valueOf(empSchedules2.get(kk)));
                                      }
                                    ScheduleSymbol scheduleSymbol=new ScheduleSymbol();
                                       
                                    try{
                                        if(nightShiftScheduleId !=0){
                                            scheduleSymbol = (ScheduleSymbol)hashTblSchedule.get(nightShiftScheduleId);
                                        }
					...
					String schldSymbol = PstScheduleSymbol.getScheduleSymbol(nightShiftScheduleId);
					String schldSymbol2 = PstScheduleSymbol.getScheduleSymbol(nightShiftScheduleId2);
                   ...
                   /* if(listPresencePersonalInOut!=null && listPresencePersonalInOut.size() > 0 ){
                     ...
                     for(int idxPer = 0;idxPer < listPresencePersonalInOut.size();idxPer++){ 
                         presenceBreak = (Presence) listPresencePersonalInOut.get(idxPer);//yang di cari harus ada leavenya 
                         //update by satrya 2012-10-1d7
                          int schPresence = 0;
                          int ipoVal=0;
                          int ipiVal=0;
                         ...
                         if(presenceBreak.getPresenceDatetime() !=null){ 
                          ...
                          if(presenceBreak.getScheduleDatetime() !=null
                                 && presenceBreak.getEmployeeId()==PresenceWeekly.getEmployeId()
                                 &&(schPresence==x)){
                        ...
                        if(ipoVal!=0 || ipiVal!=0){//jika break in atau break out ada slah satu
                          ...
                          }else{ // jika kedua duanya tidak ada maka yg diambil yg terakhir / dikurangi 1 jam hanya sekali 
                            if(breakDurationPersonal ==0 && nightShiftScheduleId!=0 && breakTimeDuration.get(""+nightShiftScheduleId) !=null){  //&& sPresenceDateTime.equals(pSelectedDate)){
                                        try{                          
                                         breakDurationPersonal = ((Long)breakTimeDuration.get(""+nightShiftScheduleId)).longValue(); //scheduleSymbol.getBreakIn().getTime()  - scheduleSymbol.getBreakOut().getTime(); 
                                        }catch(Exception ex){
            ...
            if( ovdDetail.getEmployeeId()==empId && ovdDetail.getRestTimeinHr()!=0  && dtIdx!=null && ovdDetail.getDateFrom()!=null
                ...
                idxOt=idxOt-1;
					...
					if(schldSymbol!=null && schldSymbol.length()>0)
					{
						...
						String strDuration = "";
						String strDuration2 = "";
						Date dateActualIn = (Date)empActualIn.get(kk);
						Date dateActualOut = (Date)empActualOut.get(kk);
						Date dateActualIn2 = (Date)empActualIn2.get(kk);
						Date dateActualOut2 = (Date)empActualOut2.get(kk);
						//boolean presenceIsNightShift = SessNightShift.validNightShiftPresence(nightShiftScheduleId,dateActualIn,dateActualOut);
						//update by satrya 2012-10
                                                /*if(breakTimeDuration.get(""+nightShiftScheduleId) !=null){ 
                                                breakDuration1st = ((Long)breakTimeDuration.get(""+nightShiftScheduleId)).longValue();
                                                }
                                                if(breakTimeDuration.get(""+nightShiftScheduleId2) !=null){ 
                                                breakDuration2nd = ((Long)breakTimeDuration.get(""+nightShiftScheduleId2)).longValue();
                                                }*/
						if(dateActualIn != null && dateActualOut != null ){  			
							...
							strDuration = strDurationHour + strDurationMin;*/
                                                    long iDurTimeIn = dateActualIn.getTime() / 1000; 
                                                     long iDurTimeOut = (dateActualOut.getTime()-breakDurationPersonal) / 1000;
                                                     long iDuration = 0;
							...
							strDuration2 = strDurationHour2 + strDurationMin2;
                                                   ...
                                                   if(dateActualIn2 != null && dateActualOut2 != null){
                                                            ...
                                                            iDurTimeOut2nd = (dateActualOut2.getTime()-breakDurationPersonal) / 1000;
                                                  }
							...
							presenceNull += 1;
                                                        ...
                                                        {
                                                                totalPresence = totalPresence + (totPresence += 1); 
                                                        }                                                        
						}else{
							...
							strDuration2 = "No I/O"; }
                                                ...
                                                if(schldSymbol2!= null && schldSymbol2.length()>0){
                                                    String strSymbol = schldSymbol+"/"+schldSymbol2;
                                                    String strDurations = strDuration+"/"+strDuration2;
                                                    rowxWeek.add(strSymbol);  
                                                    rowxWeek.add(strDurations);
                                                }else{
                                                    rowxWeek.add(schldSymbol);  
                                                    rowxWeek.add(strDuration);
                                                    breakDurationPersonal = 0;
                                                   // continue;
                                                }
				...
				//{
					...
					//vectDataToPdf.add(rowx);
                                        ...
                                        totalPresence = 0;
				//}    		
		}//end loop maxPresenceData
		
		if(dataAmount>0)
		{
			result.add(String.valueOf(DATA_PRINT));					
			result.add(ctrlist.drawListColor(index));
			result.add(vectDataToPdf);							
		}
		else
		{
			result.add(String.valueOf(DATA_NULL));					
			result.add("<div class=\"msginfo\">&nbsp;&nbsp;Presence data found ...</div>");				
			result.add(new Vector(1,1));										
		}	
							
	}//end listPresenceData
	else
	{
		result.add(String.valueOf(DATA_NULL));			
		result.add("<div class=\"msginfo\">&nbsp;&nbsp;Presence data found ...</div>");				
...
String strWeekTitle = "Week "+idx;//getStrWeek(idx);
...
Vector listPresenceData = new Vector(1,1);
...
{
	//listPresenceData = SessNightShift.listNightShiftDataWeekly(oidDepartment,iCalendarType,date,idx);
	// Untuk aplikasi yang membutuhkan parameter section, jika hanya membutuhkan parameter department pake yang atas 
	...
	listPresenceData = SessPresence.listPresenceDataWeekly(  oidDepartment,oidCompany,oidDivision,iCalendarType,date,idx,oidSection,empNum.trim(), fullName.trim());
       ...
       vOvertimeDetail=pstOvertimeDetail.listOvertimeDetail(oidDepartment,oidCompany,oidDivision,iCalendarType,date,idx,oidSection,empNum.trim(), fullName.trim());
}

// process on drawlist
 
Vector vectResult = drawList((listPresenceData !=null && listPresenceData.size() > 0 ? listPresenceData : new Vector()),(listPresencePersonalInOut !=null && listPresencePersonalInOut.size() > 0 ? listPresencePersonalInOut : new Vector()), dtStartDate, dtEndDate, strWeekTitle,hasDfltScheduleTbl,vOvertimeDetail); 
int dataStatus = Integer.parseInt(String.valueOf(vectResult.get(0)));
String listData = String.valueOf(vectResult.get(1));
Vector vectDataToPdf = vectResult != null && vectResult.size()>0 ? ((Vector) vectResult.get(2)) : new Vector();
//vectResult != null && vectResult.size()>0 ? ((Vector) vectResult.get(1)) : new Vector() ;
...
if(session.getValue("PRESENCE_RPT_WEEKLY")!=null){
	session.removeValue("PRESENCE_RPT_WEEKLY");
	System.out.println("\tMasuk ke remove");
}
session.putValue("PRESENCE_RPT_WEEKLY",vectPresence);
%>
...
<title>HARISMA - Weekly Presence Report</title>
<script language="JavaScript">
function cmdView(){
	document.frpresence.command.value="<%=Command.LIST%>";
	document.frpresence.action="weekly_presence.jsp";
...
function reportPdf(){
	var linkPage = "<%=printroot%>.report.attendance.WeeklyPresencePdf";
	//window.open(linkPage,"reportPage","height=600,width=800,status=no,toolbar=no,menubar=no,location=no");
         window.open(linkPage,"reportPage","status=no,toolbar=no,menubar=no,location=no");
}
...
Attendance 
                  &gt; Weekly Presence<!-- #EndEditable --> </strong></font> 
                </td>
              </tr>
              <tr>
                <td>
...
<%
										  ...
										  %>
														...
														%>
														<%=ControlCombo.draw("cmbWeek","formElemen", null, String.valueOf(weekOfMonth), weekKeys, weekValues, "") %>
                                        ...
                                        <tr>
                                          ...
                                          <td width="88%"> 
                                            <table border="0" cellspacing="0" cellpadding="0" width="212">
                                              <tr>
                                                <td width="26"><a href="javascript:cmdView()" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image10','','<%=approot%>/images/BtnSearchOn.jpg',1)" id="aSearch"><img name="Image10" border="0" src="<%=approot%>/images/BtnSearch.jpg" width="24" height="24" alt="View Weekly Presence"></a></td>
										...
										<tr>
											<td>
                                              ...
                                              <tr>
                                                ...
                                                <td width="11%"><a href="javascript:reportPdf()"><img src="../../images/BtnNew.jpg" width="24" height="24" border="0" alt="Print Weekly Presence"></a></td>
												  
                                                <td width="89%"><b><a href="javascript:reportPdf()" class="buttonlink">Print 
                                                  Weekly Presence</a></b> </td>
												</tr>
											  </table>
											 </td>
										</tr>
