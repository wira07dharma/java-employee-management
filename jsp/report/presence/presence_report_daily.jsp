...
try{
    ...
    String[] scheduleSym = FRMQueryString.requestStringValues(request, "schedule");
    String symSelected = "";
    if (scheduleSym != null){
        for(int i=0; i < scheduleSym.length;i++){
            if(!scheduleSym[i].equals("0") && !scheduleSym[i].equals(" 0 ")){
                symSelected = symSelected + scheduleSym[i] + ",";
            }
        }
    }
...
try
                {	 
                    if(selectedDateFrom!=null && selectedDateTo!=null){
                        ...
                        sessEmpSchedule.setShedule(symSelected);
	...
	//vectListAL = SessLeaveManagement.listSummaryAlStockInt(srcLeaveManagement, start, recordToGet); 
	  ...
	  listAttendanceRecordDailly = SessEmpSchedule.listEmpPresenceDaily(sessEmpSchedule.getDepartement(), sessEmpSchedule.getFromDate(), sessEmpSchedule.getToDate(),
                 sessEmpSchedule.getSection(), sessEmpSchedule.getEmpNum().trim(),  sessEmpSchedule.getEmpFullName().trim(),
                  sessEmpSchedule.getStatus1(), sessEmpSchedule==null && sessEmpSchedule.getStart() == 0 ? start : sessEmpSchedule.getStart(), recordToGet,stsPresenceSel,sessEmpSchedule.getReasonSts(),stsEmpCategorySel,statusResign,sessEmpSchedule.getOidCompany(),sessEmpSchedule.getOidDivision(),sessEmpSchedule.getShedule());
                 
                 
          }else{
              listAttendanceRecordDailly = SessEmpSchedule.listEmpPresenceDaily(oidDepartment, selectedDateFrom, selectedDateTo, oidSection, empNum, fullName, stsScheduleSel, start, recordToGet,stsPresenceSel,reason_sts,stsEmpCategorySel,statusResign,oidCompany,oidDivision,symSelected);      
          }
...
window.location.hash = storedHash;
        ...
        <%
            ...
            Vector listScheduleSymbolNormalandOff = new Vector(1, 1);
            listScheduleSymbolNormalandOff = PstScheduleSymbol.getScheduleIdbiInCategoryType("0,4");
	...
	if(!countCheckBoxes)
		...
		// set the check value for all check boxes
         ...
         function  checkSymbol(obj){
             ...
             if(symbol!=null){
                 <% if(listScheduleSymbolNormalandOff!=null){ 
                     for(int i=0;i < listScheduleSymbolNormalandOff.size(); i++ ){
                    ScheduleSymbol sch = (ScheduleSymbol)listScheduleSymbolNormalandOff.get(i);
                    %>
...
if (processDependOnUserDept) {
    ...
    for (int i = 0; i < listCostDept.size(); i++) {
        ...
        if (prevCompany.equals(dept.getCompany())) {
            ...
            <tr>
                <td width="6%" align="right" nowrap><div align=left>Schedule</div></td>
                <td width="30%" nowrap="nowrap">:
                    <%
                        Vector sch_value = new Vector(1,1);
                        Vector sch_key = new Vector(1,1);
                        Vector listSchedule = PstScheduleSymbol.list(0, 0, "", "");
                    ...
                    %>
                    <%=ControlCombo.drawStringArraySelected("schedule", "chosen-select", "", scheduleSym, sch_key, sch_value, null, "multiple data-placeholder='Select Schedule'" )%></td>
                </td>
            </tr>
            <tr>
