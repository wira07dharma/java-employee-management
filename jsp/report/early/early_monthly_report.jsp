...
<%@ page import ="java.util.*,
                  ...
                  com.dimata.harisma.session.early.EarlyMontly,
 ...
 */
public Vector cekEarlyEmployee (Date timeOut, Date timeOutSchedule, ScheduleSymbol scheduleSymbol,Date timeIn)
{
	Vector vlLate = new Vector(1,1);
	long scheduleCategoryId=PstScheduleCategory.getOID();
	try
	{
		// diproses jika "time out on schedule!=null" 
		System.out.println("timeOutSchedule..."+timeOutSchedule);
	...
	{
		System.out.println("Exc on cekEarlyEmployee : " + e.toString());
	}
	return vlLate;
}
...
public Vector drawList(Vector listEarly, Date dtSch, Date stDate, Date endDate) 
{
	...
	ctrlist.addHeader("Duration (hour, minutes)","70%", "0", ""+(max+1)+"");
	...
	{
		EarlyMontly earlyMontly = (EarlyMontly)listEarly.get(i);

        String empNum = earlyMontly.getEmpNum();
        String empName = earlyMontly.getEmpName();
        Vector empSchedules1st = earlyMontly.getEmpSchedules1st();
        Vector empSchedules2nd = earlyMontly.getEmpSchedules2nd();
        Vector empActualIn1st = earlyMontly.getEmpIn1st();
        Vector empActualIn2nd = earlyMontly.getEmpIn2nd();
        Vector empActualOut1st = earlyMontly.getEmpOut1st();
        Vector empActualOut2nd = earlyMontly.getEmpOut2nd();
        ...
        for(int x=stDate.getDate(); x<=endDate.getDate() ;x++)
		{
			...
			{ 
                ...
                Vector vect = cekEarlyEmployee (timeOut, scheduleSymbol.getTimeOut(),scheduleSymbol,timeIn); // value 1 st
                ...
                vect = cekEarlyEmployee (actualOut2nd, scheduleSymbol.getTimeOut(),scheduleSymbol,timeIn); // value 2st
                if((Integer.parseInt(""+vect.get(0))!=0) || (Integer.parseInt(""+vect.get(1))!=0))
				{
                    if(Integer.parseInt(""+vect.get(0))!=0)
					{
                    ...
                    if(Integer.parseInt(""+vect.get(1))!=0)
					{
						...
						formMinutes = "0";
						formTime = formTime+ ",  "+ formMinutes+"m"; 
					}
...
{
	listEarlyHome = SessEmployeeEarly.listEarlyDataMonthly(oidDepartment, date, oidSection);
}

Vector vEarly = drawList(listEarlyHome, date, dStartDateOfMonth, dEndDateOfMonth);
String drawList = "";
Vector vctMonthlyEarly = new Vector(1,1);
vctMonthlyEarly.add(""+oidDepartment);
vctMonthlyEarly.add(dStartDateOfMonth);
vctMonthlyEarly.add(dEndDateOfMonth);
System.out.println("dEndDateOfMonth..."+dEndDateOfMonth);
if(vEarly != null && vEarly.size()>0)
{
	drawList = (String)vEarly.get(0);
	Vector list = (Vector)vEarly.get(1);
	vctMonthlyEarly.add(list);
}
vctMonthlyEarly.add(""+oidSection);
if(session.getValue("MONTHLY_EARLY")!=null){
	session.removeValue("MONTHLY_EARLY");
}
session.putValue("MONTHLY_EARLY",vctMonthlyEarly);
%>
...
<title>HARISMA - Report Early Home Monthly</title>
<script language="JavaScript">

function reportXls(){
	window.open("<%=approot%>/servlet/com.dimata.harisma.report.MonthlyEarlyXLS");
}

function cmdView(){
	document.frpresence.command.value="<%=Command.LIST%>";
	document.frpresence.action="early_monthly_report.jsp";
	document.frpresence.submit();
}

function reportPdf(){
	var linkPage = "<%=printroot%>.report.employee.EarlyMonthlyPdf";
	window.open(linkPage,"reportMontlyPage","height=600,width=800,status=no,toolbar=no,menubar=no,location=no");
}
//-------------- script control line -------------------
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td height="20"> <font color="#FF6600" face="Arial"><strong> <!-- #BeginEditable "contenttitle" -->Attendance
                  &gt; Early Home Monthly<!-- #EndEditable --> </strong></font> </td>
              </tr>
              <tr>
                <td>
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
									...
									<input type="hidden" name="command" value="<%=iCommand%>">
									  <table width="60%" border="0" cellspacing="2" cellpadding="2">
                                        ...
                                        <tr>
                                          ...
                                          <td width="88%">: <%=ControlDate.drawDateMY("month_year",date==null || iCommand == Command.NONE?new Date():date,"MMM yyyy","formElemen",0,installInterval)%></td>
                                        </tr>
                                        <tr>
                                          <td width="3%">&nbsp;</td>
                                          ...
                                          <td> 
                                            <table width="33%" border="0" cellspacing="1" cellpadding="1">
                                              <tr> 
                                                ...
                                                <td width="37%"><b><a href="javascript:reportPdf()" class="buttonlink">Print 
                                                  Early Home</a></b></td>
                                                <td width="11%"><a href="javascript:reportXls()"><img src="../../images/BtnNew.jpg" width="24" height="24" border="0"></a></td>
                                                <td width="41%"><b>
												<%
