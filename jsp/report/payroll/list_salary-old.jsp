...
public Vector drawList(Vector objectClass,String levelCode,long oidPeriod,Vector vectDept,long sectionId,int transfered)
	{
		...
		String whereB = "slip."+PstSalaryLevelDetail.fieldNames[PstSalaryLevelDetail.FLD_TAKE_HOME_PAY]+"="+PstSalaryLevelDetail.YES_TAKE+
					   " AND "+PstSalaryLevelDetail.fieldNames[PstSalaryLevelDetail.FLD_LEVEL_CODE]+"='"+levelCode+"'"+
		...
		String whereD = "slip."+PstSalaryLevelDetail.fieldNames[PstSalaryLevelDetail.FLD_TAKE_HOME_PAY]+"="+PstSalaryLevelDetail.YES_TAKE+
					   " AND "+PstSalaryLevelDetail.fieldNames[PstSalaryLevelDetail.FLD_LEVEL_CODE]+"='"+levelCode+"'"+
					   " AND "+PstPayComponent.fieldNames[PstPayComponent.FLD_TAX_ITEM]+"!="+PstPayComponent.GAJI;			  
		String order = "PAY."+PstPayComponent.fieldNames[PstPayComponent.FLD_SORT_IDX];
...
if(summRadio==1){
// total transfered
	Vector vectTransfered= SessEmployee.getEmpSalary(vctLev,oidPeriod,0);
...
//------------------------------
double total = sumSummary + sumOvertime + sumEksAllowance;
// total kasbon
...
double sumOther = PstPaySlipComp.getOtherDeduction(oidPeriod);
//Jika ada additional
	double addOther = PstPayAdditional.getAddValue(totalOthersDeduction);
	sumOther = sumOther + addOther;
//------------------------------
double finalTotal = total - (sumKasbon+sumLeaveDeduction+sumOther);
...
function cmdView(){
	...
	document.frmreason.action="list_salary-old.jsp";
...
function reportPdf(){
	var linkPage ="<%=printroot%>.report.payroll.ListSalaryPdf";
