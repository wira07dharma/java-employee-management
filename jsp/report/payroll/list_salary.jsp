...
//------------------------------
            ...
            double nonTransfered = SessPaySlip.getSumOvtTraining(oidPeriod, totalNonTransfered);// overtime training 
            double sumSummary = 0;
            //double nonTrasferAll=0;
            
            if (summRadio == 1) {
                //update by satrya 2013-01-25
                ListSalary listSalTransfer = SessPaySlip.getSalary(false, null, oidPeriod, 0, true, totalSummaryTransfered,paidby,vEmpCategory,0);
                //  ListSalary listSalTransfer = SessPaySlip.getSalary(false, null, oidPeriod, 0, true, totalSummaryTransfered);
                ListSalary listSalNonTransfer = SessPaySlip.getSalary(true, null, oidPeriod, 0, true, totalNonTransfered,paidby,vEmpCategory,0); 
                //ListSalary listSalNonTransfer = SessPaySlip.getSalary(true, null, oidPeriod, 0, true, totalNonTransfered);
                totalTransfered = listSalTransfer.totalSalary ;// + listSalNonTransfer.totalSalary - nonTransfered;
                finalTotal = listSalTransfer.totalSalary;//  + listSalNonTransfer.totalSalary - nonTransfered;
                
           }
                      
            double total = finalTotal +  sumKasbon + sumLeaveDeduction + sumOther + sumLateDeduc + sumLateSatpamDeduc + sumSukaDukaDeduc;
            sumSummary = total - sumOvertime - sumEksAllowance - sumMealAllowance;
            ...
            /*
            long oidCategoryTrain = 0;
            String codeOvertime = "";
            try{
                oidCategoryTrain = Long.parseLong(String.valueOf(PstSystemProperty.getPropertyLongbyName("OID_TRAIINING_CATEGORY")));            
                codeOvertime = String.valueOf(PstSystemProperty.getValueByName("CODE_OVT"));
            } catch(Exception exc){
                
            }
            if(oidCategoryTrain!= 0){
                Vector vNonTrans = SessEmployee.getEmpNonTransfered(oidPeriod, ""+oidCategoryTrain);
                nonTransfered = 0;
                // jumlahkan semua nilai yang didapat
                if (vNonTrans != null && vNonTrans.size() > 0) {
                    double ovTimeVal = 0.0d;
                    for (int i = 0; i < vNonTrans.size(); i++) {
                            PaySlip paySlip = (PaySlip) vNonTrans.get(i);
                            ovTimeVal=SessPaySlip.getSumPaySlipComp(codeOvertime, paySlip.getOID());
                            nonTransfered = nonTransfered +ovTimeVal;
                    }
                }
                //Jika ada additional
                nonTransfered = nonTransfered+ PstPayAdditional.getAddValue(totalOvertime);
                }
*/
        //double nonTransferedNonTraining = nonTransfered;

        /*        
        nonTransfered=0; //non transfer here means for Intimas = Training Employee    
    ...
    <script language="JavaScript">
        ...
        window.open("<%=printroot%>.report.listRequest.ListEmpCategPdf?mls=<%="" + System.currentTimeMillis()%>");
            ...
            function cmdView(){
                ...
                document.frmreason.action="list_salary.jsp?mls=<%="" + System.currentTimeMillis()%>";
...
<td width="88%" valign="top" align="left">
    ...
    if (summRadio == 1) {
                                            ...
                                            <tr> 
                                                ...
                                                <td>&nbsp;</td>
                                                <td>Total Non Transfered (Training ) </td>
