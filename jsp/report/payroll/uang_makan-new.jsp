		...
		for (int i = 0; i <=objectClass.size(); i++) {
			...
			rowxPdf.add(""+Formater.formatDate(employee.getCommencingDate(),"dd-MMM-yyyy"));
			 ...
			 //kondisi unutk karyawan yang dibayar full--jadi dianggap dia masuk terus
                        ...
                        String formula = SessPaySlip.getFormula(employee.getOID(), oidPeriod, "TJ.MAKAN");                        
                        Vector vMealAll = SessPaySlip.calcSalaryCompPeriod(formula, employee.getOID(), oidPeriod);
                         
                        SalaryCompResult salCompRslt = (SalaryCompResult)vMealAll.get(0);
                                        rowx.add(""+salCompRslt.ScheduleNote+"/"+(salCompRslt.compValue>0.0?"+":"-")+" ");
					rowxPdf.add(""+salCompRslt.ScheduleNote+"/"+(salCompRslt.compValue>0.0?"+":"-"));
                        totalMealEmp = totalMealEmp +  (double) salCompRslt.compValue;
                        if(salCompRslt.compValue>0.0 && salCompRslt.paySlip.getDayPresent()>0 ){
                            totPresence=totPresence+1;
                        }
                        
			absence = absence + (int)salCompRslt.paySlip.getDayAbsent();
			jmlLate = jmlLate + (int)salCompRslt.paySlip.getDayLate(); 
			...
			for (int o=0;o<(maxDayfMonth-1);o++){
                                ...
                                salCompRslt = (SalaryCompResult)vMealAll.get(o+1);
			...
			/* int valueNAB = totNAB * Double.parseDouble(String.valueOf(PstSystemProperty.getValueByName("MEAL_ALLOWANCE")));  
			 ...
			 rowx.add("<div align=\"right\">"+Formater.formatNumber(totalMealEmp,frmCurrency)+"</div>");
			 rowxPdf.add(""+Formater.formatNumber(totalMealEmp,frmCurrency));
			 totMealAlloance = totMealAlloance + (totalMealEmp);
...
function deptChange() {
	...
	document.frmreason.action = "uang_makan-new.jsp";
