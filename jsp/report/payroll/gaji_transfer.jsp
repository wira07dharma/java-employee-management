...
public Vector drawList(Vector objectClass,long oidPeriod)
	{
		...
		ctrlist.addHeader("Bank Acc. Nr.","5%","0","0");
		ctrlist.addHeader("Total Transfered","5%","0","0");
			...
			//untuk footer
				...
				lstData.add(rowx);
				 ...
				 Employee employee = (Employee)temp.get(0);
                                 ...
                                 double totalBenefit   = PstPaySlipComp.getCompValueEmployeeDouble(employee.getOID(), oidPeriod, "TI")   ;//PstSalaryLevelDetail.getSumBenefitDoub(PstSalaryLevelDetail.YES_TAKE,payEmpLevel.getLevelCode(),PstPayComponent.TYPE_BENEFIT,paySlip.getOID(),PstSalaryLevelDetail.PERIODE_MONTHLY,0);
                                 double totalDeduction = PstPaySlipComp.getCompValueEmployeeDouble(employee.getOID(), oidPeriod, "TD");//PstSalaryLevelDetail.getSumBenefitDoub(PstSalaryLevelDetail.YES_TAKE,payEmpLevel.getLevelCode(),PstPayComponent.TYPE_DEDUCTION,paySlip.getOID(),PstSalaryLevelDetail.PERIODE_MONTHLY,0);
                                 ...
                                 String strTakeHomePay = String.format("%.2f",takeHomePay);
                                 takeHomePay = convertDouble(strTakeHomePay);
                                 ...
                                 String hasilFormat = Formater.formatNumber(takeHomePay, frmCurrency);
                                 hasilFormat = hasilFormat.replace(",", "");
                                 double doubleHasilFormat = Double.parseDouble(hasilFormat) ;
                                 totalTransfered = totalTransfered + (doubleHasilFormat > 0?doubleHasilFormat:takeHomePay);
                                 lstData.add(rowx);
                                 vectDataToPdf.add(rowxPdf);
...
if(iCommand == Command.LIST){
		tempQueryReport = SessEmployee.getEmpSalary(secSelect,oidPeriod,departmentId,paidby, vEmpCategory, payrollGroupId);
...
*/
   ...
   //totalTransfered = SessPaySlip.getTotalSalary(oidPeriod, totalSummaryTransfered, "");//totalNonTransfered);
    ...
    ListSalary listSalTransfer = SessPaySlip.getSalary(false, null, oidPeriod, 0, true, totalSummaryTransfered, paidby, vEmpCategory,payGroupId, payrollGroupId);
