<%-- 
    Document   : detail_sl_xls
    Created on : 19-Sep-2017, 14:45:29
    Author     : Gunadi
--%>
...
<%
    ...
    long schId = FRMQueryString.requestLong(request, "schId");
    String strDateFrom = FRMQueryString.requestString(request, "dateFrom");
    String strDateTo = FRMQueryString.requestString(request, "dateTo");
    
    DateFormat format = new SimpleDateFormat("yyyy-MM-dd");
    Date dateFrom = FRMQueryString.requestDateVer5(request, "dtFrom");
    Date dateTo = FRMQueryString.requestDateVer5(request, "dtTo");
    
    if (dateFrom == null || dateTo == null ){
        dateFrom = format.parse(strDateFrom);
        dateTo = format.parse(strDateTo); 
    }
    SpecialUnpaidLeaveTaken special = new SpecialUnpaidLeaveTaken();
    
    Vector stockV = PstSpecialUnpaidLeaveTaken.listForDetail(dateFrom, dateTo, employeeId, schId);
    
    Hashtable hashEmp = new Hashtable();
    try{
        hashEmp = PstEmployee.fetchExcHashtable(employeeId);
    }catch(Exception e){
    }
    ...
    <body>
        <div class="content-main">
            <form name="frm" method ="post" action="">
                ...
                <input type="hidden" name="dateFrom" value="<%=strDateFrom%>" />
                <input type="hidden" name="dateTo" value="<%=strDateTo%>" />
                ...
                <div style="padding: 17px; background-color: #FFF;">
                    ...
                    <table>
                        ...
                        <tr>
                            <td colspan="2">DIVISI </td><td colspan="2">: <%=hashEmp.get("DIVISION_ID")%></td>
                        </tr>
                        <tr>
                            <td colspan="2">DEPARTMENT </td><td colspan="2">: <%=hashEmp.get("DEPARTMENT_ID")%></td>
                        </tr>
                        <tr>
                            <td colspan="2">SECTION </td><td colspan="2">: <%=hashEmp.get("SECTION_ID")%></td>
                        </tr>
                        <tr>
                            <td colspan="2">POSITION </td><td colspan="2">: <%=hashEmp.get("POSITION_ID")%></td>
                        </tr>
                        <tr>
                            <td colspan="2">COMMENCING </td><td colspan="2">: <%=hashEmp.get("COMMENCING_DATE")%></td>
                        </tr>
                        <tr>
                            <td valign="top">
                                <div class="divinput">                                   
                                  <%=Formater.formatDate(dateFrom, "yyyy-MM-dd")%>  &nbsp;to&nbsp; <%=Formater.formatDate(dateTo, "yyyy-MM-dd")%> 
                                </div>                                
                            </td>
                        </tr>
                        <tr>
            ...
            <% if (stockV.size()>0){ %>
                <table class="tblStyle">
                        ...
                        <% for (int i=0; i<stockV.size(); i++){
                            SpecialUnpaidLeaveTaken slStockTaken = (SpecialUnpaidLeaveTaken) stockV.get(i);
                            String tp = "";
                            String status = "";
                            ScheduleSymbol schSymbol = new ScheduleSymbol();
                            try {
                                schSymbol = PstScheduleSymbol.fetchExc(slStockTaken.getScheduledId());
                                tp = schSymbol.getSchedule();
                            } catch (Exception exc){
                                System.out.println(exc.toString());
                            }
                            
                            LeaveApplication lvApplication = new LeaveApplication();
                            try{
                                lvApplication = PstLeaveApplication.fetchExc(slStockTaken.getLeaveApplicationId());
                                if (lvApplication.getDocStatus() == 2 || lvApplication.getDocStatus() == 3){
                                    status = "Taken";
                                } else {
                        ...
                        <tr>
                            <td><%=(i+1)%></td>
                            <td><%=Formater.formatDate(slStockTaken.getTakenDate(), "yyyy-MM-dd")%> </td>
                            <td><%=Formater.formatDate(slStockTaken.getTakenFinnishDate(), "yyyy-MM-dd")%></td>
                            <td><%=tp%></td>
                            <td><%=slStockTaken.getTakenQty()%></td>
                            <td><%=status%></td>
                        </tr>
                        <% } %>
