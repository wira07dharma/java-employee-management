...
public Vector drawList(Vector objectClass,String yearPeriod)
	{
		...
		ctrlist.addHeader("Entitled","7%","2","0");
		ctrlist.addHeader("Balance","7%","2","0");
		...
		for (int i = 0; i <= objectClass.size(); i++) {
			if(i==objectClass.size()){
				...
				rowx.add("");
				rowx.add("");
				rowx.add(""+Formater.formatNumber(totLeaveValue, frmCurrency));
				lstData.add(rowx);
			...
			String endDate = "";
				...
				float  sumLeave = PstAlStockTaken.getSumLeave(where);
				rowx.add(""+sumLeave);
		...
		double totOvValue = ((ovValue/timeWorksMonth)*2*timeWorksDay);
		rowx.add(""+Formater.formatNumber(totOvValue, frmCurrency));
		double totalLeaveValue = 0;
		// nilai untuk karyawan yang sisa cutinya tidak diganti dengan uang
		if(employee.getLeaveStatus()==PstEmployee.NO_CHANGE){
			totalLeaveValue = 0;
		}else{
			totalLeaveValue = (totOvValue*residuePure);
		}
		rowx.add(""+Formater.formatNumber(totalLeaveValue, frmCurrency));
		totLeaveValue = totLeaveValue + totalLeaveValue;
			lstData.add(rowx);
			vectDataToPdf.add(rowx);
			//lstLinkData.add("");
		}
...
String sectionName ="";
		try{
			sec = PstSection.fetchExc(oidSection);
