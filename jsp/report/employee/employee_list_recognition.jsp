...
public String drawList(Vector objectClass, int st){
	//Vector status = new Vector(1,1);
		...
		Vector status = (Vector)objectClass.get(j);
		 empStatus = (Employee)status.get(0);

	}
	//System.out.println("status     "+empStatus.getResigned());
	//System.out.println("")
	...
	if(empStatus.getResigned()==0){
		...
		ctrlist.addHeader("Resigned Date","10%");
	}
	...
	for (int i = 0; i < objectClass.size(); i++) {
		...
		if(employee.getResigned()==0){
			...
			if(employee.getResignedDate()==null){
				...
				//rowx.add(""+employee.getResignedDate());
				rowx.add(Formater.formatDate(employee.getResignedDate(),"dd-MMM-yy"));

			}
		}
		rowx.add(department.getDepartment());
		rowx.add(section.getSection());
		rowx.add(employee.getResignedDesc());
		lstData.add(rowx);
...
session.putValue(SessEmployee.SESS_SRC_EMPLOYEE_RESIGNATION, srcEmployee);
...
if(iCommand == Command.SAVE && prevCommand == Command.ADD)
{
	...
	System.out.println("vectSize 1"+vectSize);
}
else
{
	vectSize = sessEmployee.countListRecognition(srcEmployee);
...
if(iCommand == Command.SAVE && prevCommand==Command.ADD)
{
	listEmployee = sessEmployee.searchEmployeeResignation(new SrcEmployee(), start, recordToGet);
	
}
else
{
	listEmployee = sessEmployee.searchEmployeeResignation(srcEmployee, start, recordToGet);
}
%>
...
<script language="JavaScript">
	...
	function cmdPrint(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.EmployeeListResignationPdf");
	}
	...
	function cmdListFirst(){
		...
		document.frm_employee.action="employee_list_recognition.jsp";
		document.frm_employee.submit();
	}
	...
	function cmdBack(){
		...
		document.frm_employee.action="list_employee_resignation1.jsp";
		document.frm_employee.submit();
	}
          ...
          <td height="20">
		    <font color="#FF6600" face="Arial"><strong>
			  <!-- #BeginEditable "contenttitle" --> 
                  Employee &gt; List Off Employee Resignation<!-- #EndEditable --> 
            </strong></font>
	      </td>
        </tr>
        <tr> 
