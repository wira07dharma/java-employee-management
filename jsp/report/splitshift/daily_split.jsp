...
<% int  appObjCode = AppObjInfo.composeObjCode(AppObjInfo.G1_REPORTS, AppObjInfo.G2_SPLIT_SHIFT_REPORT, AppObjInfo.OBJ_SPLIT_SHIFT_DAILY_REPORT); %>
 ...
 */
public Vector drawList(Vector listSplitShiftPresence)
{
	...
	if(listSplitShiftPresence!=null && listSplitShiftPresence.size()>0)
	{
		...
		ctrlist.addHeader("Schedule Split I","6%","0","3");
		ctrlist.addHeader("Symbol","2%","0","0");
		ctrlist.addHeader("Time In","2%","0","0");
		ctrlist.addHeader("Time Out","2%","0","0");
		ctrlist.addHeader("Presence Split I","2%","0","3");
		ctrlist.addHeader("Time In","2%","0","0"); 
		ctrlist.addHeader("Duration","2%","0","0");	
		ctrlist.addHeader("Time Out","2%","0","0");
		ctrlist.addHeader("Schedule Split II","6%","0","3");
		ctrlist.addHeader("Symbol","2%","0","0");
		ctrlist.addHeader("Time In","2%","0","0");
		ctrlist.addHeader("Time Out","2%","0","0");
		ctrlist.addHeader("Presence Split II","2%","0","3");
		ctrlist.addHeader("Time In","2%","0","0");
		ctrlist.addHeader("Duration","2%","0","0");	
		ctrlist.addHeader("Time Out","2%","0","0");
		...
		int maxSplitShiftPresence = listSplitShiftPresence.size();   
		int dataAmount = 0;							
		for(int i=0; i<maxSplitShiftPresence; i++) 
		{
			SplitShiftDaily splitShiftDaily = (SplitShiftDaily)listSplitShiftPresence.get(i);
			Date dateIn1st = splitShiftDaily.getActualIn1st();
			Date dateIn2nd = splitShiftDaily.getActualIn2nd();
			Date dateOut1st = splitShiftDaily.getActualOut1st();
			Date dateOut2nd = splitShiftDaily.getActualOut2nd();			
			
			if( ! (dateIn1st==null && dateIn2nd==null && dateOut1st==null && dateOut2nd==null) )
			{									
				dataAmount++;
				
				Vector rowx = new Vector(1,1);				
				rowx.add(""+(dataAmount));
				rowx.add(splitShiftDaily.getEmpFullName().toUpperCase());  
		
				// schedule I
				rowx.add(splitShiftDaily.getScheduleSymbol1());
				rowx.add(splitShiftDaily.getScheduleIn1st()!=null ? Formater.formatTimeLocale(splitShiftDaily.getScheduleIn1st(), "HH:mm") : "");		
				rowx.add(splitShiftDaily.getScheduleOut1st()!=null ? Formater.formatTimeLocale(splitShiftDaily.getScheduleOut1st(), "HH:mm") : "");
						
				// actual I
				String formTimeI = "";
				if(dateIn1st != null && dateOut1st != null){			
					long iDuration = dateOut1st.getTime()/60000 - dateIn1st.getTime()/60000;
					...
					formTimeI = strDurationHour + strDurationMin;
				}else{
					formTimeI = "";
				}
				rowx.add(dateIn1st!=null ? Formater.formatTimeLocale(dateIn1st, "HH:mm") : "");																									
				rowx.add(formTimeI);
				rowx.add(dateOut1st!=null ? Formater.formatTimeLocale(dateOut1st, "HH:mm") : "");																											
				
				// schedule II		
				rowx.add(splitShiftDaily.getScheduleSymbol2());
				rowx.add(splitShiftDaily.getScheduleIn2nd()!=null ? Formater.formatTimeLocale(splitShiftDaily.getScheduleIn2nd(), "HH:mm") : "");		
				rowx.add(splitShiftDaily.getScheduleOut2nd()!=null ? Formater.formatTimeLocale(splitShiftDaily.getScheduleOut2nd(), "HH:mm") : "");
		
				// actual II
				String formTimeII = "";
				if(dateIn2nd != null && dateOut2nd != null){			
					long iDuration = dateOut2nd.getTime()/60000 - dateIn2nd.getTime()/60000;
					long iDurationHour = (iDuration - (iDuration % 60)) / 60;
					long iDurationMin = iDuration % 60;
					String strDurationHour = iDurationHour + "h, ";
					String strDurationMin = iDurationMin + "m ";
					formTimeII = strDurationHour + strDurationMin; 
				}else{
					formTimeII = "";
				}		
				rowx.add(dateIn2nd!=null ? Formater.formatTimeLocale(dateIn2nd, "HH:mm") : "");																											
				rowx.add(formTimeII);
				rowx.add(dateOut2nd!=null ? Formater.formatTimeLocale(dateOut2nd, "HH:mm") : "");																													
...
{
	//listSplitShiftPresence = SessSplitShift.listSplitShiftDataDaily(oidDepartment,date);
	// khusus unutk client yang parametermya butuh section,kalo hanya butuh department pake yang atas
	//update by devin 2014-01-29 listSplitShiftPresence = SessSplitShift.listSplitShiftDataDaily(oidDepartment,date,oidSection);
    listSplitShiftPresence = SessSplitShift.listSplitShiftDataDaily(oidDepartment,oidCompany,oidDivision,date,oidSection,empNum.trim(),fullName.trim());
}


// process data on control drawlist
Vector vectResult = drawList(listSplitShiftPresence);
...
if(session.getValue("SPLIT_SHIFT_DAILY")!=null){
	session.removeValue("SPLIT_SHIFT_DAILY");
}
session.putValue("SPLIT_SHIFT_DAILY",vectPresence);			
%>
<!-- End of JSP Block -->
<html>
...
function cmdView(){
	...
	document.frpresence.action="daily_split.jsp";
...
function reportPdf(){	 
	var linkPage = "<%=printroot%>.report.attendance.DailySplitShiftPdf";     
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr> 
                <td height="20"> <font color="#FF6600" face="Arial"><strong> <!-- #BeginEditable "contenttitle" -->Attendance 
                  &gt; Daily Split Shift<!-- #EndEditable --> </strong></font> 
                </td>
              </tr>
              <tr> 
                <td> 
                                            ...
                                            <!-- update by devin 2014-01-29 -->
                                                                                            <td width="6%" nowrap="nowrap"><div align="left">Payrol Num </div></td>
                                                                                             <td width="30%" nowrap="nowrap">:
                                                                                               <input class="masterTooltip" type="text" size="40" name="emp_number"  value="<%= splitShiftDaily.getEmpNum() != null ? splitShiftDaily.getEmpNum() : empNum %>" title="You can Input Payroll Number more than one, ex-sample : 1111,2222" class="elemenForm" onKeyDown="javascript:fnTrapKD()">
                                          </td>
                                                                                             <td width="5%" nowrap="nowrap"><div align="right"> Full Name </div> </td>
                                                                                             <td width="59%" nowrap="nowrap">:
                                                                                             <input class="masterTooltip" type="text" size="50" name="full_name"  value="<%= splitShiftDaily.getEmpFullName() != null ? splitShiftDaily.getEmpFullName() : fullName%>" title="You can Input Full Name more than one, ex-sample : saya,kamu" class="elemenForm" onKeyDown="javascript:fnTrapKD()">
                                                                                             </td>
                                                                                            </tr>
                                                                                            <!-- update by devin 2014-01-29  -->
                                                                                            <tr>
    ...
    for (int i = 0; i < listCostDept.size(); i++) {
        ...
        if (prevCompany.equals(dept.getCompany())) {
            ...
            prevDivision = dept.getDivision();             
              ...
              <td width="30%" nowrap="nowrap"> : <%=ControlDate.drawDate("date",date==null || iCommand == Command.NONE?new Date():date,"formElemen",0,installInterval)%></td>
                                        </tr>
                                        <tr> 
                                          <td width="3%">&nbsp;</td>
                                          <td width="9%" nowrap> 
										...
										<tr>
											<td>
											  <table width="18%" border="0" cellspacing="1" cellpadding="1">
												<tr>
												  <td width="17%"><a href="javascript:reportPdf()"><img src="../../images/BtnNew.jpg" width="24" height="24" border="0" alt="Print Daily Split Shift"></a></td>												  
                                                <td width="83%"><b><a href="javascript:reportPdf()" class="command">Print 
                                                  Daily Split Shift</a></b> </td>
												</tr>
											  </table>
											 </td>
										</tr>
