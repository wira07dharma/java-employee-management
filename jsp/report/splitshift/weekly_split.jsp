...
<% int  appObjCode = AppObjInfo.composeObjCode(AppObjInfo.G1_REPORTS, AppObjInfo.G2_SPLIT_SHIFT_REPORT, AppObjInfo.OBJ_SPLIT_SHIFT_WEEKLY_REPORT); %>
 ...
 */
public Vector drawList(Vector listSplitShiftPresence, int intStartDate, int intEndDate, String weekTitle) 
{
	Vector result = new Vector(1,1);
	//System.out.println("listSplitShiftPresence.size()   "+listSplitShiftPresence.size());
	if(listSplitShiftPresence!=null && listSplitShiftPresence.size()>0)
	{
		...
		ctrlist.addHeader("Employee","18%", "2", "0");		
		for(int j=intStartDate; j<=intEndDate; j++){
			ctrlist.addHeader(""+j,"10%", "0", "2");
			ctrlist.addHeader("1st", "5%", "0", "0");
			ctrlist.addHeader("2nd", "5%", "0", "0");						
		}
		ctrlist.addHeader("Total","4%", "2", "0");
		...
		{
			SplitShiftWeekly splitShiftWeekly = (SplitShiftWeekly)listSplitShiftPresence.get(i);
			String empNum = splitShiftWeekly.getEmpNum();
			String empName = splitShiftWeekly.getEmpName();
			Vector empSchedules1st = splitShiftWeekly.getEmpSchedules1st();
			Vector empActualIn1st = splitShiftWeekly.getEmpIn1st();
			Vector empActualOut1st = splitShiftWeekly.getEmpOut1st();			
			Vector empSchedules2nd = splitShiftWeekly.getEmpSchedules2nd();
			Vector empActualIn2nd = splitShiftWeekly.getEmpIn2nd();
			Vector empActualOut2nd = splitShiftWeekly.getEmpOut2nd();
			...
			if(containSchldSplitShift)
			{
				...
				{
					// process 1st
					String strDuration1st = "";					
					String schldSymbol = PstScheduleSymbol.getScheduleSymbol(Long.parseLong(String.valueOf(empSchedules1st.get(isch))),PstScheduleCategory.CATEGORY_SPLIT_SHIFT);
					//System.out.println("listSplitShiftPresence.size()   "+listSplitShiftPresence.size());
					if(schldSymbol!=null && schldSymbol.length()>0)
					{
						Date dateActualIn = (Date)empActualIn1st.get(isch);
						Date dateActualOut = (Date)empActualOut1st.get(isch);
						if(dateActualIn != null && dateActualOut != null){			
							...
							// ngecek durasi waktu apakah 4 jam atau lebih														
							//if(iDurationHour>=4)
							//{
								long iDurationMin = iDuration % 60;
								String strDuration1stHour = iDurationHour + "h, ";
								String strDuration1stMin = iDurationMin + "m";
								strDuration1st = strDuration1stHour + strDuration1stMin;
								presenceNull += 1; 
							//}
						}else{
							strDuration1st = "";
						}					
						rowxWeek.add(strDuration1st);												
					}
					else
					{
						rowxWeek.add("");											
					...
					// process 2nd
					String strDuration2nd = "";					
					String schldSymbol2nd = PstScheduleSymbol.getScheduleSymbol(Long.parseLong(String.valueOf(empSchedules2nd.get(isch))),PstScheduleCategory.CATEGORY_SPLIT_SHIFT);
					if(schldSymbol2nd!=null && schldSymbol2nd.length()>0)
					{
						Date dateActualIn = (Date)empActualIn2nd.get(isch);
						Date dateActualOut = (Date)empActualOut2nd.get(isch);
						if(dateActualIn != null && dateActualOut != null){			
							long iDuration = dateActualOut.getTime()/60000 - dateActualIn.getTime()/60000;
							long iDurationHour = (iDuration - (iDuration % 60)) / 60;
							...
							//{							
								long iDurationMin = iDuration % 60;
								String strDuration2ndHour = iDurationHour + "h, ";
								String strDuration2ndMin = iDurationMin + "m";
								strDuration2nd = strDuration2ndHour + strDuration2ndMin;
								presenceNull += 1;							
							//}	
						}else{
							strDuration2nd = "";
						}				
						rowxWeek.add(strDuration2nd);   						
						/*System.out.println("strDuration1st   "+strDuration1st);
						System.out.println("strDuration2nd   "+strDuration2nd);*/
						
						
						if((strDuration1st!=null && strDuration1st.length()>0) && (strDuration2nd!=null && strDuration2nd.length()>0))
						{
							totalSplitShift += 1;
						}
					}
...
{
	//listSplitShiftPresence = SessSplitShift.listSplitShiftDataWeekly(oidDepartment,iCalendarType,date,idx);
	// untuk aplikasi yang mebutuhkan parameter section,jika parameternya hanya department maka gunakan yang atas ya...
	listSplitShiftPresence = SessSplitShift.listSplitShiftDataWeekly(oidDepartment,oidCompany,oidDivision,iCalendarType,date,idx,oidSection,empNum.trim(),fullName.trim());
	
}

// process on drawlist
Vector vectResult = drawList(listSplitShiftPresence, intStartDate, intEndDate, strWeekTitle);
...
if(session.getValue("SPLIT_SHIFT_WEEKLY")!=null){
	session.removeValue("SPLIT_SHIFT_WEEKLY");
}
session.putValue("SPLIT_SHIFT_WEEKLY",vectPresence);
...
function cmdView(){
	...
	document.frpresence.action="weekly_split.jsp";
...
function reportPdf(){
	var linkPage = "<%=printroot%>.report.attendance.WeeklySplitShiftPdf";
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td height="20"> <font color="#FF6600" face="Arial"><strong> <!-- #BeginEditable "contenttitle" -->Attendance 
                  &gt; Weekly Split Shift<!-- #EndEditable --> </strong></font> 
                </td>
              </tr>
              <tr>
                <td>
                                            ...
                                            <!-- update by devin 2014-01-29 -->
                                                                                            <td width="6%" nowrap="nowrap"><div align="left">Payrol Num </div></td>
                                                                                             <td width="30%" nowrap="nowrap">:
                                                                                               <input class="masterTooltip" type="text" size="40" name="emp_number"  value="<%= splitShiftWeekly.getEmpNum() != null ? splitShiftWeekly.getEmpNum() : empNum %>" title="You can Input Payroll Number more than one, ex-sample : 1111,2222" class="elemenForm" onKeyDown="javascript:fnTrapKD()">
                                                                                             </td>
                                                                                             <td width="5%" nowrap="nowrap"><div align="right"> Full Name </div> </td>
                                                                                             <td width="59%" nowrap="nowrap">:
                                                                                             <input class="masterTooltip" type="text" size="50" name="full_name"  value="<%= splitShiftWeekly.getEmpName() != null ? splitShiftWeekly.getEmpName()  : fullName%>" title="You can Input Full Name more than one, ex-sample : saya,kamu" class="elemenForm" onKeyDown="javascript:fnTrapKD()">
                                                                                             </td>
                                                                                            </tr>
                                                                                            <!-- update by devin 2014-01-29 -->
                                                                                            <tr>
    ...
    for (int i = 0; i < listCostDept.size(); i++) {
        ...
        if (prevCompany.equals(dept.getCompany())) {
            ...
            prevDivision = dept.getDivision();             
              ...
              <td width="30%" nowrap="nowrap"> :<%//=ControlDate.drawDateMY("month_year",date==null || iCommand == Command.NONE?new Date():date,"MMM yyyy","formElemen",0,installInterval,"onChange=\"javascript:week()\"")%>
										  	<%
                                        ...
                                        <tr>
                                          ...
                                          <td width="88%"> 
                                            <table border="0" cellspacing="0" cellpadding="0" width="201">
                                              <tr>
                                                <td width="26"><a href="javascript:cmdView()" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image10','','<%=approot%>/images/BtnSearchOn.jpg',1)" id="aSearch"><img name="Image10" border="0" src="<%=approot%>/images/BtnSearch.jpg" width="24" height="24" alt="Search Split Shift"></a></td>
                                                <td width="4"><img src="<%=approot%>/images/spacer.gif" width="4" height="1"></td>
                                                <td width="171" class="command" nowrap><a href="javascript:cmdView()">View 
                                                  Weekly Split Shift</a></td>
                                              </tr>
                                            </table></td>
                                        </tr>
                                      </table>
									  ...
									  <table width="100%" border="0" cellspacing="2" cellpadding="2">
										...
										<tr>
											<td>
                                              ...
                                              <tr>												  
                                                <td width="11%"><a href="javascript:reportPdf()"><img src="../../images/BtnNew.jpg" width="24" height="24" border="0" alt="Print Weekly Split Shift"></a></td>												  
                                                <td width="89%"><b><a href="javascript:reportPdf()" class="buttonlink">Print 
                                                  Weekly Split Shift</a></b> </td>
												</tr>
											  </table>
											 </td>
										</tr>
