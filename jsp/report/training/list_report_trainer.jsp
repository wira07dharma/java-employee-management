<%-- 
    Document   : list_report_trainer
    Created on : Jan 29, 2009, 11:22:18 AM
    Author     : bayu
--%>
...
<%
    ...
    session.putValue("TRAINING_REPORT_TRN", srcTraining);
    ...
    // !
    vectSize = sessionReport.countTrainingProfileTrainer(srcTraining);
    ...
    // !
    Vector listEmpTraining  = sessionReport.getTrainingProfileTrainer(srcTraining, start, recordToGet);	
    ...
    if(vectSize == listEmpTraining.size()){
        ...
        Vector v = sessionReport.getTrainingProfileTrainer(srcTraining, 0, vectSize);
        vct.add(v);
    }
    
    vct.add(srcTraining);

    session.putValue("TRAINING_REPORT_TRN_PRINT", vct);	
%>
<!-- End of Jsp Block -->
...
<script language="JavaScript">
	...
	function cmdPrint(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.employee.TrainingByTrainerPdf?approot=<%=approot%>");
	}

	function cmdPrintXLS(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.employee.TrainingByTrainerXLS?approot=<%=approot%>");
	}
		
	function cmdListFirst(){
		document.frm_employee.command.value="<%=Command.FIRST%>";
		document.frm_employee.action="list_report_trainer.jsp";
	...
	function cmdBack(){
		...
		document.frm_employee.action="src_report_trainer.jsp";
		document.frm_employee.submit();
	}
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr> 
                <td height="20"> <font color="#FF6600" face="Arial"><strong> <!-- #BeginEditable "contenttitle" -->Training 
                  &gt; Training By Trainer &gt; Search Result<!-- #EndEditable --> 
                  </strong></font> </td>
              </tr>
              <tr> 
                <td> 
