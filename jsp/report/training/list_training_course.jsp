<%-- 
    Document   : list_training_course
    Created on : Jan 29, 2009, 3:49:52 PM
    Author     : bayu
--%>
...
<%
    ...
    session.putValue("TRAINING_COURSE_REPORT", srcTraining);

    SessTrainingReport sessionReport = new SessTrainingReport();
   	
 
    // !
    vectSize = sessionReport.countTrainingProfileCourse(srcTraining);
    ...
    // !
    Vector listEmpTraining  = sessionReport.getTrainingProfileCourse(srcTraining, start, recordToGet);	
    ...
    if(vectSize == listEmpTraining.size()){
        ...
        Vector v = sessionReport.getTrainingProfileCourse(srcTraining, 0, vectSize);
        vct.add(v);
    }
    
    vct.add(srcTraining);

    session.putValue("TRAINING_COURSE_REPORT_PRINT", vct);	
%>
<!-- End of Jsp Block -->
...
<script language="JavaScript">
	...
	function cmdPrint(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.employee.TrainingCoursePdf?approot=<%=approot%>");
	}

	function cmdPrintXLS(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.employee.TrainingCourseXLS?approot=<%=approot%>");
	}
		
	function cmdListFirst(){
		document.frm_employee.command.value="<%=Command.FIRST%>";
		document.frm_employee.action="list_training_course.jsp";
	...
	function cmdBack(){
		...
		document.frm_employee.action="src_training_course.jsp";
		document.frm_employee.submit();
	}
                                      ...
                                      <table width="100%" border="0" cellspacing="2" cellpadding="2">
                                        ...
                                        <tr> 
                                          <td colspan="3"> 
                                            ...
                                            try {
                                                Training trn = PstTraining.fetchExc(srcTraining.getTrainingId());
                                            ...
                                            %>
                                            <div align="left"><b><font size="2">Course Name : <%=name%></font></b></div>
                                          </td>
                                        </tr>
                                      </table>
