<%-- 
    Document   : list_report_emp
    Created on : Jan 28, 2009, 4:08:28 PM
    Author     : bayu
--%>
...
<%!
    public String drawList(Vector objectClass, int index){
            ...
            ctrlist.addHeader("Course Detail","20%");
            ctrlist.addHeader("Date","10%");
            ctrlist.addHeader("Duration","5%", "align='right'");
            ctrlist.addHeader("Trainer","20%");
...
<%
    ...
    session.putValue("TRAINING_REPORT_EMP", srcTraining);

    SessTrainingReport sessionReport = new SessTrainingReport();    
    Department dept = null;
    
    try {
         dept = PstDepartment.fetchExc(srcTraining.getDepartmentId());
    }
    catch(Exception e) {
        dept = new Department();
    }
    ...
    // !
    vectSize = sessionReport.countTrainingList(srcTraining);
    ...
    // !
    Vector listEmpTraining  = sessionReport.getTrainingList(srcTraining, start, recordToGet);	
    ...
    if(vectSize == listEmpTraining.size()){
        ...
        Vector v = sessionReport.getTrainingList(srcTraining, 0, vectSize);
        vct.add(v);
    }
    
    vct.add(srcTraining);

    session.putValue("TRAINING_REPORT_EMP_PRINT", vct);	
%>
<!-- End of Jsp Block -->
...
<script language="JavaScript">
	...
	function cmdPrint(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.employee.TrainingByEmpPdf?approot=<%=approot%>");
	}

	function cmdPrintXLS(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.employee.TrainingByEmpXLS?approot=<%=approot%>");
	}
		
	function cmdListFirst(){
		document.frm_employee.command.value="<%=Command.FIRST%>";
		document.frm_employee.action="list_report_emp.jsp";
	...
	function cmdBack(){
		...
		document.frm_employee.action="src_report_emp.jsp";
		document.frm_employee.submit();
	}
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr> 
                <td height="20"> <font color="#FF6600" face="Arial"><strong> <!-- #BeginEditable "contenttitle" -->Training 
                  &gt; Training By Department &gt; Search Result<!-- #EndEditable --> 
                  </strong></font> </td>
              </tr>
              <tr> 
                <td> 
                                      ...
                                      <table width="100%" border="0" cellspacing="2" cellpadding="2">
                                        ...
                                        <tr> 
                                            ...
                                            <tr>
                                                <td>Payroll</td>
                                                <td>:</td>
                                                <td><%=srcTraining.getPayroll()%></td>
                                            </tr>
                                            <tr>
                                                <td>Name</td>
                                                <td>:</td>
                                                <td><%=srcTraining.getName()%></td>
                                            </tr>
                                            <tr>
