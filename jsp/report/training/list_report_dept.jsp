...
<%-- 
    Document   : list_report_dept
    Created on : Jan 28, 2009, 2:41:19 PM
    Author     : bayu
--%>
...
<%
    ...
    if((iCommand==Command.NEXT)||(iCommand==Command.FIRST)||
       (iCommand==Command.PREV)||(iCommand==Command.LAST)||(iCommand == Command.BACK)){
         ...
         try{ 
            srcTraining = (SrcTrainingRpt)session.getValue("TRAINING_REPORT_DEPT"); 
    ...
    session.putValue("TRAINING_REPORT_DEPT", srcTraining);

    SessTrainingReport sessionReport = new SessTrainingReport();
    
    vectSize = sessionReport.countTrainingProfile(srcTraining);
    ...
    Vector listEmpTraining  = sessionReport.getTrainingProfileList(srcTraining, start, recordToGet);	
    ...
    if(vectSize == listEmpTraining.size()){
        ...
        Vector v = sessionReport.getTrainingProfileList(srcTraining, 0, vectSize);
        vct.add(v);
    }
    
    vct.add(srcTraining);

    session.putValue("TRAINING_REPORT_DEPT_PRINT", vct);	
%>
<!-- End of Jsp Block -->
...
<script language="JavaScript">
	...
	function cmdPrint(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.employee.TrainingByDeptPdf?approot=<%=approot%>");
	}

	function cmdPrintXLS(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.employee.TrainingByDeptXLS?approot=<%=approot%>");
	}
		
	function cmdListFirst(){
		document.frm_employee.command.value="<%=Command.FIRST%>";
		document.frm_employee.action="list_report_dept.jsp";
	...
	function cmdBack(){
		...
		document.frm_employee.action="src_report_dept.jsp";
		document.frm_employee.submit();
	}
                                  ...
                                  <td valign="top"> <!-- #BeginEditable "content" --> 
                                    <form name="frm_employee" method="post" action="">
                                      ...
                                      <table width="100%" border="0" cellspacing="2" cellpadding="2">
                                        ...
                                        <tr> 
                                          <td colspan="3"> 
                                            <div align="left"><b><font size="2">Department : <%=PstDepartment.fetchExc(srcTraining.getDepartmentId()).getDepartment()%></font></b></div>
                                          </td>
                                        </tr>
