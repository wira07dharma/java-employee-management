<%-- 
    Document   : list_training_profiles
    Created on : Jan 28, 2009, 10:55:51 AM
    Author     : bayu
--%>
...
<%!
    public String drawList(Vector objectClass, int start){
            ...
            for (int i = 0; i < objectClass.size(); i++) {
                ...
                try {
                    histDuration = Integer.parseInt(String.valueOf(temp.get(3)));
                }
                catch(Exception e) {}
                ...
                rowx.add(SessTraining.getDurationString(histDuration) + "&nbsp;");
...
<%
    ...
    FrmSrcTrainingTarget frmSrcTrainingTarget = new FrmSrcTrainingTarget(request, srcTrainingTarget);
    Control control = new Control();
    
    if(iCommand == Command.LIST){
        frmSrcTrainingTarget.requestEntityObject(srcTrainingTarget);			
    }
    ...
    session.putValue("TRAINING_TARGET_SEARCH", srcTrainingTarget);
    
    SessTrainingReport sessionReport = new SessTrainingReport();

    // !
    vectSize = sessionReport.countTrainingTarget(srcTrainingTarget);
    ...
    // !
    Vector listEmpTraining  = sessionReport.getTrainingTargetList(srcTrainingTarget, start, recordToGet);	
    ...
    if(vectSize == listEmpTraining.size()){
        ...
        vct.add(srcTrainingTarget);
    }
    else {
        Vector v = sessionReport.getTrainingTargetList(srcTrainingTarget, 0, vectSize);
        vct.add(v);
        vct.add(srcTrainingTarget);
    }

    session.putValue("TRAINING_TARGET_REPORT", vct);	
%>
<!-- End of Jsp Block -->
<html><!-- #BeginTemplate "/Templates/main.dwt" -->
<head>
<!-- #BeginEditable "doctitle" --> 
<title>HARISMA - Training Target Achievement</title>
<script language="JavaScript">

	function cmdPrint(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.employee.TrainingTargetPdf?approot=<%=approot%>");
	}

	function cmdPrintXLS(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.employee.TrainingTargetXLS?approot=<%=approot%>");
	}
		
	function cmdListFirst(){
		document.frm_employee.command.value="<%=Command.FIRST%>";
		document.frm_employee.action="list_training_target.jsp";
	...
	function cmdBack(){
		...
		document.frm_employee.action="src_training_target.jsp";
		document.frm_employee.submit();
	}
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr> 
                <td height="20"> <font color="#FF6600" face="Arial"><strong> <!-- #BeginEditable "contenttitle" -->Training 
                  &gt; Training Target &gt; Search Result<!-- #EndEditable --> 
                  </strong></font> </td>
              </tr>
              <tr> 
                <td> 
