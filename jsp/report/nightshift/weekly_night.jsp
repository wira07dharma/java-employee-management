...
<!-- JSP Block -->
<% int  appObjCode = AppObjInfo.composeObjCode(AppObjInfo.G1_REPORTS, AppObjInfo.G2_NIGHT_SHIFT_REPORT, AppObjInfo.OBJ_NIGHT_SHIFT_WEEKLY_REPORT); %>
 ...
 */
public Vector drawList(Vector listNightShiftPresence, int intStartDate, int intEndDate, String weekTitle) 
{
	...
	if(listNightShiftPresence!=null && listNightShiftPresence.size()>0)
	{
		...
		{
			NightShiftWeekly nightShiftWeekly = (NightShiftWeekly)listNightShiftPresence.get(i);
			...
			if(containSchldNightShift)
			{
				...
				{
					long nightShiftScheduleId = Long.parseLong(String.valueOf(empSchedules.get(isch)));
					...
					{
						String strDuration = "";
						Date dateActualIn = (Date)empActualIn.get(isch);
						Date dateActualOut = (Date)empActualOut.get(isch);
				...
				if(presenceNull>0)
				{
					...
					rowx.add(String.valueOf(totalNightShift)); 																	
					lstData.add(rowx);
...
{
	//listNightShiftPresence = SessNightShift.listNightShiftDataWeekly(oidDepartment,iCalendarType,date,idx);
	// Untuk aplikasi yang membutuhkan parameter section, jika hanya membutuhkan parameter department pake yang atas 
	//update by devin 2014-01-29
    //listNightShiftPresence = SessNightShift.listNightShiftDataWeekly(oidDepartment,iCalendarType,date,idx,oidSection);
    listNightShiftPresence = SessNightShift.listNightShiftDataWeekly(oidDepartment,oidCompany,oidDivision,iCalendarType,date,idx,oidSection,empNum.trim(),fullName.trim());
}

// process on drawlist
Vector vectResult = drawList(listNightShiftPresence, intStartDate, intEndDate, strWeekTitle);
...
if(session.getValue("NIGHT_SHIFT_WEEKLY")!=null){
	session.removeValue("NIGHT_SHIFT_WEEKLY");
	System.out.println("\tMasuk ke remove");
}
session.putValue("NIGHT_SHIFT_WEEKLY",vectPresence);
%>
...
function cmdView(){
	...
	document.frpresence.action="weekly_night.jsp";
...
function reportPdf(){
	var linkPage = "<%=printroot%>.report.attendance.WeeklyNightShiftPdf";
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td height="20"> <font color="#FF6600" face="Arial"><strong> <!-- #BeginEditable "contenttitle" -->Attendance 
                  &gt; Weekly Night Shift<!-- #EndEditable --> </strong></font> 
                </td>
              </tr>
              <tr>
                <td>
                                            ...
                                            <!-- update by devin 2014-01-29 -->
                                                                                            <td width="6%" nowrap="nowrap"><div align="left">Payrol Num </div></td>
                                                                                             <td width="30%" nowrap="nowrap">:
                                                                                               <input class="masterTooltip" type="text" size="40" name="emp_number"  value="<%= nightShiftWeekly.getEmpNum() != null ? nightShiftWeekly.getEmpNum() : empNum %>" title="You can Input Payroll Number more than one, ex-sample : 1111,2222" class="elemenForm" onKeyDown="javascript:fnTrapKD()">
                                                                                             </td>
                                                                                             <td width="5%" nowrap="nowrap"><div align="right"> Full Name </div> </td>
                                                                                             <td width="59%" nowrap="nowrap">:
                                                                                             <input class="masterTooltip" type="text" size="50" name="full_name"  value="<%= nightShiftWeekly.getEmpName() != null ? nightShiftWeekly.getEmpName() : fullName%>" title="You can Input Full Name more than one, ex-sample : saya,kamu" class="elemenForm" onKeyDown="javascript:fnTrapKD()">
                                                                                             </td>
                                                                                            </tr>
                                                                                            <!-- update by devin 2014-01-29 -->
                                                                                            <tr>
