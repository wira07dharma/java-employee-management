...
<% int  appObjCode = AppObjInfo.composeObjCode(AppObjInfo.G1_REPORTS, AppObjInfo.G2_NIGHT_SHIFT_REPORT, AppObjInfo.OBJ_NIGHT_SHIFT_DAILY_REPORT); %>
 ...
 */
public Vector drawList(Vector listNightShiftPresence) 
{
	...
	if(listNightShiftPresence!=null && listNightShiftPresence.size()>0)
	{
		...
		{
			NightShiftDaily nightShiftDaily = (NightShiftDaily)listNightShiftPresence.get(i);
			Date schldIn = nightShiftDaily.getSchldIn();
			Date schldOut = nightShiftDaily.getSchldOut();			
			Date dateIn = nightShiftDaily.getActualIn();
			Date dateOut = nightShiftDaily.getActualOut();								
			...
			{									
				...
				rowx.add(""+(i+1));
				rowx.add(nightShiftDaily.getEmpName().toUpperCase());  
		
				// schedule
				rowx.add(nightShiftDaily.getSchldSymbol());
				...
				if(dateIn != null && dateOut != null){			
					long iDuration = dateOut.getTime()/60000 - dateIn.getTime()/60000;
				...
				rowx.add(dateIn!=null ? Formater.formatTimeLocale(dateIn, "HH:mm") : "");																									
				rowx.add(formTime);
				rowx.add(dateOut!=null ? Formater.formatTimeLocale(dateOut, "HH:mm") : "");																											
				
				// Different
				String formTimeIn = "";
				if(schldIn != null && dateIn != null){
					schldIn.setSeconds(0);			
					long lTimeIn = schldIn.getTime();
					long lAcTimeIn = dateIn.getTime();																					
					long iDuration = lTimeIn/60000 - lAcTimeIn/60000;
					long iDurationHour = (iDuration - (iDuration % 60)) / 60;
					long iDurationMin = iDuration % 60;
					String strDurationHour = iDurationHour + "h, ";
					String strDurationMin = iDurationMin + "m ";				
					formTimeIn = strDurationHour + strDurationMin;
				}else{
					formTimeIn = "";
				}
				...
				String formTimeOut = "";
				if(schldOut != null && dateOut != null){
					schldOut.setSeconds(0);			
					long lTimeOut = schldOut.getTime();
					long lAcTimeOut = dateOut.getTime();																					
					long iDuration =  lAcTimeOut/60000 - lTimeOut/60000;
					long iDurationHour = (iDuration - (iDuration % 60)) / 60;
					long iDurationMin = iDuration % 60;
					String strDurationHour = iDurationHour + "h, ";
					String strDurationMin = iDurationMin + "m ";				
					formTimeOut = strDurationHour + strDurationMin;
				}else{
					formTimeOut = "";
				}
				rowx.add(formTimeOut);
		...
		else
		{
			...
			result.add("<div class=\"msginfo\">&nbsp;&nbsp;No night shift data found ...</div>");				
...
try
{
	session.removeValue("NIGHT_SHIFT_DAILY");
}
catch(Exception e)
{
	System.out.println("Exc when session.removeValue(NIGHT_SHIFT_DAILY) : " + e.toString());
}
...
Vector listNightShiftPresence = new Vector(1,1);
NightShiftDaily nightShiftDaily = new NightShiftDaily();
if(iCommand == Command.LIST)
{
	//listNightShiftPresence = SessNightShift.listNightShiftDataDaily(oidDepartment,date);
	// Untuk aplikasi yang membutuhkan parameter section, jika hanya membutuhkan parameter department pake yanga atas 
	//update by devin 2014-01-29
    //listNightShiftPresence = SessNightShift.listNightShiftDataDaily(oidDepartment,date,oidSection);
    listNightShiftPresence = SessNightShift.listNightShiftDataDaily(oidDepartment,oidCompany,oidDivision,date,oidSection,empNum.trim(),fullName.trim());

}

// process data on control drawlist
Vector vectResult = drawList(listNightShiftPresence);
...
if(session.getValue("NIGHT_SHIFT_DAILY")!=null){
	session.removeValue("NIGHT_SHIFT_DAILY");
}
session.putValue("NIGHT_SHIFT_DAILY",vectPresence);			
%>
<!-- End of JSP Block -->
<html>
...
<title>HARISMA - Night Shift Report</title>
<script language="JavaScript">
function cmdView(){
	document.frpresence.command.value="<%=Command.LIST%>";
	document.frpresence.action="daily_night.jsp";
...
function reportPdf(){	 
	var linkPage = "<%=printroot%>.report.attendance.DailyNightShiftPdf";     
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr> 
                <td height="20"> <font color="#FF6600" face="Arial"><strong> <!-- #BeginEditable "contenttitle" -->Attendance 
                  &gt; Daily Night Shift<!-- #EndEditable --> </strong></font> 
                </td>
              </tr>
              <tr> 
                <td> 
                                            ...
                                            <!-- update by devin 2014-01-29 -->
                                                                                            <td width="6%" nowrap="nowrap"><div align="left">Payrol Num </div></td>
                                                                                             <td width="30%" nowrap="nowrap">:
                                                                                               <input class="masterTooltip" type="text" size="40" name="emp_number"  value="<%= nightShiftDaily.getEmpNum() != null ? nightShiftDaily.getEmpNum() : empNum %>" title="You can Input Payroll Number more than one, ex-sample : 1111,2222" class="elemenForm" onKeyDown="javascript:fnTrapKD()">
                                          </td>
                                                                                             <td width="5%" nowrap="nowrap"><div align="right"> Full Name </div> </td>
                                                                                             <td width="59%" nowrap="nowrap">:
                                                                                             <input class="masterTooltip" type="text" size="50" name="full_name"  value="<%= nightShiftDaily.getEmpName() != null ? nightShiftDaily.getEmpName() : fullName%>" title="You can Input Full Name more than one, ex-sample : saya,kamu" class="elemenForm" onKeyDown="javascript:fnTrapKD()">
                                                                                             </td>
                                                                                            </tr>
                                                                                            <!-- update by devin 2014-01-29 -->
                                                                                            <tr>
    ...
    for (int i = 0; i < listCostDept.size(); i++) {
        ...
        if (prevCompany.equals(dept.getCompany())) {
            ...
            prevDivision = dept.getDivision();             
              ...
              <td width="30%" nowrap="nowrap"> :
                  <%=ControlDate.drawDate("date",date==null || iCommand == Command.NONE?new Date():date,"formElemen",0,installInterval)%></td>
                                        </tr>
                                        <tr> 
                                          <td width="3%">&nbsp;</td>
                                          <td width="9%" nowrap> 
										...
										<tr>
											<td>
											  <table width="18%" border="0" cellspacing="1" cellpadding="1">
												<tr>
												  <td width="17%"><a href="javascript:reportPdf()"><img src="../../images/BtnNew.jpg" width="24" height="24" border="0" alt="Print Daily Night Shift"></a></td>												  
                                                <td width="83%"><b><a href="javascript:reportPdf()" class="command">Print 
                                                  Daily Night Shift</a></b> </td>
												</tr>
											  </table>
											 </td>
										</tr>
