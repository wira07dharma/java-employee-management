...
<%
    ...
    Vector vDPOpnameDate = new Vector();
    vDPOpnameDate = com.dimata.harisma.session.leave.SessDPUpload.getAllDpUpload(0,0);
    vectSize = vDPOpnameDate.size();
    ...
    vDPOpnameDate = com.dimata.harisma.session.leave.SessDPUpload.getAllDpUpload(start,recordToGet);
    
%>

<script language="JavaScript">
    function cmdSelect(date) {
        self.opener.document.forms.<%=FrmSrcDPUpload.FRM_DP_UPLOAD%>.<%=String.valueOf(FrmSrcDPUpload.fieldNames[FrmSrcDPUpload.FRM_FIELD_OPNAME_DATE])%>.value = date;
        self.close();
    }
    
        function cmdListFirst(){
		document.<%=FrmSrcDPUpload.FRM_DP_UPLOAD%>.command.value="<%=String.valueOf(Command.FIRST)%>";
		document.<%=FrmSrcDPUpload.FRM_DP_UPLOAD%>.action="dp_opname_list.jsp";
...
<html><!-- #BeginTemplate "/Templates/main.dwt" -->
    <head>
        ...
        <title>HARISMA - DP Opname List</title>
        <!-- #EndEditable --> 
                                            ...
                                            <table width="100%" border="0" cellspacing="1" cellpadding="1" class="tabbg">
                                                <tr> 
                                                    <td valign="top">
                                                        ...
                                                        <%
                                                        if(vDPOpnameDate.size()>0){
                                                            out.println(drawList(vDPOpnameDate, start));
