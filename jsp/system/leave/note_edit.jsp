...
long oid = FRMQueryString.requestLong(request,"hidden_AL_id");
...
Employee emp = new Employee();
	...
	try{
		//update by satrya 2013-01-06
                //objAlStockManagement = PstAlStockManagement.fetchExc(oid);
            if(oid!=0){
                objAlUpload = PstAlUpload.fetchExc(oid);
            }
            if(objAlUpload.getEmployeeId()!=0){
                objEmployee = PstEmployee.fetchExc(objAlUpload.getEmployeeId());
            }
                //objEmployee = PstEmployee.fetchExc(objAlStockManagement.getEmployeeId());
                //System.out.println("emp id "+objAlStockManagement.getEmployeeId());
                //System.out.println("Nama employee "+objEmployee.getFullName());
		
		if(iCommand==Command.SAVE){
                    //update by satrya 2013-01-06
                       // objAlStockManagement.setStNote(note);
                       //oid = PstAlStockManagement.updateExc(objAlStockManagement);
                        
                        PstAlUpload.updateNote(note,objAlUpload.getOID());
...
function cmdSave(oid){
	...
	document.frmnote.action="note_edit.jsp?hidden_AL_id="+oid;
...
<input type="hidden" name="hidden_leave_stock_id" value="<%=oid%>">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	String notereal = "";
        //update by satrya 2013-01-06
       // notereal = objAlStockManagement.getStNote() == null ? "" : objAlStockManagement.getStNote();
        notereal = objAlUpload.getNote() == null ? "" : objAlUpload.getNote();
	%>
