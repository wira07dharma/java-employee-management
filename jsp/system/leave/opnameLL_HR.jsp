...
<%!
    public String drawList(Vector objEmpLLData, Date opnameDate, int start){
        ...
        ctrlist.addHeader("No","5%","1","0");       // 0
        ctrlist.addHeader("Payroll","6%","1","0");  // 1
        ctrlist.addHeader("Name","30%","1","0");    // 2
        ctrlist.addHeader("Commencing Date","14%","1","0"); // 3
        //ctrlist.addHeader("Entitle","7%","1","0");
        ctrlist.addHeader("Stock ","10%","1","0");  // 4
        /*for(int i=0;i<5;i++){
	  ctrlist.addHeader("<center>"+String.valueOf(i+1)+"</center>","6%","0","0");
	}*/
        ctrlist.addHeader("Status","7%","1","0");   // 5
        ctrlist.addHeader("Process/<br>Reprocess","7%","1","0");    // 6
        ...
        */
        ...
        for (int i = 0; i < objEmpLLData.size(); i++){
            ...
            if(!objEmpCategory.getEmpCategory().equals("TRAINEE")) {
                ...
                rowx.add(String.valueOf(i+1+start)); // 0
                ...
                rowx.add("<input type=\"hidden\" name=\""+FrmLLUpload.fieldNames[FrmLLUpload.FRM_FLD_EMPLOYEE_ID]+"\" value=\""+objEmployee.getOID()+ "\">"
                        ...
                        +objEmployee.getEmployeeNum());  // 1
                rowx.add("<input type=\"hidden\" name=\"data_emp_name\" value=\""+objEmployee.getFullName()+ "\">"+objEmployee.getFullName()); // 2

                if(objEmployee.getCommencingDate()!=null){
                    //System.out.println("MASUK ADA");
                    rowx.add("<input type=\"hidden\" name=\"data_emp_comm_date\" value=\""+objEmployee.getCommencingDate()+ "\">"+String.valueOf(objEmployee.getCommencingDate())); // 3
                    ...
                    // -----
                    Date commencing = objEmployee.getCommencingDate();
                    Calendar dateToHaveLL = new GregorianCalendar(commencing.getYear()+1900, commencing.getMonth(), commencing.getDate());
                    dateToHaveLL.add(Calendar.MONTH, leaveConfig.getIntervalLL()[I_Leave.INTERVAL_LL_5_YEAR]); 
                    ...
                    //rowx.add("<input type=\"hidden\" name=\"data_ll_entitled\" value=\"\">"+strEntitle); // unused

                    //Taken
                    /*if(iEntitled_Intv5<=0){
                        rowx.add("<input type=\"hidden\" name=\""+FrmLLUpload.fieldNames[FrmLLUpload.FRM_FLD_LL_TAKEN_YEAR1]+"\" value=\"0\"><input type=\"text\" size=\"10\" name=\"\"  class=\"elemenForm\" value=\""+(objLLUpload.getLlTakenYear1()>0?objLLUpload.getLlTakenYear1():0)+"\" disabled=\"true\">");      // 4
                        ...
                        if(new Date().getYear() >= dateToHaveLL.get(Calendar.YEAR)-1900) { 
                            rowx.add("<input type=\"text\" size=\"10\" name=\""+FrmLLUpload.fieldNames[FrmLLUpload.FRM_FLD_LL_TAKEN_YEAR1]+"\"  class=\""+(commDateAtYearInt8.getYear()==commDateAtYear.getYear()?"styleInt8":"elemenForm")+"\" value=\""+(objLLUpload.getLlTakenYear1()>0?objLLUpload.getLlTakenYear1():0)+"\" >");    // 4
                        }
                        else {
                            rowx.add("<input type=\"text\" size=\"10\" name=\""+FrmLLUpload.fieldNames[FrmLLUpload.FRM_FLD_LL_TAKEN_YEAR1]+"\"  class=\""+(commDateAtYearInt8.getYear()==commDateAtYear.getYear()?"styleInt8":"elemenForm")+"\" value=\""+(objLLUpload.getLlTakenYear1()>0?objLLUpload.getLlTakenYear1():0)+"\" disabled=\"true\">");   // 4
                        }
                    ...
                    rowx.add("<input type=\"text\" size=\"10\" name=\""+FrmLLUpload.fieldNames[FrmLLUpload.FRM_FLD_STOCK]+"\"  class=\"elemenForm\" value=\""+(objLLUpload.getStock()>0?objLLUpload.getStock():0)+"\">");      // 4

                    //==============================================/INPUT DATA
                    if(objLLUpload.getDataStatus()==PstLLUpload.FLD_DOC_STATUS_PROCESS){
                        rowx.add("<input type=\"hidden\" name=\""+FrmLLUpload.fieldNames[FrmLLUpload.FRM_FLD_DATA_STATUS]+"\" value=\""+PstLLUpload.FLD_DOC_STATUS_PROCESS+"\">"+PstLLUpload.fieldStatusNames[PstLLUpload.FLD_DOC_STATUS_PROCESS]);     // 5
                        //rowx.add(""+objAlUpload.getDataStatus());
                    }else{
                        rowx.add("<input type=\"hidden\" name=\""+FrmLLUpload.fieldNames[FrmLLUpload.FRM_FLD_DATA_STATUS]+"\" value=\""+PstLLUpload.FLD_DOC_STATUS_NOT_PROCESS+"\">");  // 5
                    }

                    if(objEmployee.getCommencingDate().getTime()<=opnameDate.getTime() && (new Date().after(dateToHaveLL.getTime()) )){
                        rowx.add("<center><input type=\"checkbox\" name=\"data_is_process"+i+"\" value=\"1\" checked=\"true\"></center>");  // 6
                    }else{
                        rowx.add("<center><input type=\"checkbox\" name=\"data_is_process"+i+"\" value=\"0\" disabled=\"true\"></center>"); // 6
                    }

                }else{
                    //System.out.println("MASUK TIDAK ADA");
                    rowx.add("<input type=\"hidden\" name=\"data_emp_comm_date\" value=\"\">");     //  3
                    //==============================================INPUT DATA
                    //rowx.add("<input type=\"hidden\" name=\"data_ll_entitled\">0");   // unused
                    rowx.add("<input type\"text\" size=\"10\" class=\"elemenForm\" value=\"0\" >");     // 4
                    ...
                    */
                    //==============================================/INPUT DATA
                    rowx.add("<input type=\"hidden\" name=\""+FrmLLUpload.fieldNames[FrmLLUpload.FRM_FLD_DATA_STATUS]+"\" value=\""+PstLLUpload.FLD_DOC_STATUS_NOT_PROCESS+"\">");          // 5
                    rowx.add("<input type=\"hidden\" name=\"data_is_process"+i+"\" value=\"0\"><center><input type=\"checkbox\" name=\"\" value=\"false\" disabled=\"true\"></center>");    // 6
                }
                lstData.add(rowx);    
            }                       
        }
...
<%
	...
	// Inisialisasi variable yang meng-handle nilai2 berikut
        ...
        try {
            ...
            if(emp_id != null) {
                taken_year_1 = new String[emp_id.length];
                taken_year_2 = new String[emp_id.length];
                taken_year_3 = new String[emp_id.length];
                taken_year_4 = new String[emp_id.length];
                taken_year_5 = new String[emp_id.length];
            ...
            stock = request.getParameterValues(FrmLLUpload.fieldNames[FrmLLUpload.FRM_FLD_STOCK]);
        ...
        if(iCommand == Command.SAVE){//jika disimpan saja
            ...
            Vector vLLUploadStock = new Vector(1,1);
            
            vLLUploadId = com.dimata.harisma.session.leave.SessLLUpload.saveLLUploadHR(vLLUpload);
            ...
            vLLUploadId = com.dimata.harisma.session.leave.SessLLUpload.saveLLUploadHR(vLLUpload);
            status = com.dimata.harisma.session.leave.SessLLUpload.opnameLLAllDataHR(vLLUploadId);
        }
    }
...
function cmdUpdateDep(){
	...
	document.<%=FrmSrcLLUpload.FRM_LL_UPLOAD%>.action="opnameLL_HR.jsp"; 
