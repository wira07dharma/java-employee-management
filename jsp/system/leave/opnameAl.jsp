...
<%!
    public String drawList(Vector objEmpALData, Date opnameDate,int start){
        ...
        /*ctrlist.addHeader("AL "+(opnameDate.getYear()-1+1900),"9%");
        ctrlist.addHeader("Balance "+(opnameDate.getYear()-1+1900),"9%");
        ctrlist.addHeader("AL "+(opnameDate.getYear()+1900),"9%");
        ctrlist.addHeader("Taken "+(opnameDate.getYear()+1900),"9%");
        * */
        ctrlist.addHeader("System Prev","9%");
        ctrlist.addHeader("Previous","9%");
        ctrlist.addHeader("New AL ","9%");
        ctrlist.addHeader("Taken ","9%");
	...
	*/
        ...
        for (int i = 0; i < objEmpALData.size(); i++){
            ...
            //System.out.println(objAlUpload.getOID());
            ...
            if(objEmployee.getCommencingDate()!=null){
                ...
                //searchData = com.dimata.harisma.session.leave.SessAlUpload.searchData(objAlUpload.getOpnameDate(),objAlUpload.getEmployeeId()); 
                //System.out.println("OID "+objAlUpload.getOID());
                if(objAlUpload.getOID()==0){
                    rowx.add("<input type=\"text\" name=\"data_emp_earned_curr_per"+i+"\" value=\""+strAl_curr+"\">");                    
                    //System.out.println("emp id = 0 new AL "+objAlUpload.getNewAl()+" / "+strAl_curr);
                }else{ 
                    rowx.add("<input type=\"text\" name=\"data_emp_earned_curr_per"+i+"\" value=\""+objAlUpload.getNewAl()+"\">");                                                     
                    //System.out.println("emp id != 0 Al current "+strAl_curr+" / "+objAlUpload.getNewAl());
                }
...
<%
    ...
    TglSekarang = str_dt_StartDate+"-"+dateNow.getMonth()+"-"+dateNow.getDate();
...
function cmdUpdateDep(){
	...
	document.<%=FrmSrcAlUpload.FRM_AL_UPLOAD%>.action="opnameAl.jsp"; 
