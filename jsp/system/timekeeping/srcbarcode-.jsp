...
<%!
    public String drawList(Vector objectClass){
        ...
        for (int i = 0; i < objectClass.size(); i++) 
		{
			...
			{
                rowx.add("<input type=\"textbox\" size=\"10\" class=\"elementForm\" name=\"barcode_number\" value=\"" + barcode + "\">");
            }
            else 
			{
...
<%
    ...
    if (iCommand == Command.SAVE) {
        ...
        try {
			...
			//System.out.println("barcode:::"+barcode_number);
        ...
        if ((barcode_number != null) && (barcode_number.length > 0)) 
		{
            for (int i = 0; i < barcode_number.length; i++) 
			{
				...
				//if(!s_barcode_number.equals(s_old_barcode_number)){					
					...
					out.println("s_barcode_numbe  atas "+s_barcode_number);
                    PstEmployee.updateBarcode(Long.parseLong(employee_id[i]), s_barcode_number);
					System.out.println("s_barcode_numbe   "+s_barcode_number);
