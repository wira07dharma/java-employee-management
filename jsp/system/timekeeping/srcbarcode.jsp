				...
				//if(!s_barcode_number.equals(s_old_barcode_number)){					
                    ...
                    if ((s_barcode_number != null) && (s_barcode_number.length() > 0)) {
                        if((s_emp_pin!=null) && (s_emp_pin.length()>0))
                        {
                            ...
                            if(machineNumbers.length>0 && !(machineNumbers[0].equals("")) && machineNumbers[0].length()>0){			
                                svc.executeCommand(svc.UPLOAD, machineNumbers[0], s_barcode_number + "\\"+s_emp_pin+"\\T"+machineNumbers[0]);
                            }
                            if(machineNumbers.length>1 && !(machineNumbers[1].equals("")) && machineNumbers[1].length()>0){
                                svc.executeCommand(svc.UPLOAD, machineNumbers[1], s_barcode_number + "\\"+s_emp_pin+"\\T"+machineNumbers[1]);	
                            }				
                        }
                        else
                        {
                                //System.out.println("Masuk ke insert barcode yg baru : "+s_barcode_number);
                            if(machineNumbers.length>0 && !(machineNumbers[0].equals("")) && machineNumbers[0].length()>0){			
                                svc.executeCommand(svc.UPLOAD, machineNumbers[0], s_barcode_number + "\\000000\\T"+machineNumbers[0]);
                            }
                            if(machineNumbers.length>1 && !(machineNumbers[1].equals("")) && machineNumbers[1].length()>0){
                                svc.executeCommand(svc.UPLOAD, machineNumbers[1], s_barcode_number + "\\000000\\T"+machineNumbers[1]);	
                            }												
                        }
                    }
				//}													
