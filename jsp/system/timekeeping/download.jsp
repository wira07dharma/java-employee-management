...
if (privStart) 
{
    ...
    while(strTokenizer.hasMoreTokens()){
        ...
        System.out.println("CABSEN MACHINE :::::::::: "+machineNumbers[count]);
        count ++;
    }
    try 
    {
            AccessTMA accessTMA = new AccessTMA();
            if(machineNumbers.length>0 && !(machineNumbers[0].equals("")) && machineNumbers[0].length()>0){
                v1 = accessTMA.executeCommand(accessTMA.DOWNLOAD, machineNumbers[0], "");
                if (String.valueOf(v1.get(0)).equals("OFF") == false) 
                {
                        succeed01 = true;
                }
                tma01Status = String.valueOf(v1.get(0)) + " transaction(s) downloaded from TMA-"+machineNumbers[0];
            }
    }
    catch (Exception e) 
    {
    ...
    try 
    {
            ...
            if(machineNumbers.length>1 && !(machineNumbers[1].equals("")) && machineNumbers[1].length()>0){
                v2 = accessTMA.executeCommand(accessTMA.DOWNLOAD, machineNumbers[1], "");
                if (String.valueOf(v2.get(0)).equals("OFF") == false) 
                {
                        succeed02 = true;
                }
                tma02Status = String.valueOf(v2.get(0)) + " transaction(s) downloaded from TMA-"+machineNumbers[1];
            }
    }
    catch (Exception e) 
    {
