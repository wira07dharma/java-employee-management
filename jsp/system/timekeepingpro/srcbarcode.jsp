...
<%!
    public String drawList(Vector objectClass){
        ...
        ctrlist.addHeader("Full Name","30%");
        ctrlist.addHeader("Position","30%");
        ctrlist.addHeader("Finger No","10%");
	...
	//int iAddFingerId = Integer.parseInt(String.valueOf(PstSystemProperty.getValueByName("FINGER_PRINT_NUMBER")));
		...
		System.out.println("exc when fetch barcode : " + e.toString());
            ...
            rowx.add(""+empFingerId);
            
            if ((barcode != null) && (barcode.length() > 0)){
                rowx.add("<input type=\"hidden\" name=\"employee_id\" value=\"" + employee.getOID() + "\">"+
			"<input type=\"hidden\" class=\"elementForm\" name=\"old_barcode_number_"+i+"\" value=\"" + barcode + "\">"+
                        "<input type=\"hidden\" class=\"elementForm\" name=\"finger_print_number_"+i+"\" value=\""+empFingerId+"\">"+
                        "<input type=\"text\" onclick=\"javascript:searchFinger("+i+")\" readonly size=\"10\" class=\"elementForm\" name=\"barcode_number_"+i+"\" value=\"" + barcode + "\" >");
	/*			<input type=\"hidden\" name=\"employee_finger_id\" value=\"" + empFingerId + "\">
			...
			{
                rowx.add("<input type=\"hidden\" name=\"employee_id\" value=\"" + employee.getOID() + "\">"+
			"<input type=\"hidden\" size=\"10\" class=\"elementForm\" name=\"old_barcode_number_"+i+"\" value=\"\">"+
			"<input type=\"hidden\" class=\"elementForm\" name=\"finger_print_number_"+i+"\" value=\"\">"+
                        "<input type=\"text\" onclick=\"javascript:searchFinger("+i+")\" readonly size=\"10\" class=\"elementForm\" name=\"barcode_number_"+i+"\" value=\"\">");
	/*			<input type=\"hidden\" name=\"employee_finger_id\" value=\"" + empFingerId + "\">
				<input type=\"hidden\" name=\"employee_nick_name\" value=\"" + empNickName + "\">"); */
            }
...
<%
    ...
    String machineNumberC = "02";
    String[] machineNumbers;
    String[] machineNumbersC;
  ...
  //  SessEmployee.setAllEmpFingerPrintNumber();
	...
	machineNumberC = String.valueOf(PstSystemProperty.getValueByName("CANTEEN_TMA_NO"));
	StringTokenizer strTokenizer = new StringTokenizer(machineNumber,",");
	StringTokenizer strTokenizerC = new StringTokenizer(machineNumberC,",");
	machineNumbers = new String[strTokenizer.countTokens()];
	machineNumbersC = new String[strTokenizerC.countTokens()];
	...
	int countC = 0;
        ...
        while(strTokenizerC.hasMoreTokens()){
            machineNumbersC[countC] = strTokenizerC.nextToken();
            System.out.println("CANTEEN MACHINE :::::::::: "+machineNumbersC[countC]);
            countC ++;
        }
	...
	// Jika array barcode_number tidak kosong (ada/minimal satu barcode employee diisi)	
        ...
        if ((employee_id != null) && (employee_id.length > 0)){
            for (int i = 0; i < employee_id.length; i++){
                ...
                System.out.println("Processing employee ke-" + i);
                
                String oldEmpNum = FRMQueryString.requestString(request, "old_barcode_number_"+i);
                String newEmpNum = FRMQueryString.requestString(request, "barcode_number_"+i);
                ...
                //Jika Emp Num sebelumnya tidak sama dengan yang sekarang
                // dan tidak null
                //if(( true || !newEmpNum.equals(oldEmpNum) ) && !newEmpNum.equals("") && newEmpNum.length()>0){
                if((!newEmpNum.equals(oldEmpNum)) && !newEmpNum.equals("") && newEmpNum.length()>0){
                       
                        String strMax = FRMQueryString.requestString(request, "finger_print_number_"+i);
                        ...
                        employeeUp.setBarcode(newEmpNum);
                        //Masukkan Data Ke Mesin Absensi
                        for(int icount=0;icount<machineNumbers.length;icount++){
                            String strOn = FRMQueryString.requestString(request, "MACHINE"+icount);
                            if(strOn.equals("ON")){
                                System.out.println("MACHINE NO "+icount+" IS ON ");
                                ...
                                System.out.println("\t ================================ "+employeeUp.getName()+" :::: "+employeeUp.getBarcode()+" ::: "+strMax);
                                if(!status && (icount == 0)){
                                        String strMsg = strName+" ["+employee.getBarcodeNumber()+"]";
                                    vMsg.add(strMsg);
                                }
                            }else{
                                System.out.println("MACHINE NO "+icount+" IS OFF ");
                            }
                        }
                        //Masukkan data ke kanten
                        for(int icount=0;icount<machineNumbersC.length;icount++){
                            String strOn = FRMQueryString.requestString(request, "MACHINE_C"+icount);
                            if(strOn.equals("ON")){
                                System.out.println("MACHINE CANTEEN NO "+icount+" IS ON ");
                                //iUseFingerPrint untuk menghitung banyak sidik jari yang dipergunakan
                                I_Machine i_Machine = MachineBroker.getMachineByNumber(machineNumbersC[icount]);
                                boolean status = i_Machine.processUpload(employeeUp);
                                System.out.println("\t ================================ "+employeeUp.getName()+" :::: "+employeeUp.getBarcode()+" ::: "+strMax);
                                if(!status && (icount == 0)){
                                        String strMsg =strName+" ["+employee.getBarcodeNumber()+"] (Canteen)";
                                    vMsg.add(strMsg);
                                }
                            }else{
                                System.out.println("MACHINE CANTEEN NO "+icount+" IS OFF ");
                            }
                        }
                        PstEmployee.updateBarcodeAndPin(Long.parseLong(employee_id[i]), newEmpNum,strMax);
                        strMax = "";
                }            
            }
        }
...
function searchFinger(i){
    //alert("INDEX "+i);
    window.open("<%=approot%>/masterdata/srcFingerPrint.jsp?formName=frmbarcode&indexFinger="+i, null, "height=400,width=400, status=yes,toolbar=no,menubar=no,location=no, scrollbars=yes");
}
                                ...
                                <% if (privStart) { %>
                                    ...
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                      <tr>
                                            ...
                                            %>                                                            
                                                    <input type="checkbox" name="MACHINE<%=String.valueOf(i) %>" value="ON" checked="checked" /> (M)Absen-<%=String.valueOf(i)%>&nbsp;&nbsp;						

                                            <%}
                                            %>
                                            <% 
                                                    for(int i=0;i<machineNumbersC.length;i++){
                                            %>                                                            
                                                    <input type="checkbox" name="MACHINE_C<%=String.valueOf(i) %>" value="ON" checked="checked" /> (C)Canteen-<%=String.valueOf(i)%>&nbsp;&nbsp;						
