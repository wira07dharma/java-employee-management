...
<%
            /*
             * Page Name  		:  outsource_evaluation_report.jsp
...
<%!
...

	{
        ...
        ctrlist.addHeader("Posisi", "10%", "2", "0");
        
        if (Posisi.size() > 0){
            for (int pos = 0; pos < Posisi.size(); pos++){
                Position position = (Position) Posisi.get(pos);
                    ctrlist.addHeader(""+position.getPosition(), "15%", "0", "3" );
                    ctrlist.addHeader("Penyedia", "5%", "0", "0");
                    ctrlist.addHeader("nilai", "5%", "0", "0");
                    ctrlist.addHeader("jumlah", "5%", "0", "0");
                
            }
        }
        
        ctrlist.addHeader("Rata", "5%", "2", "0");
        ctrlist.addHeader("Total", "5%", "2", "0");
        ...
        try {
               ctrlist.drawListHeaderWithJsVer2(outJsp);//header
                    ...
                    for (int idxcom = 0; idxcom < Divisi.size(); idxcom++) {
                        Division division = (Division) Divisi.get(idxcom);
                        
                        //mencari vector provider
                        String wherediv = " ose."+ PstOutSourceEvaluation.fieldNames[PstOutSourceEvaluation.FLD_DIVISION_ID] + " = " + division.getOID();
                        Vector Provider = PstContactList.listJoinOutSourceEval(0, 0, wherediv,"",srcObject);
                        for (int pro = 0 ; pro < Provider.size(); pro++){
                            ContactList providerObj = (ContactList) Provider.get(pro);
                        //panggil nama provider
                        ...
                        String where = " osep."+PstOutSourceEvaluationProvider.fieldNames[PstOutSourceEvaluationProvider.FLD_PROVIDER_ID] + " = " + providerObj.getOID();
                        Hashtable listOutSourceEvalProviderPerDiv = PstOutSourceEvaluationProvider.listOutSourceEvalProviderPerDiv(0, 0, "", where,division.getOID(),srcObject);
                        double nilaitotal = 0;
                        double rataRata= 0; 
                        double pembagi = 0 ;
                        if (Posisi.size() > 0){
                            for (int pos = 0; pos < Posisi.size(); pos++){
                                    ...
                                    OutSourceEvaluationProvider outSourceEvaluationProvider = new OutSourceEvaluationProvider(); 
                                    try {
                                        outSourceEvaluationProvider = (OutSourceEvaluationProvider) listOutSourceEvalProviderPerDiv.get(""+position.getOID());
                                    } catch (Exception e){ }
                                    
                                    rowx.add(""+providerObj.getCompName());   
                                    rowx.add(""+hashstatus.get(""+(outSourceEvaluationProvider != null ?outSourceEvaluationProvider.getEvalPoint() : 0)) );
                                    rataRata = rataRata + (outSourceEvaluationProvider != null ?outSourceEvaluationProvider.getEvalPoint() : 0) ;
                                    if ((outSourceEvaluationProvider != null ?outSourceEvaluationProvider.getEvalPoint() : 0) > 0){
                                        pembagi = pembagi + 1;
                                    }
                                    rowx.add(""+(outSourceEvaluationProvider != null ?outSourceEvaluationProvider.getNumberOfPerson() : 0) );
                                    nilaitotal = nilaitotal + (outSourceEvaluationProvider != null ?outSourceEvaluationProvider.getNumberOfPerson() : 0) ;
                            }
                        }
                        ...
                        ctrlist.drawListRowJsVer2(outJsp, 0, rowx, idxcom);
                            ...
                            Vector rowxJum = new Vector();
                            rowxJum.add("-");
                            rowxJum.add("Total");
                            for(int p = 0; p < Posisi.size(); p++){
                                rowxJum.add("-");
                                rowxJum.add("-");
                                rowxJum.add("-");
                            }
                            ...
                            ctrlist.drawListRowJsVer2(outJsp, 0, rowxJum, Divisi.size()+1);
...
<%!
	...
	public String drawList2(JspWriter outJsp,int iCommand,FrmOutSourceEvaluation frmObject, Vector Divisi, Vector Posisi, SrcObject srcObject)

	{
        ...
        if (Posisi.size() > 0){
            for (int pos = 0; pos < Posisi.size(); pos++){
                Position position = (Position) Posisi.get(pos);
                    ctrlist.addHeader(""+position.getPosition(), "25%", "0", "3" );
                    ctrlist.addHeader("Plan", "5%", "0", "0");
                    ctrlist.addHeader("Realisasi", "5%", "0", "0");
                    ctrlist.addHeader("Selisih", "5%", "0", "0");
                
            }
        }
        
        ctrlist.addHeader("Plan Total", "5%", "2", "0");
        ctrlist.addHeader("Realisasi Total", "5%", "2", "0");
        ...
        try {
               ctrlist.drawListHeaderWithJsVer2(outJsp);//header
                    ...
                    for (int idxcom = 0; idxcom < Divisi.size(); idxcom++) {
                        ...
                        String where = ""+PstOutSourcePlanLocation.fieldNames[PstOutSourcePlanLocation.FLD_DIVISIONID] + " = " + division.getOID();
                        Hashtable listPlan = PstOutSourcePlanLocation.hashList(0, 0, where, ""); 
                        double plantotal = 0;
                        double realisasitotal = 0 ;
                          if (Posisi.size() > 0){
                            for (int pos = 0; pos < Posisi.size(); pos++){
                                    ...
                                    try {
                                        outSourcePlanLocation = (OutSourcePlanLocation) listPlan.get(position.getOID());
                                    } catch (Exception e){ }
                                    
                                    Vector srcObjectPeriode = srcObject.getPayPeriodIdV();
                                    PayPeriod payPeriod = new PayPeriod();
                                    double plan = 0;
                                    if (srcObjectPeriode.size() > 0){
                                        long periodeId = (Long) srcObjectPeriode.get(srcObjectPeriode.size()-1); 
                                        payPeriod = PstPayPeriod.fetchExc(periodeId);
                                        Date dateN = payPeriod.getEndDate();
                                        int month = dateN.getMonth();
                                        if (month >= 0 && month<=2 ){
                                            plan = outSourcePlanLocation.getNumberTW1();
                                        } else if (month >= 3 && month<=5 ){
                                            plan = outSourcePlanLocation.getNumberTW2();
                                             if (plan ==0) {
                                                if (outSourcePlanLocation.getNumberTW1() > 0){
                                                        plan = outSourcePlanLocation.getNumberTW1();
                                                    }
                                             }

                                        } else if (month >= 6 && month<=8 ){
                                            plan = outSourcePlanLocation.getNumberTW3();
                                            if (plan ==0) {
                                                if (outSourcePlanLocation.getNumberTW2() > 0){
                                                        plan = outSourcePlanLocation.getNumberTW2();
                                                } else if (outSourcePlanLocation.getNumberTW1() > 0){
                                                        plan = outSourcePlanLocation.getNumberTW1();
                                                } 
                                             }
                                        } else if (month >= 9  ){
                                            plan = outSourcePlanLocation.getNumberTW4();
                                                if (outSourcePlanLocation.getNumberTW3() > 0){
                                                            plan = outSourcePlanLocation.getNumberTW3();
                                                } else if (outSourcePlanLocation.getNumberTW2() > 0){
                                                            plan = outSourcePlanLocation.getNumberTW2();
                                                } else if (outSourcePlanLocation.getNumberTW1() > 0){
                                                            plan = outSourcePlanLocation.getNumberTW1();
                                                } 
                                    ...
                                    double realisasi = PstOutsrcCostProv.listOutSourcetotalEmployeeProviderPerDiv(0, 0, "", "",division.getOID(),srcObject,position.getOID());
                                    rowx.add(""+realisasi );
                                    rowx.add(""+Math.abs(plan-realisasi));
                                    realisasitotal= realisasitotal+realisasi;
                            }
                        }
                        
                        rowx.add(""+plantotal);   
                        rowx.add(""+realisasitotal);   
...
<%!
	...
	public String drawList3(JspWriter outJsp,int iCommand,FrmOutSourceEvaluation frmObject, Vector Divisi, Vector Posisi, SrcObject srcObject)
                        ...
                        double realisasitotal = 0 ;
                          if (Posisi.size() > 0){
                            for (int pos = 0; pos < Posisi.size(); pos++){
                                    ...
                                    double cost = PstOutSourcePlanCost.getcostOutSourcePlanCost(0, 0, "", "", division.getOID(), srcObject, position.getOID());
                                                                               //nilai posisi  = pstpos
                                    
                                    rowx.add(""+(plan*cost));
                                    plantotal = plantotal+(plan*cost);
                                    
                                    double realisasiHuman = PstOutsrcCostProv.listOutSourcetotalEmployeeProviderPerDiv(0, 0, "", "",division.getOID(),srcObject,position.getOID());
                                    double realisasiCost = PstOutsrcCostProvDetail.getcostvalue(0, 0, "", "",division.getOID(),srcObject,position.getOID());
                                   // double realisasiBiayaperorang = PstOutsrcCostProv.listOutSourceCostProviderPerDiv(0, 0, "", "",division.getOID(),srcObject,position.getOID());
                                    rowx.add(""+(realisasiHuman*realisasiCost));
                                    
                                    rowx.add(""+Math.abs((plan*cost)-(realisasiHuman*realisasiCost)));
                                    
                                    realisasitotal= realisasitotal+(realisasiHuman*realisasiCost);
                            }
                        }
    ...
    <head> 
        ...
        <script language="JavaScript">
            ...
            function cmdAdd(){
                ...
                document.frmoutSourceEvaluation.action="outsource_evaluation_report.jsp";
            ...
            function cmdSearch(nilai){
                ...
                document.frmoutSourceEvaluation.prev_command.value="<%=prevCommand%>";
                document.frmoutSourceEvaluation.action="outsource_evaluation_report.jsp?type="+nilai;
                document.frmoutSourceEvaluation.submit();
            }
                                                                                            ...
                                                                                            <td height="8"  colspan="3">
                                                                                             <table width="30%" border="0" cellspacing="0" cellpadding="0">
                                                                                                      ...
                                                                                                      <tr>
                                                                                                            ...
                                                                                                            <td width="39%"><a href="javascript:cmdSearch(2)" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image10','','<%=approot%>/images/BtnSearchOn.jpg',1)" id="aSearch"><img name="Image10" border="0" src="<%=approot%>/images/BtnSearch.jpg" width="24" height="24" alt="Search "></a>
                                                                                                            <img src="<%=approot%>/images/spacer.gif" width="6" height="1">
                                                                                                            <a href="javascript:cmdSearch(2)">Search Realiasai Rekap Jumlah Per lokasi dan Posisi </a></td>  
                                                                                                            <td width="39%"><a href="javascript:cmdSearch(3)" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image10','','<%=approot%>/images/BtnSearchOn.jpg',1)" id="aSearch"><img name="Image10" border="0" src="<%=approot%>/images/BtnSearch.jpg" width="24" height="24" alt="Search "></a>
                                                                                                            <img src="<%=approot%>/images/spacer.gif" width="6" height="1">
                                                                                                            <a href="javascript:cmdSearch(3)">Search Biaya Alih daya - Per lokasi dan Posisi </a></td> 
                                                                                                ...
                                                                                                <table width="100%" border="0" cellspacing="2" cellpadding="2">
                                                                                                    ...
                                                                                                    <tr align="left" valign="top">
                                                                                                        <td>&nbsp;OutSourceEvaluation List </td>
                                                                                                    </tr>
                                                                                                    <tr align="left" valign="top">
                                                                                                        <td>&nbsp;</td>
                                                                                                    </tr>
                                                                                                    ...
                                                                                                    <%if (type == 1 && Divisi.size()>0 ){ %>
                                                                                                    <tr align="left" valign="top">
                                                                                                        <td height="14" valign="middle" colspan="3" class="listtitle">Realiasai Jumlah Alih Daya per Provider</td>
                                                                                                        
                                                                                                    </tr>
                                                                                                    <tr align="left" valign="top">
                                                                                                        <td>
                                                                                                            <%=drawList(out,iCommand ,frmOutSourceEvaluation , Divisi, Posisi, srcObject)%>
                                                                                                        </td>
                                                                                                    </tr>
                                                                                                    <% }else if (type == 2 && Divisi2.size()>0 ){ %>
                                                                                                    ...
                                                                                                    <tr align="left" valign="top">
                                                                                                        <td>
                                                                                                            <%=drawList2(out,iCommand ,frmOutSourceEvaluation , Divisi2, Posisi2, srcObject)%>
                                                                                                        </td>
                                                                                                    </tr>
                                                                                                        <% }else if (type == 3 ){ %>
                                                                                                        <tr align="left" valign="top">
                                                                                                    ...
                                                                                                    <tr align="left" valign="top">
                                                                                                        <td>
                                                                                                            <%=drawList3(out,iCommand ,frmOutSourceEvaluation , Divisi3, Posisi3, srcObject)%>
                                                                                                        </td>
                                                                                                    </tr>
                                                                                                        <% } %>
