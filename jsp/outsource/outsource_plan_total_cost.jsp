...
<a href="javascript:openTotalCost('<%=outSourcePlan.getOID() %>')">View Total Cost</a>
  ...
  <tr><td><hr></td></tr>
  <tr><td class="listheader" align="center" > <h2>Master Plan Alih Daya  <%=outSourcePlan.getPlanYear()%> : <%=outSourcePlan.getTitle() %> </h2> </td></tr>
  <tr><td><hr></td></tr>
  <%
  if(listCost!=null && listCost.size()>0){
    for(int idc=0;idc < listCost.size() ; idc++){
        OutSourceCostMaster cost = (OutSourceCostMaster) listCost.get(idc);
        oidOutMasterCost = cost.getOID();
        strCost =cost.getCostName();
     %>
        <tr>
            <td>
                <table>
                    ...
                    <tr>
                        <td width="50%" >
                           ...
                           %>
                          <%=strCost %> <%=(outSourcePlan.getPlanYear()-1)%>
                        </td>
                        <td width="50%" >
                          Perkiraan  <%=strCost %>  <%=outSourcePlan.getPlanYear()%>
                        </td>
                    </tr>
                    <tr>
                        <td width="50%" >
...
<%                                               for (int k = 0; k < listSourcePlanLocationPosition.size(); k++) {
                                                       ...
                                                       for (int j = 0; j < 1 /*listSourcePlanCost.size()*/; j++) {
                                                         try{       
                                                            ...
                                                            <td width="3" align="center">
                                                               <% try{ %>
                                                                  <input type="text" readonly size="4" name="<%=PstOutSourcePlanCost.fieldNames[PstOutSourcePlanCost.FLD_INCRSTOPREVYEAR]%>-<%=entOutSourcePlanCost.getOutSourcePlanLocId()%>-<%=oidOutMasterCost%>"  value="<%=entOutSourcePlanCost.getIncrsToPrevYear() %>" class="form-control" style="text-align:center">% 
                                                              <%}catch(Exception exc){} %>
                                                            </td>
                                                          ...
                                                          <td width="3" align="center">
                                                            <input type="text"  readonly size="4" name="<%=PstOutSourcePlanCost.fieldNames[PstOutSourcePlanCost.FLD_INCRSTOPREVYEAR]%>-<%=outPlanLoc.getOID()%>-<%=oidOutMasterCost%>"  value="0" class="form-control" style="text-align:center"> 
                                                          </td>
                                                            <%}
                                                      }%> 
                                          </tr>
                                                     ...
                                                     for (int k = 0; k < listSourcePlanLocationPosition.size(); k++) {
                                                       ...
                                                       for (int j = 0; j < 1 /*listSourcePlanCost.size()*/ ; j++) {
                                                         try{       
                                                            ...
                                                            <td width="3" align="center">
                                                               <% try{ %>
                                                                <input type="text" readonly size="8" name="<%=PstOutSourcePlanCost.fieldNames[PstOutSourcePlanCost.FLD_PLANAVRGCOST]%>-<%=entOutSourcePlanCost.getOutSourcePlanLocId()%>-<%=oidOutMasterCost%>"  value="<%=Formater.formatNumber(entOutSourcePlanCost.getPlanAvrgCost(),"###,###.##")  %>" class="form-control" style="text-align:right"> 
                                                              <%}catch(Exception exc){} %>
                                                            </td>
                                                          ...
                                                          <td width="3" align="center"> 
                                                            <input type="text" readonly size="6" name="<%=PstOutSourcePlanCost.fieldNames[PstOutSourcePlanCost.FLD_PLANAVRGCOST]%>-<%=outPlanLoc.getOID()%>-<%=oidOutMasterCost%>"  value="0" class="form-control" style="text-align:center"> 
                                                          </td>
                                                            <%}
                                                      }%>
                                                  </tr>
     ...
     <% }} %>
      <tr>
            <td>
             Total Rencana Biaya <%=outSourcePlan.getPlanYear()%> 
            </td>
        </tr>
        <tr> 
            <td>
                <table width="100%" border="1" style="background-color:#90EE90">
                            <%if(listSourcePlanLocationPosition.size()>0){
                                String whereClause =" ho."+PstOutSourcePlanLocation.fieldNames[PstOutSourcePlanLocation.FLD_OUTSOURCEPLANID]+"="+oidOutSourcePlan;
                                Hashtable<String, Double>  hPlanCost = PstOutSourcePlanCost.hSumPlanAllCostKeyLocPos(0, 0, whereClause, "");
                                Vector<OutSourceValues> vSubTotalCost = new Vector<OutSourceValues>(); 
                                OutSourceValues grandTotalCost = new OutSourceValues();
                                %>
                                      <tr>
