...
<%!
	...
	//int iAddFingerId = Integer.parseInt(String.valueOf(PstSystemProperty.getValueByName("FINGER_PRINT_NUMBER")));
		...
		System.out.println("exc when fetch barcode : " + e.toString());
			...
			"<input type=\"hidden\" class=\"elementForm\" name=\"old_barcode_number_"+i+"\" value=\"" + barcode + "\">"+
                        ...
                        "<input type=\"text\" onclick=\"javascript:searchFinger("+i+")\" readonly=\"readonly\" size=\"10\" class=\"elementForm\" name=\"barcode_number_"+i+"\" value=\"" + barcode + "\" >");
	/*			<input type=\"hidden\" name=\"employee_finger_id\" value=\"" + empFingerId + "\">
			...
			"<input type=\"text\" onclick=\"javascript:searchFinger("+i+")\" readonly=\"readonly\" size=\"10\" class=\"elementForm\" name=\"barcode_number_"+i+"\" value=\"\">");
	/*			<input type=\"hidden\" name=\"employee_finger_id\" value=\"" + empFingerId + "\">
				<input type=\"hidden\" name=\"employee_nick_name\" value=\"" + empNickName + "\">"); */
            }
...
<%
  ...
  //  SessEmployee.setAllEmpFingerPrintNumber();
	...
	System.out.println(" CEK CANTEEN MACHINE :::::::::: "+machineNumbers.length);
	...
	//		finger_id = request.getParameterValues("employee_finger_id");
        ...
        if ((employee_id != null) && (employee_id.length > 0)){
            for (int i = 0; i < employee_id.length; i++){
                ...
                // dan tidak null
                //!newEmpNum.equals(oldEmpNum) && 
                
                
                if(!newEmpNum.equals("") && newEmpNum.length()>0){
                        
                        String strMax = FRMQueryString.requestString(request, "finger_print_number_"+i);                  
                        try{
                            employeeUp.setUserHexId(Integer.parseInt(strMax));                  
                        }catch(Exception E){                            
                            employeeUp.setUserHexId(1111); 
                            Employee objEmpEroor = new Employee();
                            objEmpEroor.setFullName(strName);
                            objEmpEroor.setEmployeeNum(newEmpNum);
                            lstEmp.add(objEmpEroor);
                            System.out.println("[exception] "+E.toString());
                            System.out.println("Employee number :: "+newEmpNum+" name :: "+strName+" Not have pin, set default :: 1111");
                        }
                                                ...
                                                if(vMsg.size()>0){
                                                    ...
                                                    if(lstEmp != null && lstEmp.size() > 0){
                                                        for(int i = 0 ; i < lstEmp.size() ; i++){
                                                            ...
                                                            erremployee = (Employee)lstEmp.get(i);
                                                    %>
                                                    <BR><font color="red">Employee number : <%=erremployee.getEmployeeNum() %> Name :<%=erremployee.getFullName()%> Not have pin number, default 1111, please set !!</font><BR>
