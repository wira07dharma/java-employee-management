    ...
    <td colspan="5"   style="color: <%=warnaFont%>; border-bottom: 1px solid <%=garis1%> "><!-- garis border bawah-->
        <table align="right" height="<%=!strInformation.equals("stop")?"80px":"20px"%>">
            <tr> 
                ...
                <td  style="color: <%=warnaFont%>" align="right"><%=Formater.formatDate(new Date(), "EE,dd-MM-yyyy HH:mm")%> |  <a href="#" title="user"  style="color: <%=warnaFont%>"><%=userIsLogin.toLowerCase()%></a> | <%=menuChangeTemplate%> <%=changePicture%>   <a href="<%=approot%>/logout.jsp" title="logout"  style="color: <%=warnaFont%>">Logout </a><a href="<%=approot%>/logout.jsp?cekOut=1" title="logout"  style="color: <%=warnaFont%>">CekOut </a></td>
                <%}else{%>
