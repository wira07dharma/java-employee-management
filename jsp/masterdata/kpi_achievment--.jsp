<%@page import="com.dimata.harisma.entity.masterdata.KPI_Employee_Achiev"%>
...
<%!    public String drawList(int iCommand, Vector objectClass, long kpi_achievmentId, FrmKPI_Company_Target frmObject, int tahun, long kpiListId, long CompanyId) {
        ...
        for (int kl = month1; kl < (month2+1); kl++ ){
            ...
            ctrlist.addHeader(""+monthString,"");
        }
            
        ctrlist.addHeader("TOTAL","10%");   
        ctrlist.addHeader("COMPETITOR","10%");   
        ctrlist.addHeader("COMPETITOR VALUE","10%");   
        
        ctrlist.addHeader("Achievment","20%");  
        ctrlist.addHeader("GAP","10%");  
        ...
        int index = -1;
            ...
            double n = PstKPI_Employee_Achiev.listAlldataAchCount(tahun, kpiListId, CompanyId,0,0,0);  
	...
	//	    total = total + kPI_Company_Target.getTarget();
         //           
           //     }    
            ...
            lstData.add(rowx);
           // lstLinkData.add(String.valueOf(kpi_achievment.getOID()));
...
<%!
	...
	public String drawListAll(JspWriter outJsp,int iCommand, Hashtable objectClass, long kpi_achievmentId, FrmKPI_Company_Target frmObject, int tahun, long kpiListId, long companyId, long divId, Hashtable HDepartment,Vector VDepartment,long depId, Hashtable HSection,Vector VSection,long secId, Hashtable HEmployee,Vector VEmployee)
{
		ControlList ctrlist = new ControlList();
                ctrlist.setAreaWidth("100%");
                ctrlist.setListStyle("listgen");
...
<%!
	...
	public String drawListAllEdit(JspWriter outJsp,int iCommand, Hashtable objectClass, long kpi_achievmentId, FrmKPI_Company_Target frmObject, int tahun, long kpiListId, long companyId, long divId, Hashtable HDepartment,Vector VDepartment,long depId, Hashtable HSection,Vector VSection,long secId, Hashtable HEmployee,Vector VEmployee,Hashtable HlistKPI_Employee_Achiev,long CompanyId)
{
		ControlList ctrlist = new ControlList();
                ctrlist.setAreaWidth("100%");
                ctrlist.setListStyle("listgen");
		...
		int index = -1;
			 ...
			 Division division = (Division)VDivision.get(i);
                         KPI_Division_Target kPI_Division_Target = (KPI_Division_Target) objectClass.get(division.getOID());
                         double valueK = PstKPI_Employee_Achiev.listAlldataAchCount(tahun, kpiListId, CompanyId,division.getOID(),0,0);  
                         if (kPI_Division_Target == null){
                             kPI_Division_Target = new KPI_Division_Target();
                         }
			 rowx = new Vector();
                        if (kPI_Division_Target.getTarget()>0 && iCommand ==  Command.EDIT){
                         ...
                         rowx.add(""+valueK);
                            ...
                            if (divId >0 && divId == division.getOID()){
                                     ...
                                     Department department = (Department)VDepartment.get(id);
                                     KPI_Department_Target kPI_Department_Target = (KPI_Department_Target) HDepartment.get(department.getOID());
                                     double valueDep = PstKPI_Employee_Achiev.listAlldataAchCount(tahun, kpiListId, CompanyId,divId ,department.getOID(),0); 
                                     if (kPI_Department_Target == null){
                                         kPI_Department_Target = new KPI_Department_Target();
                                     }
                                     rowx = new Vector();
                                    // rowx.add(""+department.getDepartment() );
                                     ...
                                     rowx.add(""+Formater.formatNumber(((kPI_Department_Target.getTarget()/vtotal.size())), "#,###.##"));
                                     rowx.add(""+valueDep); 
                                     if (kPI_Department_Target.getTarget() > 0){
                                        lstData.add(rowx);
                                     } 
                                        ...
                                        if (depId > 0 && depId == department.getOID()){     
                                            for (int is = 0; is < VSection.size(); is++) {
                                                         Section section = (Section)VSection.get(is);
                                                         KPI_Section_Target kPI_Section_Target = (KPI_Section_Target) HSection.get(section.getOID());
                                                         double valueSec = PstKPI_Employee_Achiev.listAlldataAchCount(tahun, kpiListId, CompanyId,divId,depId,secId); 
                                                         if (kPI_Section_Target == null){
                                                             kPI_Section_Target = new KPI_Section_Target();
                                                         }
                                                         rowx = new Vector();
                                                        // rowx.add(""+department.getDepartment() );
                                                         ...
                                                         rowx.add(""+Formater.formatNumber(((kPI_Section_Target.getTarget()/vtotal.size())), "#,###.##"));
                                                         rowx.add(""+valueSec); 
                                                         if (kPI_Section_Target.getTarget() > 0){
                                                            lstData.add(rowx);
                                                        }
                                                           ...
                                                           if (secId > 0 && secId == section.getOID()){                                                  
                                                            for (int ie = 0; ie < VEmployee.size(); ie++) {
                                                                             Employee employee = (Employee)VEmployee.get(ie);
                                                                             KPI_Employee_Target kPI_Employee_Target = (KPI_Employee_Target) HEmployee.get(employee.getOID());
                                                                             KPI_Employee_Achiev kPI_Employee_Achiev = (KPI_Employee_Achiev) HlistKPI_Employee_Achiev.get(employee.getOID());
                                                                             if (kPI_Employee_Target == null){
                                                                                 kPI_Employee_Target = new KPI_Employee_Target();
                                                                             }
                                                                             if (kPI_Employee_Achiev == null){
                                                                            ...
                                                                            // rowx.add(""+department.getDepartment() );
                                                                             ...
                                                                             //rowx.add("Section ");
                                                                             //rowx.add("Name ");
                                                                             //rowx.add(""+Formater.formatNumber((total/100*kPI_Employee_Target.getTarget()), "#,###.##"));
                                                                             ...
                                                                             rowx.add(""+Formater.formatNumber((kPI_Employee_Target.getTarget()), "#,###.##") );
                                                                             rowx.add(""+Formater.formatNumber(((kPI_Employee_Target.getTarget()/vtotal.size())), "#,###.##"));
                                                                             rowx.add("<input type=\"text\" name=\""+frmObject.fieldNames[FrmKPI_Company_Target.FRM_FIELD_ACHIEVEMENT_E]+employee.getOID() +"\" value=\""+Formater.formatNumber((kPI_Employee_Achiev.getAchievement()), "#,###.##") +"\" class=\"elemenForm\"> * ");
                                            ...
                                            VemployeDepart = PstEmployee.list(0, 0, " DEPARTMENT_ID = "+department.getOID()+" AND SECTION_ID = "+0,"");
                                                 //employee
                                           Hashtable HlistKPI_Employee_Ach = new Hashtable(1, 1);

                                           HlistKPI_Employee_Ach = PstKPI_Employee_Achiev.Hlist(0, 0, whereClauseKpiList, "");
                                                ...
                                                for (int ie = 0; ie < VemployeDepart.size(); ie++) {
                                                                             ...
                                                                             KPI_Employee_Achiev kPI_Employee_Achiev = (KPI_Employee_Achiev) HlistKPI_Employee_Ach.get(employee.getOID());
                                                                            ...
                                                                            // rowx.add(""+department.getDepartment() );
                                                                             ...
                                                                             //rowx.add("Section ");
                                                                             //rowx.add("Name ");
                                                                             rowx.add("<input type=\"hidden\" name=\""+frmObject.fieldNames[FrmKPI_Company_Target.FRM_FIELD_PERCENT_E]+employee.getOID() +"\" value=\""+Formater.formatNumber((kPI_Employee_Target.getTarget()/total*100), "#,###.##")+"\" class=\"elemenForm\">"+Formater.formatNumber((kPI_Employee_Target.getTarget()/total*100), "#,###.##"));
                                                                               // rowx.add("safas");
                                                                             rowx.add(""+Formater.formatNumber((kPI_Employee_Target.getTarget()), "#,###.##") );
                                                                             rowx.add(""+Formater.formatNumber(((kPI_Employee_Target.getTarget()/vtotal.size())), "#,###.##"));
                                                                            rowx.add("<input type=\"text\" name=\""+frmObject.fieldNames[FrmKPI_Company_Target.FRM_FIELD_ACHIEVEMENT_E]+employee.getOID() +"\" value=\""+Formater.formatNumber((kPI_Employee_Achiev.getAchievement()), "#,###.##") +"\" class=\"elemenForm\"> ");
...
<%
            ...
            long oidKPI_Company_Target = FRMQueryString.requestLong(request, "hidden_kpi_achievment_id");
          ...
          //  }
           ...
           iErrCode = ctrlKPI_Company_Target.action(iCommand, oidKPI_Company_Target, kpiListId, tahun, CompanyId,type,divId,depId,secId,kpiAchiev);
           ...
           VEmployeeDepart = PstEmployee.list(0, 0, " DEPARTMENT_ID = "+depId+" AND SECTION_ID = "+0,"");
                     }
           ...
           VEmployee = PstEmployee.list(0, 0, " DIVISION_ID = " +divId+ " AND DEPARTMENT_ID="+ depId + " AND SECTION_ID = "+secId,"");
                     }
           
           
           //employee
           Hashtable HlistKPI_Employee_Achiev = new Hashtable(1, 1);
           if (secId>0){
           HlistKPI_Employee_Achiev = PstKPI_Employee_Achiev.Hlist(0, 0, whereClauseKpiList, "");
                     }
            /*switch statement */
            ...
            KPI_Company_Target kpi_achievment = ctrlKPI_Company_Target.getdKPI_Company_Target();
          ...
          //  listKPI_Company_Target = PstKPI_Company_Target.list(start, recordToGet, whereClause, orderClause);
           ...
           //cek dulu sudah ada atau tidak kpi company targetnya
            ...
            if ((iCommand == Command.SAVE) && (iErrCode == FRMMessage.NONE)) {
                //start = PstKPI_Company_Target.findLimitStart(kpi_achievment.getOID(),recordToGet, whereClause);
                oidKPI_Company_Target = kpi_achievment.getOID();
            }
...
function cmdAdd(){
	...
	document.frmKPI_Company_Target.action="kpi_achievment.jsp";
...
function cmdSaveModif(type){
	...
	document.frmKPI_Company_Target.action="kpi_achievment.jsp?type = "+type;
	document.frmKPI_Company_Target.submit();
}
...
function cmdDep(divId){
	...
	document.frmKPI_Company_Target.action="kpi_achievment.jsp?divId = "+divId;
	document.frmKPI_Company_Target.submit();
}
...
function cmdSec(depId){
	...
	document.frmKPI_Company_Target.prev_command.value="<%=prevCommand%>";
        ...
        document.frmKPI_Company_Target.secId.value=0;
	document.frmKPI_Company_Target.action="kpi_achievment.jsp?depId = "+depId;
	document.frmKPI_Company_Target.submit();
}
function cmdEmp(secId){
	...
	document.frmKPI_Company_Target.action="kpi_achievment.jsp?secId = "+secId;
	document.frmKPI_Company_Target.submit();
}
function cmdEditDep(divisiId,tahun,kpilistId,companyId){
	...
	document.frmKPI_Company_Target.command.value="<%=Command.EDIT%>";
	document.frmKPI_Company_Target.prev_command.value="<%=prevCommand%>";
	document.frmKPI_Company_Target.action="kpi_achievment.jsp?divisiOid= "+divisiId+"";
	document.frmKPI_Company_Target.submit();
}
                                          ...
                                          <td height="8"  colspan="3"> 
                                            <table width="100%" border="0" cellspacing="0" cellpadding="0">                                              
                                              ...
                                              <tr align="left" valign="top"> 
                                                   ...
                                                   <%= drawList(iCommand, listKPI_Company_Target, oidKPI_Company_Target, frmKPI_Company_Target, tahun, kpiListId, CompanyId)%>
                                                  ...
                                                  <%if(iCommand == Command.EDIT){%>
                                                   ...
                                                   <%= drawListAllEdit(out,iCommand, HlistKPI_Division_Target, oidKPI_Company_Target, frmKPI_Company_Target, tahun, kpiListId, CompanyId, divId,HlistKPI_Department_Target, VDepartment,depId, HlistKPI_Section_Target, VSection,secId, HlistKPI_Employee_Target, VEmployee,HlistKPI_Employee_Achiev, CompanyId)%>
                                                  
                                            
                                                   <%if ((divId> 0) && (depId> 0)&& (secId> 0) &&  (iCommand == Command.EDIT) && VEmployee.size() > 0 ) {%>
                                                   <a href="javascript:cmdSaveModif(5)" class="command">Save Employee Achiev</a>
