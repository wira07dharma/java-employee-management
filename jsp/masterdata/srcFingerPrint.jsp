...
<%!
...

	{
		...
		for (int i = 0; i < objectClass.size(); i++) {
			...
			lstData.add(rowx);
			lstLinkData.add(fingerPrint.getEmployeeNum()+"','"+String.valueOf(fingerPrint.getFingerPrint()));
		}
...
int indexFinger = FRMQueryString.requestInt(request, "indexFinger");
...
String whereClause = PstFingerPrint.fieldNames[PstFingerPrint.FLD_EMPLOYEE_NUM]
        +" NOT IN ("+SessFingerPrint.getEmployeeBarcode()+")";
...
function cmdEdit(empNum,fingerNum){
        self.opener.document.<%=formName%>.barcode_number_<%=""+indexFinger%>.value = empNum;
        self.opener.document.<%=formName%>.finger_print_number_<%=""+indexFinger%>.value = fingerNum;
        self.close();
}
...
function cmdAdd(){
	...
	document.frmfingerPrint.action="srcFingerPrint.jsp";
                            ...
                            <td valign="top">
		    				  <!-- #BeginEditable "content" --> 
                                    <form name="frmfingerPrint" method ="post" action="">
                                      ...
                                      <input type="hidden" name="indexFinger" value="<%=""+indexFinger%>">
