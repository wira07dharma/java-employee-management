...
public String drawList(Vector objectClass )
{
	...
	ctrlist.addHeader("Dep Head","18%");	
	ctrlist.addHeader("HR Manager","18%");
	...
	for (int i=0; i<objectClass.size(); i++) 
	{
		...
		LeaveApplication objLeaveApplication = (LeaveApplication) temp.get(0);
		...
		if(objLeaveApplication.getDepHeadApproval() != 0)
		{
			try
			{
				Employee objEmployee = PstEmployee.fetchExc(objLeaveApplication.getDepHeadApproval());
				strDepHead = objEmployee.getFullName();
			}
			catch(Exception e)
			{
				System.out.println("Exc when fetch dep head approval : " + e.toString());
			}
		}
		...
		if(objLeaveApplication.getHrManApproval() != 0)
		{
			try
			{
				Employee objEmployee = PstEmployee.fetchExc(objLeaveApplication.getHrManApproval());
				strHrMan = objEmployee.getFullName();
			}
			catch(Exception e)
			{
				System.out.println("Exc when fetch hr manager approval : " + e.toString());
			}
		}
		
		Vector rowx = new Vector();
		if(objLeaveApplication.getDocStatus() == PstLeaveApplication.FLD_DOC_STATUS_VALID)
		{
			rowx.add("<a href=\"javascript:cmdEdit('"+objLeaveApplication.getOID()+"')\">"+employee.getEmployeeNum()+"</a>");		
		}
		else
		{
		...
		rowx.add(strDepHead);
		rowx.add(strHrMan);
		rowx.add(PstLeaveApplication.fieldStatusNames[objLeaveApplication.getDocStatus()]);						
...
long oidPublicLeaveType= FRMQueryString.requestLong(request, ""+FrmPublicLeaveDetail.fieldNames[FrmPublicLeaveDetail.FRM_FIELD_TYPE_LEAVE]);  
...
vectSize = sessLeaveApplication.getCountSearchLeaveApplication(srcLeaveApplication);
...
Vector records = sessLeaveApplication.searchLeaveApplication(srcLeaveApplication, start, recordToGet);
%>
<!-- End of Jsp Block -->
...
function cmdEdit(oid)
{
	...
	document.frm_leave_application.hidden_leave_application_id.value = oid;
	document.frm_leave_application.action="leave_application_edit.jsp";
	document.frm_leave_application.submit();
}
...
function cmdBack()
{
	...
	document.frm_leave_application.action="src_leave_application.jsp";
	document.frm_leave_application.submit();
}
