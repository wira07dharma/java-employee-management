...
<%-- 
    Document   : leave_app_execute
    Created on : Jan 14, 2010, 11:56:37 AM
    Author     : Tu Roy
--%>
...
public String drawList(Vector objectClass,boolean isHRDLogin,int isAdminExecuteLeave,Hashtable hashViewPeriod,I_Leave leaveConfig,long hrdApprove ) 
{
...
long hrdApprove=0;
if(leaveConfig!=null && leaveConfig.getConfigurationLeaveApprovall()==I_Leave.LEAVE_CONFIG_AFTER_APPROVALL_HRD_YES_EXECUTE){
         Vector listHRMan = leaveConfig.listHRManager(0);
         if(listHRMan!=null && listHRMan.size()>0){
             for(int x=0;x<listHRMan.size();x++){
                  Employee employee = (Employee)listHRMan.get(x); 
                  if(emplx.getOID()==employee.getOID()){ 
                        hrdApprove=employee.getOID(); 
                        break;
                  }
             }
         }
...
//update by satrya 2012-07-25
if(iCommand == Command.LIST || (source != null && "presence".equals(source) && iCommand == Command.EDIT)){
    ...
    if(emplx!=null && emplx.getOID()!=0 && leave_application_id != null && leave_application_id.length > 0){         
        ...
        for(int i=0; i<leave_application_id.length; i++){
            ...
            if(ix==1){
                ...
                if(hrdApprove!=0 && appid!=0 && leaveConfig!=null && leaveConfig.getConfigurationLeaveApprovall()==I_Leave.LEAVE_CONFIG_AFTER_APPROVALL_HRD_YES_EXECUTE){
                    try{
                            ...
                            leaveApplication.setHrManApproval(hrdApprove);
                            leaveApplication.setHrManApproveDate(new Date());  
                            PstLeaveApplication.updateExc(leaveApplication); 
                            CtrlLeaveApplication.sendEmail(leaveApplication,approot,hrdApprove); 
                    }catch(Exception exc){
...
vectSize = sessLeaveApplication.countSearchLeaveApplicationDocStatus(srcLeaveApp,(leaveConfig!=null && leaveConfig.getConfigurationLeaveApprovall()==I_Leave.LEAVE_CONFIG_AFTER_APPROVALL_HRD_YES_EXECUTE?PstLeaveApplication.FLD_STATUS_APPlICATION_TO_BE_APPROVE:PstLeaveApplication.FLD_STATUS_APPlICATION_APPROVED));
if((iCommand==Command.FIRST)||(iCommand==Command.NEXT)||(iCommand==Command.PREV)||(iCommand==Command.LAST)||(iCommand==Command.LIST))
{
	start = ctrlLeave.actionList(iCommand, start, vectSize, recordToGet);
}
Hashtable hashViewPeriod = sessLeaveApplication.viewLeaveAppPeriod(srcLeaveApp, start, recordToGet,(leaveConfig!=null && leaveConfig.getConfigurationLeaveApprovall()==I_Leave.LEAVE_CONFIG_AFTER_APPROVALL_HRD_YES_EXECUTE?PstLeaveApplication.FLD_STATUS_APPlICATION_TO_BE_APPROVE:PstLeaveApplication.FLD_STATUS_APPlICATION_APPROVED)); 
Vector records = sessLeaveApplication.searchLeaveApplicationDocumentStatus(srcLeaveApp, start, recordToGet,(leaveConfig!=null && leaveConfig.getConfigurationLeaveApprovall()==I_Leave.LEAVE_CONFIG_AFTER_APPROVALL_HRD_YES_EXECUTE?PstLeaveApplication.FLD_STATUS_APPlICATION_TO_BE_APPROVE:PstLeaveApplication.FLD_STATUS_APPlICATION_APPROVED));
//Vector records = sessLeaveApplication.searchLeaveApplicationExcecute(srcLeaveApp, start, recordToGet);
...
<html>
<head>
<title>HARISMA - Leave Application Execute</title>
<script language="JavaScript">

function cmdProccess() {
    document.frm_leave_application.command.value = "<%=String.valueOf(Command.LIST)%>";
    document.frm_leave_application.action = "leave_app_execute.jsp";
                            ...
                            <td valign="top">
							  ...
							  %> 
								<form name="frm_leave_application" method="post" action="">
								  ...
								  {
									 out.println(drawList(records,isHRDLogin,isAdminExecuteLeave,hashViewPeriod,leaveConfig,hrdApprove)); 
								  }										  
								  else
								  {
