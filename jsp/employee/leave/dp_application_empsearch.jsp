...
{		
	...
	for (int i = 0; i < objectClass.size(); i++) 
	{
		...
		lstData.add(rowx);
		lstLinkData.add(String.valueOf(employee.getOID()) + "','" + employee.getFullName() + "','" + strDeparment);
	}
...
function cmdSearchEmp()
{
	...
	document.frm_dp_application.action="dp_application_empsearch.jsp";
	document.frm_dp_application.submit();	
}

function cmdEdit(oid, empname, department) 
{
	self.opener.document.frm_dp_application.<%=FrmDpApplication.fieldNames[FrmDpApplication.FRM_FLD_EMPLOYEE_ID]%>.value = oid;
	self.opener.document.frm_dp_application.EMP_NAME.value = empname;	
	self.opener.document.frm_dp_application.EMP_SIGN_NAME.value = empname;			
	self.opener.document.frm_dp_application.EMP_DEPARTMENT.value = department;
	self.close();
}
</script>
...
<script language="javascript">
	document.frm_dp_application.EMP_NUMBER.focus();
</script>
</body>
</html>
