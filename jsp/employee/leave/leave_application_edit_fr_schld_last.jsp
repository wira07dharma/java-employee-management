...
{
	for(int i=0; i<leaveTypeMaximum; i++)
	{
		int intLeaveType = FRMQueryString.requestInt(request, (FrmLeaveApplicationDetail.fieldNames[FrmLeaveApplicationDetail.FRM_FLD_LEAVE_TYPE]+"_"+i));
		Date dtTakenDateFrom = FRMQueryString.requestDate(request, (FrmLeaveApplicationDetail.fieldNames[FrmLeaveApplicationDetail.FRM_FLD_TAKEN_DATE_FROM]+"_"+i));
		Date dtTakenDateTo = FRMQueryString.requestDate(request, (FrmLeaveApplicationDetail.fieldNames[FrmLeaveApplicationDetail.FRM_FLD_TAKEN_DATE_TO]+"_"+i));
	
		if( (intLeaveType>-1) && (dtTakenDateFrom!=null) && (dtTakenDateTo!=null) )
		{
			LeaveApplicationDetail objLeaveApplicationDetail = new LeaveApplicationDetail();
			objLeaveApplicationDetail.setLeaveType(intLeaveType);
			objLeaveApplicationDetail.setTakenDateFrom(dtTakenDateFrom);
			objLeaveApplicationDetail.setTakenDateTo(dtTakenDateTo);	
...
if(listOfDetail!=null && listOfDetail.size()>0)
{
	...
	com.dimata.util.DateCalc objDateCalc = new com.dimata.util.DateCalc();
	for(int ik=0; ik<listOfDetailSize; ik++)
	{
		...
		{
			if(objLeaveApplicationDetail.getTakenDateFrom()!=null && objLeaveApplicationDetail.getTakenDateTo()!=null)
			{                    
				intAlToBeTaken = intAlToBeTaken + (int) (objDateCalc.dayDifference(objLeaveApplicationDetail.getTakenDateFrom(), objLeaveApplicationDetail.getTakenDateTo())+1);
			}			
		}
		...
		if(objLeaveApplicationDetail.getLeaveType() == PstLeaveApplicationDetail.LEAVE_APPLICATION_LL)
		{
			if(objLeaveApplicationDetail.getTakenDateFrom()!=null && objLeaveApplicationDetail.getTakenDateTo()!=null)
			{                    
				intLlToBeTaken = intLlToBeTaken + (int) (objDateCalc.dayDifference(objLeaveApplicationDetail.getTakenDateFrom(), objLeaveApplicationDetail.getTakenDateTo())+1);
			}						
		}
	}
}
                                                    ...
                                                    <tr> 
                                                      <td> 
                                                        <table width="100%" border="0"  bgcolor="#F9FCFF" cellpadding="1" cellspacing="1">
														  ...
														  <%
														  int detLeaveType = -1;
														  Date detTakenDateFrom = null;
														  Date detTakenDateTo = null;														  														  
														  
														  int yearInterval = installationDate.getYear() - (new Date()).getYear() - 1;														  
														  //for(int it=0; it<leaveTypeMaximum; it++)
                                                          ...
                                                          <tr> 
                                                            ...
                                                            <%
															/*												  
															Vector leaveTypeKey = new Vector(1,1);
															Vector leaveTypeValue = new Vector(1,1);
															  
															leaveTypeKey.add("select leave type ...");
															leaveTypeValue.add("-1");															  

															leaveTypeKey.add(""+PstLeaveApplicationDetail.strApplicationType[PstLeaveApplicationDetail.LEAVE_APPLICATION_AL]);
															leaveTypeValue.add(""+PstLeaveApplicationDetail.LEAVE_APPLICATION_AL);															  

															leaveTypeKey.add(""+PstLeaveApplicationDetail.strApplicationType[PstLeaveApplicationDetail.LEAVE_APPLICATION_LL]);
															leaveTypeValue.add(""+PstLeaveApplicationDetail.LEAVE_APPLICATION_LL);															  

															leaveTypeKey.add(""+PstLeaveApplicationDetail.strApplicationType[PstLeaveApplicationDetail.LEAVE_APPLICATION_MATERNITY]);
															leaveTypeValue.add(""+PstLeaveApplicationDetail.LEAVE_APPLICATION_MATERNITY);															  

															leaveTypeKey.add(""+PstLeaveApplicationDetail.strApplicationType[PstLeaveApplicationDetail.LEAVE_APPLICATION_DC]);
															leaveTypeValue.add(""+PstLeaveApplicationDetail.LEAVE_APPLICATION_DC);															  

															leaveTypeKey.add(""+PstLeaveApplicationDetail.strApplicationType[PstLeaveApplicationDetail.LEAVE_APPLICATION_UNPAID]);
															leaveTypeValue.add(""+PstLeaveApplicationDetail.LEAVE_APPLICATION_UNPAID);															  

															leaveTypeKey.add(""+PstLeaveApplicationDetail.strApplicationType[PstLeaveApplicationDetail.LEAVE_APPLICATION_SPECIAL]);
															leaveTypeValue.add(""+PstLeaveApplicationDetail.LEAVE_APPLICATION_SPECIAL);															  
															*/
															
															if(it < listOfDetailSize)
															{
																...
																detTakenDateFrom = objLeaveApplicationDetail.getTakenDateFrom();
																detTakenDateTo = objLeaveApplicationDetail.getTakenDateTo();														  														  																
															}
															else
															{
															    detLeaveType = -1;
															    detTakenDateFrom = null;
															    detTakenDateTo = null;														  														  															
															}
															
															//out.println(ControlCombo.draw(FrmLeaveApplicationDetail.fieldNames[FrmLeaveApplicationDetail.FRM_FLD_LEAVE_TYPE]+"_"+it, null, ""+detLeaveType, leaveTypeValue, leaveTypeKey, ""));														  
															%>
															<input type="hidden" name="<%=FrmLeaveApplicationDetail.fieldNames[FrmLeaveApplicationDetail.FRM_FLD_LEAVE_TYPE]+"_"+it%>" value="<%=detLeaveType%>">
															<%=(it+1)+".&nbsp;"%><input type="text" name="LEAVE_APP_TYPE" style="background-color:#F5F5F5" readonly value="<%=PstLeaveApplicationDetail.strApplicationType[detLeaveType]%>">															
															</td>
                                                            <td width="6%"><i>from</i></td>   
                                                            <td width="31%"> 
															<input type="hidden" name="<%=FrmLeaveApplicationDetail.fieldNames[FrmLeaveApplicationDetail.FRM_FLD_TAKEN_DATE_FROM]+"_"+it+"_yr"%>" value="<%=detTakenDateFrom.getYear()%>">
															<input type="hidden" name="<%=FrmLeaveApplicationDetail.fieldNames[FrmLeaveApplicationDetail.FRM_FLD_TAKEN_DATE_FROM]+"_"+it+"_mn"%>" value="<%=detTakenDateFrom.getMonth()%>">															
															<input type="hidden" name="<%=FrmLeaveApplicationDetail.fieldNames[FrmLeaveApplicationDetail.FRM_FLD_TAKEN_DATE_FROM]+"_"+it+"_dy"%>" value="<%=detTakenDateFrom.getDate()%>">																														
															<input type="text" name="LEAVE_APP_DATE_FROM" style="background-color:#F5F5F5" readonly value="<%=detTakenDateFrom%>">																																													
															</td>
                                                            <td width="4%"><i>to</i></td>
                                                            <td width="27%"> 
															<input type="hidden" name="<%=FrmLeaveApplicationDetail.fieldNames[FrmLeaveApplicationDetail.FRM_FLD_TAKEN_DATE_TO]+"_"+it+"_yr"%>" value="<%=detTakenDateTo.getYear()%>">
															<input type="hidden" name="<%=FrmLeaveApplicationDetail.fieldNames[FrmLeaveApplicationDetail.FRM_FLD_TAKEN_DATE_TO]+"_"+it+"_mn"%>" value="<%=detTakenDateTo.getMonth()%>">															
															<input type="hidden" name="<%=FrmLeaveApplicationDetail.fieldNames[FrmLeaveApplicationDetail.FRM_FLD_TAKEN_DATE_TO]+"_"+it+"_dy"%>" value="<%=detTakenDateTo.getDate()%>">																														
															<input type="text" name="LEAVE_APP_DATE_TO" style="background-color:#F5F5F5" readonly value="<%=detTakenDateTo%>">																																																																											
															</td>
                                                    ...
                                                    <tr align="center"> 
													  ...
													  {												  
														  ...
														  { 
															  ...
															  if(objLeaveApplication.getDepHeadApproval() == objEmp.getOID())
															  {
																  strDivHeadManager = objEmp.getFullName();
																  break;
															  }
														  }
														  out.println(strDivHeadManager);														  
													  }
                                                      %>
                                                      </td>
                                                      <td width="33%"> 
													  ...
													  if( objLeaveApplication.getHrManApproval()!=0 )
													  {
														  ...
														  for(int i=0; i<listHrMan.size(); i++)
														  {
															  ...
															  if(objLeaveApplication.getHrManApproval() != objEmp.getOID())
															  {
															  	  strHrMan = objEmp.getFullName();	
															  }
														  }
														  out.println(strHrMan);														  
													  }
													  else
													  {
														 out.println("&nbsp;");
