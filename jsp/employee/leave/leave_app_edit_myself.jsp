...
<%-- 
    Document   : leave_app_edit_myself
    Created on : May 10, 2010, 1:50:34 PM
    Author     : root
--%>
                    ...
                    if(oidApprover!=0){
                        ...
                        switch(indexApproval){
                            case 1 : objLeaveApplication.setDepHeadApproval(oidApprover);
                                     //if(objLeaveApplication.getDepHeadApproveDate()==null){
                            ...
                            case 3 : objLeaveApplication.setGmApproval(oidApprover);
                                    //if(objLeaveApplication.getGmApprovalDate()==null){
...
function cmdCancel()
{
	...
	document.frm_leave_application.action="leave_app_edit_myself.jsp";
...
function cmdBack()
{
	document.frm_leave_application.command.value="<%=String.valueOf(Command.LOAD)%>"; 
	...
	if(empLoggedIn != 0)
	{
		if(empApprovalSelected != 0)
		{
			...
			//{
				...
				document.frm_leave_application.action="leave_app_approval_myself.jsp";
				document.frm_leave_application.submit();  		
			//}else{
                          /*  if(index==1){
                                      ...
                                      <tr>
                                          ...
                                          <td width="60%" ><b>
                                              <%if(false){
                                                  ...
                                                  try{
                                                      employee = PstEmployee.fetchExc(objLeaveApplication.getDepHeadApproval());
                                                      strApp = employee.getFullName();
                                                  }catch(Exception ex){}
                                              %>
                                                  <%=strApp%>
                                                  <input type="hidden" name="<%=FrmSpecialLeave.fieldNames[FrmSpecialLeave.FRM_FIELD_APPROVAL_ID]%>" value="<%=String.valueOf(objLeaveApplication.getDepHeadApproval())%>">
                                              <%}else{%>
                                              <%												  
                                                  if(objLeaveApplication.getOID()!=0)
                                                  {
                                      ...
                                      <tr>
                                          ...
                                          <% 
                                              if(false)
                                              {
                                                  ...
                                                  Employee employee = new Employee();
                                                  try{
                                                      employee = PstEmployee.fetchExc(objLeaveApplication.getHrManApproval());
                                                      strApp = employee.getFullName();
                                                  }catch(Exception ex){}
                                              %>
                                                  <%=strApp%>
                                                  <input type="hidden" name="<%=FrmLeaveApplication.fieldNames[FrmLeaveApplication.FRM_FLD_HR_MAN_APPROVAL]%>" value="<%=String.valueOf(objLeaveApplication.getHrManApproval())%>">
                                              <%}else{%>
                                              <%												  
                                                    if(objLeaveApplication.getOID()!=0)
                                                    {
