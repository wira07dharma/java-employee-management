...
<html><!-- #BeginTemplate "/Templates/maintab.dwt" -->
    <head>
        ...
        <script type="text/javascript">
            ...
            function cmdAddFamily(oid){
                var newWindow = window.open("../databank/familymemberdetail.jsp?command=<%=Command.ADD%>&employee_oid=<%=oidEmployee%>&oid="+oid, "Add Family Member" , "height=650,width=990,status=yes,toolbar=no,menubar=no,location=center,scrollbars=yes");
                newWindow.focus();
            }
            
            function cmdDetailFamily(oid){
                document.FrmFamilyMember.family_member_oid.value=oid;
                var newWindow = window.open("../databank/familymemberdetail.jsp?command=<%=Command.EDIT%>&employee_oid=<%=oidEmployee%>&oid="+oid, "Edit Family Member" , "height=650,width=990,status=yes,toolbar=no,menubar=no,location=center,scrollbars=yes");
                newWindow.focus();
            }
            
            function cmdBackEmployeeList() {
                document.FrmFamilyMember.action = "employee_list.jsp?command=<%=Command.FIRST%>";
                document.FrmFamilyMember.submit();
            }
        </script>
        <style type="text/css">
        ...
        <div class="content-main">
            <form name="FrmFamilyMember" method="post" action="">
                    ...
                    <table cellspacing="1" cellpadding="1" class="tblStyle">
                        ...
                        <%  
                            ...
                            for (int idx = 0; idx < vctMember.size(); idx++) {
                                FamilyMember member = (FamilyMember) vctMember.get(idx);
                                String relationship = "";
                                try {
                                    FamRelation famRelation = PstFamRelation.fetchExc(Long.valueOf(member.getRelationship()));
                                    relationship = famRelation.getFamRelation();
                                } catch (Exception e) {
                                    
                                }
                                
                                btnDetail = "<button type='button' class=\"btn\" onclick=\"cmdDetailFamily('"+member.getOID()+"')\">Detail</button>";
                                btnDelete = "<button type='button' class=\"btn\" onclick=\"cmdAsking('"+member.getOID()+"')\">Delete</button>";
                                
                                out.print("<tr>");
                                out.print("<td>" + (idx + 1) + "</td>");
                                out.print("<td>" + member.getFullName() + "</td>");
                                out.print("<td>" + relationship + "</td>");
                                out.print("<td>" + btnDetail + "&nbsp; " + btnDelete + "</td>");
                                out.print("</tr>");
                            }
                            ...
                            for (int idx=0; idx < 0; idx++ ){ 
                                ...
                                if (vctMember != null && vctMember.size() > 0 && vctMember.size() > idx) {
                                    ...
                                    //ADD DETAIL
                                    btnDetail = "<button class=\"btn\" onclick=\"cmdDetailFamily('"+famX.getOID()+"')\">&iexcl;</button>";
                ...
                * <%= FrmFamilyMember.getErrorMsg(FrmEmpEducation.FRM_FIELD_EDUCATION_ID) %>                                                                                                                                           
                    ...
                    <div>
                        <button type="button" id="btn" onclick="cmdAddFamily(0)">Add New Family</button>
                        <button type="button" id="btn" onclick="cmdExportExcel()">Export to Excel</button>
