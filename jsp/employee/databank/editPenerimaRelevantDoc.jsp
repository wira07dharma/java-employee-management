...
<%    
    ...
    if ( iCommand == Command.GOTO ){
                          String whereClause = PstEmpRelvtDocPage.fieldNames[PstEmpRelvtDocPage.FLD_DOC_RELEVANT_ID] + " = " + relevanDocId;
                          Vector docAttach = PstEmpRelvtDocPage.list(0, 0, whereClause, "");
                          for (int em = 0; em < docAttach.size(); em++){
                              EmpRelvtDocPage empRelvtDocPage = (EmpRelvtDocPage) docAttach.get(em);
                              ...
                              DataSource fds=new FileDataSource(harismaURL+empRelvtDocPage.getFileName());
                              
                              attachment.add(fds);
                              attachmentName.add(empRelvtDocPage.getFileName());
                          }
                            Vector<String> listRec = new Vector();
                            listRec.add("" + emailAddress);
                            email.sendEmail(listRec, null, null, " Relevant List ", "Relevan Doc", attachment, attachmentName);
    }
%>
<html>
<head> 
