...
public String drawList(Vector objectClass) {
	...
	for (int i = 0; i < objectClass.size(); i++) {
		...
		try
		{
			...
			|| sc.getCategoryType()==PstScheduleCategory.CATEGORY_NEAR_ACCROSS_DAY
			){
...
<%
// check user data (department and position)
...
String startDatePeriod = (String.valueOf(PstSystemProperty.getValueByName("START_DATE_PERIOD")));
out.println("START_DATE_PERIOD  "+startDatePeriod);
...
if(iCommand==Command.SAVE)
{
	...
	{
		empSchedulePrevUpdate.setOID(empSchedule.getOID());
		empSchedulePrevUpdate.setPeriodId(empSchedule.getPeriodId());
		empSchedulePrevUpdate.setEmployeeId(empSchedule.getEmployeeId());				
	}
	com.dimata.harisma.entity.attendance.PstPresence.importPresenceTriggerByImportEmpScheduleExcel(empSchedulePrevUpdate, empSchedule); 																		
...
//isAuthorizeUserLogin = isAuthorizeUserLogin && (employee.getDepartmentId()==departmentOid ? true : false);
                                                  ...
                                                  <table width="100%" border="0" cellspacing="2" cellpadding="2">
                                                                  ...
                                                                  <td>
                                                                    <table border="1" cellpadding="3" cellspacing="0" bgcolor="#B0DDF0">
                                                                      ...
                                                                      <tr>
																		...
																		out.println("nDayOfWeekStart  "+nDayOfWeekStart );
																		int total = nDayOfMonthStart + nDayOfWeekStart + 3;
																		// nDayOfMonthStart = jumlah hari dalam bulan itu
																		// nDayOfWeekStart = dimulai dari hari keberapa ( hitung mulai senin)
																		...
																		int jumlah = nDayOfMonthStart + nDayOfWeekStart + 3;
																		out.println("nDayOfWeekStart  "+nDayOfWeekStart);
																		int awal = dateStart-1;
																		for (i = 25; i < nDayOfMonthStart+dS; i++)
																		{
																			if(awal==nDayOfMonthStart){
																				awal=1;
                                                                        ...
                                                                        for (j = 0; j < dateEnd + remainder; j++, i++) {
                                                                            if ((i % 7) == 0) {
																				...
																				out.println(ControlCombo.draw(dField[j], null, dSelect[j], scd_value, scd_key, "onChange=\"javascript:changeSchedule(this,'"+(dSelect[dS - 1])+"','"+dS+"')\""));
																				if(dSelect2nd[j].equals("0"))
																					{																						
																						...
																						else
																						{
																							...
																							{
																								out.println("<br>"+ControlCombo.draw(dField2nd[j], null, dSelect2nd[j], scd_2nd_value, scd_2nd_key, "style=\"visibility:''\" onChange=\"javascript:change2ndSchedule(this,'"+(dSelect2nd[dS - 1])+"','"+dS+"')\""));																						
																							}
																						}																																												
																					}
																					else
                                                    ...
                                                    <tr>
                                                      ...
                                                      <td width="91%">
														...
														if(!(isAuthorizeUserLogin || isHRDLogin))
														{
															ctrLine.setConfirmDelCaption("");
															ctrLine.setDeleteCaption("");
															ctrLine.setEditCaption("");
