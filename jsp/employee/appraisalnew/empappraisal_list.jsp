...
if(iCommand == Command.SAVE && prevCommand == Command.ADD){
	start = PstEmployee.findLimitStart(oidAppraisalMain,recordToGet, whereClause,orderClause);
        ...
        if(isHRDLogin)
            ...
            vectSize = SessAppraisalMain.countAppraisal(srcAppraisal,depGroup);	
}
...
if(iCommand == Command.SAVE && prevCommand==Command.ADD){
        if(isHRDLogin){
            ...
            listAppraisalMain = SessAppraisalMain.searchAppraisal(new SrcAppraisal(), start, recordToGet,depGroup);
        }    
}else{
        if(isHRDLogin)
           ...
           //listAppraisalMain = SessAppraisalMain.searchAppraisal(new SrcAppraisal(), start, recordToGet,depGroup); 
           listAppraisalMain = SessAppraisalMain.searchAppraisal(srcAppraisal, start, recordToGet,depGroup); 
}
%>
<!-- End of Jsp Block -->
