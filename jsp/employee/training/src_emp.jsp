<%-- 
    Document   : src_emp
    Created on : Jan 28, 2009, 5:38:24 PM
    Author     : bayu
--%>
...
public String drawList(Vector objectClass, int st){
	...
	for (int i = 0; i < objectClass.size(); i++) {
		...
		lstData.add(rowx);
		lstLinkData.add(String.valueOf(employee.getOID()) + "','" + String.valueOf(employee.getDepartmentId()) + "','" + String.valueOf(employee.getEmployeeNum()) + "','" + employee.getFullName());
	}
	return ctrlist.draw();
}
%>
...
<%
    ...
    String payroll = FRMQueryString.requestString(request,"payroll");
    String oid = FRMQueryString.requestString(request,"oid");
    String dept = FRMQueryString.requestString(request,"dept");
...
<script language="JavaScript">
    ...
    function cmdEdit(oid, dept, payroll, fullname) {
        self.opener.document.<%=formName%>.<%=payroll%>.value = payroll;
        self.opener.document.<%=formName%>.<%=name%>.value = fullname;  
        self.opener.document.<%=formName%>.<%=dept%>.value = dept;
        self.opener.document.<%=formName%>.<%=oid%>.value = oid;
        self.opener.document.<%=formName%>.command.value="<%=Command.NONE%>";
...
function cmdUpdateDep(){
	...
	document.searchEmp.action="src_emp.jsp"; 
        ...
        <tr> 
          <td>
            ...
            for (int i = 0; i < listDept.size(); i++) {
                Department deptm = (Department) listDept.get(i);
                dept_key.add(deptm.getDepartment());
                dept_value.add(String.valueOf(deptm.getOID()));
            }
