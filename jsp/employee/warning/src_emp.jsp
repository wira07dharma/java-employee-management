...
public String drawList(Vector objectClass, int st){
	...
	for (int i = 0; i < objectClass.size(); i++) {
		...
		lstData.add(rowx);
		lstLinkData.add(String.valueOf(employee.getOID()) + "','" + employee.getFullName());
	}
	return ctrlist.draw();
}
%>
...
<script language="JavaScript">
    ...
    function cmdEdit(oid, fullname) {
        self.opener.document.<%=formName%>.<%=FrmEmpWarning.fieldNames[FrmEmpWarning.FRM_FIELD_WARN_BY]%>.value = fullname;
        self.close();
    }
function cmdAdd() {
            ...
            if (processDependOnUserDept) {
                if (emplx.getOID() > 0) {
                    if (isHRDLogin || isEdpLogin || isGeneralManager) {
                        ...
                        String whereClsDep="("+PstDepartment.TBL_HR_DEPARTMENT+"."+"DEPARTMENT_ID = " + departmentOid+")";
                        //update by satrya 2013-12-19 String whereClsDep="(DEPARTMENT_ID = " + departmentOid+")";
                        try {
