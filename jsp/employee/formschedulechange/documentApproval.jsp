...
String alamatJsp = FRMQueryString.requestString(request, "alamatJsp");
...
int approvedLevel = FRMQueryString.requestInt(request, "approvedLevel");
long oidEmpScheduleChange = FRMQueryString.requestLong(request, "oidEmpScheduleChange");
long scheduleType = FRMQueryString.requestLong(request, "scheduleType");
...
<%if (loggedInSuccess){   
    ...
    try{
      ...
      if (approvedLevel == 1){
        empScheduleChange.setApprovalDateApplicant(dateNew);
      } else if (approvedLevel ==2){
        empScheduleChange.setApprovalDateExchange(dateNew);
      } else if (approvedLevel == 3 ){
        empScheduleChange.setApprovalDateLevel1(dateNew);
        empScheduleChange.setApprovalLevel1Id(oidApprover);
      } else if (approvedLevel == 4 ){
        empScheduleChange.setApprovalDateLevel2(dateNew);
        empScheduleChange.setApprovalLevel2Id(oidApprover);
      }
      long n1 = PstEmpScheduleChange.updateExc(empScheduleChange);
      
      
      
          
      
      //cek sudah semua di diapprove kah?
      if (scheduleType == 0){
      if (empScheduleChange.getApprovalDateApplicant() != null && empScheduleChange.getApprovalDateExchange() != null && empScheduleChange.getApprovalDateLevel1() != null && empScheduleChange.getApprovalDateLevel2() != null){
               long scheduleIdOri = PstEmpSchedule.getSchedule(empScheduleChange.getOriginalDate().getDate(), empScheduleChange.getApplicantEmployeeId(), empScheduleChange.getOriginalDate()); 
               long scheduleIdExc = PstEmpSchedule.getSchedule(empScheduleChange.getNewChangeDate().getDate(), empScheduleChange.getExchangeEmployeeId(), empScheduleChange.getNewChangeDate()); 
               long periodeIdOri = PstPeriod.getPeriodIdBySelectedDate(empScheduleChange.getOriginalDate());
               long periodeIdExc = PstPeriod.getPeriodIdBySelectedDate(empScheduleChange.getNewChangeDate());
               PstEmpSchedule.updateSchedule(periodeIdOri, empScheduleChange.getApplicantEmployeeId(), empScheduleChange.getOriginalDate(), empScheduleChange.getNewChangeScheduleId());
               PstEmpSchedule.updateSchedule(periodeIdExc, empScheduleChange.getExchangeEmployeeId(), empScheduleChange.getNewChangeDate(), empScheduleChange.getOriginalScheduleId());
               //update status
             empScheduleChange.setStatusDoc(I_DocStatus.DOCUMENT_STATUS_FINAL);  
             long n = PstEmpScheduleChange.updateExc(empScheduleChange);
      }
      } else if (scheduleType == 1){
      if (empScheduleChange.getApprovalDateApplicant() != null && empScheduleChange.getApprovalDateLevel1() != null){
             //update status
             empScheduleChange.setStatusDoc(I_DocStatus.DOCUMENT_STATUS_FINAL);  
             long n = PstEmpScheduleChange.updateExc(empScheduleChange);
      }    
...
{
  //getApprovalDate();
  document.frmEmpScheduleChange.command.value="<%=String.valueOf(Command.SUBMIT)%>"; 
  document.frmEmpScheduleChange.action = "documentApproval.jsp";
  document.frmEmpScheduleChange.submit();
}

function cmdBack(oidEmpScheduleChange)	
{
  //getApprovalDate();
  document.frmEmpScheduleChange.command.value="<%=String.valueOf(Command.SUBMIT)%>"; 
  document.frmEmpScheduleChange.action = "emp_schedule_change_edit.jsp?oidEmpScheduleChange="+oidEmpScheduleChange;
  document.frmEmpScheduleChange.submit();
}	
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" height="100%" bgcolor="#FFFFFF">
  ...
  <tr>
    <td valign="middle" align="left">
      <form name="frmEmpScheduleChange" action="" onsubmit="window.status=''">
        <input type="hidden" name="command" value="<%=String.valueOf(iCommand)%>">  
        <input type="hidden" name="oidApprover" value="<%=String.valueOf(oidApprover)%>">
        <input type="hidden" name="approvedLevel" value="<%=String.valueOf(approvedLevel)%>">
        <input type="hidden" name="oidEmpScheduleChange" value="<%=String.valueOf(oidEmpScheduleChange)%>">
        <input type="hidden" name="isApprove" value="<%=String.valueOf(isApprove)%>">
        <input type="hidden" name="start" value="<%=String.valueOf(start)%>">
        <input type="hidden" name="scheduleType" value="<%=String.valueOf(scheduleType)%>">
        
        <input type="hidden" name="alamatJsp" value="<%=String.valueOf(alamatJsp)%>">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr> 
            <td width="100%" align="center"><font color="#FF0000" size="4"><b><font face="Verdana, Arial, Helvetica, sans-serif">.:: 
                    USER NOT FOUND Please create User for Login : <a href="javascript:cmdBack('<%=oidEmpScheduleChange%>')">BACK</a> ::.</font></b></font></td>
          </tr>
          <% } else {%>
          <tr> 
...
<script language="JavaScript">
 document.frmEmpScheduleChange.pass_wd.focus();
</script>
</body>
</html>
