...
<% 
/* 
 * Page Name  		:  training_exist_list.jsp
...
<%!
	...
	public String drawList(Vector objectClass, int st, SrcTraining srcTraining){
		...
		if(srcTraining.getTypeOfSearch()==0){
			...
			ctrlist.addHeader("Training Date","20%");
			...
			for (int i = 0; i < objectClass.size(); i++) {
				...
				rowx.add(Formater.formatDate(training.getStartDate(), "dd-MM-yy")+" // "+Formater.formatDate(training.getEndDate(), "dd-MM-yy"));
				rowx.add(training.getTrainer());
				
				lstData.add(rowx);
				lstLinkData.add(String.valueOf(employee.getOID()));
			}
		}
		//list yang belum dapat
		else{
			ctrlist.addHeader("No.","4%");
			ctrlist.addHeader("Payroll","6%");
...
<%
	...
	vectSize = sessTrainingHistory.getSize(srcTraining);
	...
	Vector listEmpTraining  = sessTrainingHistory.searchTrainingHistory(srcTraining, start, recordToGet);	
    //out.println("listEmpTraining "+listEmpTraining.size());
	...
	if(vectSize==listEmpTraining.size()){
		...
		Vector v = sessTrainingHistory.searchTrainingHistory(srcTraining, 0, vectSize);
		vct.add(v);
	}
...
<script language="JavaScript">
	...
	function cmdPrint(){
		window.open("<%=approot%>/servlet/com.dimata.harisma.report.employee.TrainingSpecialPdf?approot=<%=approot%>");
	}
	...
	function cmdListFirst(){
		...
		document.frm_employee.action="training_exist_list.jsp";
		document.frm_employee.submit();
	}
	...
	function cmdBack(){
		...
		document.frm_employee.action="src_training_exist.jsp";
		document.frm_employee.submit();
	}
                                  ...
                                  <td valign="top"> <!-- #BeginEditable "content" --> 
                                    <form name="frm_employee" method="post" action="">
                                      ...
                                      <table border="0" width="100%">
                                        ...
                                        <tr> 
                                          <td height="8" width="100%"><%=drawList(listEmpTraining, start, srcTraining)%></td>
                                        </tr>
                                        <%}else{%>
                                        <tr> 
                                            ...
                                            <table border="0" cellspacing="0" cellpadding="0" align="left" width="309">
                                              <tr> 
                                                ...
                                                <td width="78"><a href="javascript:cmdPrint()" class="command">Print 
                                                  Out</a></td>
                                                <% if(privAdd){%>
                                                <td width="9"><img src="<%=approot%>/images/spacer.gif" width="4" height="4"></td>
                                                <%}%>
                                                <% if(privPrint){%>
                                                <%}%>
                                              </tr>
