...
function cmdView(titleName){
	location="'<%=approot%>/report/payroll/page_report.jsp'";
}
    ...
    function fwLoadMenus() {
        ...
        fw_menu_5.hideOnMouseOut=true;
			...
			/*
			// report payroll
			window.fw_menu_6_13 = new Menu("Payroll",250,17,"Verdana, Arial, Helvetica, sans-serif",10,"#000000","#ffffff","#ffffff","#000084");
				<%@ page language = "java" %>
				<!-- package java -->
				...
				<%
					Vector listQueryTitle = new Vector(1,1);
					listQueryTitle  = PstQuery.listAll();
					String titleName = "";
					if(listQueryTitle.size() > 0 && listQueryTitle!=null){
						for(int i = 0 ; i<listQueryTitle.size();i++){
							Query title= (Query)listQueryTitle.get(i);
								titleName = title.getReportTitle();
							  %>
							   fw_menu_6_13.addMenuItem("<%=titleName%>", "location='<%=approot%>/report/payroll/page_report.jsp?titleName=<%=titleName%>'");
							<%
						}
					}
				%>
