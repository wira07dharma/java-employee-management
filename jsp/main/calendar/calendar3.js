...
function calendar3(obj_target, obj_target_yr, obj_target_mn, obj_target_dt,
				   obj_target_j, obj_target_m, obj_target_d, obj_target_mer) {
	...
	this.popup_first    = cal_popup_firsh3;
	...
	this.target_yr = obj_target_yr;
	this.target_mn = obj_target_mn;
	this.target_dt = obj_target_dt;
	this.target_j = obj_target_j;
	this.target_m = obj_target_m;
	this.target_d = obj_target_d;
	this.target_mer = obj_target_mer;
	
	this.time_comp = BUL_TIMECOMPONENT;
	this.year_scroll = BUL_YEARSCROLL;
	this.cal_path = PATH_DEFAULT;
...
function cal_popup_firsh3 (str_datetime) {
	if (str_datetime)
	...
	var obj_calwindow = window.open(
		''+this.cal_path+'calendar/calendar3.html?id=' + this.id + '&s=' + this.dt_selected.valueOf() + '&c=' + this.dt_current.valueOf(),
		'Calendar', 'width=200,height=' + (this.time_comp ? 220 : 190) +
...
function cal_popup3 (str_datetime) {
	...
	if (!this.dt_current) return;
	var obj_calwindow = window.open(
		'calendar3.html?id=' + this.id + '&s=' + this.dt_selected.valueOf() + '&c=' + this.dt_current.valueOf(),
		'Calendar', 'width=200,height=' + (this.time_comp ? 220 : 190) +
		',status=no,resizable=no,top=200,left=200,dependent=yes,alwaysRaised=yes'
	); 
	obj_calwindow.opener = window;
